...
- [x] A buffer read or edit on an entity that does not carry the element type fails with an error
      naming the entity and the type, and no `GetBuffer` invoke is made.
- [x] `ecs_get_buffer` requests READ-ONLY access, since it never writes.
- [x] `ecs_buffer_edit` keeps write access, and its empty-buffer guard no longer stands in for a
      presence check.
- [x] The presence check costs one invoke and runs on both buffer tools through one shared path, so
      a later buffer tool cannot skip it.
- [x] Verified live against the reference target: reading and editing an absent buffer each fail
      cleanly, and the game is still healthy afterwards.
- [x] Verified live that a buffer the entity does carry reads and edits exactly as before.
- [x] The checks-disabled behavior of `GetBuffer` on a missing component is recorded in the ECS
