...
**It is also worth reading for the practice a page recommends**: a reader who reaches a page unaided will follow its recommendation, and this game is built against several of them. The system-groups page's advice to reach another system through `[CreateAfter]` plus `GetExistingSystem` is the worked case — this game declares neither attribute, never runs the sorted creation path, and following the manual turns a working recipe into an exception out of `OnCreate`.

**Never authoritative for API shape, names, counts, or what this game's build does** — though the API reference is a legitimate *lead*. Two facts about the version are why.

**The declared version is not the version that runs.** `%CSII_ENTITIESVERSION%` and the mod project's manifest at `%CSII_UNITYMODPROJECTPATH%/Packages/manifest.json` declare Entities 1.3.10, Collections 2.5.7, Burst 1.8.23 and Mathematics 1.3.2 — what a mod compiles against. The shipped `Unity.Entities.dll` is **1.3.5 to 1.3.8** — the six Entities build paths the shipped Burst library embeds all name `com.unity.entities@1.3.10` and do not contest this, since a build path records where source sat on the build machine rather than what the directory held; `research/conflicts.md` carries the ruled entry. No Unity package assembly states its **package** version — the `VersionInternal` attribute entry 1 describes is Colossal's own, and the four packages that matter all report `AssemblyVersion("0.0.0.0")` — so re-dating one is an investigation: [dating a shipped Unity package](solutions/dating-a-shipped-unity-package.md) carries the method and the five traps, and the other three packages have not been dated this way. The gap is benign because nothing a mod plausibly calls changed across it, and this manifest is where a wider one would show first.
...
**One pairing sits just inside that line and is the honest way to use the docs for a runtime behaviour: the decompile proves the game does not intervene, and the docs supply the engine rule it therefore inherits.** `Resources.Load`'s case-insensitive path matching reaches a shipped sentence that way — the C# hands the name to the engine untouched, so the engine's rule is the only rule in play.

**Two whole families of page describe machinery this build does not run, and both read as though they describe it.**
**The default world bootstrap is the larger.** A grep for `ComponentSystemGroup|IRateManager|RateUtils|ScriptBehaviourUpdateOrder|ICustomBootstrap` across every `src/` assembly whose directory does not begin `Unity.` returns no file at all — the pattern names types, so it reaches a bare reference as well as a use. Colossal build a bare world and order systems imperatively, `World.Update()` is never called, and no rate manager exists. So every page on group ordering, `OrderFirst`/`OrderLast`, rate managers, fixed-step semantics and player-loop injection is inapplicable here, and a correction may never travel from one of them toward this game's ordering.
**The baking workflow is the subtler**, because its vocabulary collides word for word with Colossal's own prefab layer. `Baker<T>`, `SubScene`, `EntityScene`, `BakingSystem` and the entity-prefab pages describe a mechanism this game does not use — it names `Unity.Entities.Prefab` nowhere, calls no `EntityManager.Instantiate`, and builds instances from a cached archetype instead. A pass reading those pages without the guard imports the wrong mechanism while every word looks familiar.
...
- **Editor manual**, `https://docs.unity3d.com/2022.3/Documentation/Manual/<Page>.html`. The game runs Unity 2022.3.71f1, so this one pins exactly. It owns the low-level job system: `JobSystem.html` is the entry point.
  **`https://docs.unity3d.com/2022.3/Documentation/ScriptReference/<Page>.html` is the same prefix's other half** and takes the same version segment — it is where a question about a type's own API contract lands, `Object-operator_eq.html` and `Resources.Load.html` among them. A pass that does not know this shape exists composes an unversioned scripting-reference URL and hits the trap below.
- **Package manual**, `https://docs.unity3d.com/Packages/com.unity.<package>@<major.minor>/manual/<page>.html`, with `/api/<Type>.html` for the scripting reference and `/changelog/CHANGELOG.html` for when a behaviour changed. The packages that matter are `entities`, `collections`, `burst` and `mathematics`. Read `@1.3` for Entities, because that is the branch this game's assembly sits on; `@1.4` resolves and describes a package this build does not run. Read `@1.8` for Burst.
  **A branch URL serves that branch's newest patch, and the header on the page is where you read which** — `@1.8` served 1.8.29 and 1.8.30 within one session, against a declared 1.8.23; `@1.3` served 1.3.14 and 1.3.15. So the gap between the docs and the build moves under you, and a page that matters is worth a glance at its header.
...
**It is also where the code generators live, and for anything they emit the generator is the definition where the decompile is only a sample.** The DOTS source generators sit under `com.unity.entities@<version>/Unity.Entities/SourceGenerators/Source~/` and Burst's IL post-processor under `com.unity.burst@<version>/Unity.Burst.CodeGen/`. Every generated field name is one string interpolation in one of those files, so reading them enumerates a naming scheme **by construction** — which is the only way to clear a shipped set at its edge, since a grep of `src/Game` returns what this build happens to use and can never return what the generator can emit. The eight-hex Burst mangling and the completeness of the generated handle-name family were both settled this way and neither was settleable from the checkout.
...
## 16. The Burst-generated library

`Cities2_Data/Plugins/x86_64/lib_burst_generated.dll` under `%CSII_INSTALLATIONPATH%` — Burst's ahead-of-time output for this build, and the one part of the install no entry above reaches, since entry 5 stops at `Content/`.

**Authoritative for whether a given `[BurstCompile]` method was actually compiled**, which nothing else here can answer: the attribute in the decompile is a request rather than a record, and Burst silently declines a method it cannot see closed at build time. The library embeds fragments of what it compiled — string constants lifted from the compiled bodies, and absolute source paths from the build machine — so a byte-grep for a literal you can see in a method's decompiled body settles it.
**What it embeds is partial, so a hit settles a question and a miss only narrows one.** It holds 43 distinct `.cs` paths for a game with hundreds of Burst jobs. Grep it with a job you know is compiled as the control.
Its embedded package directory names look like a statement of which package version each assembly was built from, and that reading was ruled out — a vendored directory records where source sat, not what was in it. `research/conflicts.md` carries the ruled entry, against entry 13's version range.
