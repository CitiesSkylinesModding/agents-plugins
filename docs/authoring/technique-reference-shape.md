...
The form every reference under `plugins/cs2-modding/skills/cs2-modding/references/technique/` takes.
...
The header block above, and an entry file closing on `## What this reference hands to others` — the bridges, and what a reader leaves with.
...
**A trap provable only by running the game carries `UNVERIFIED:` in place of the `Source:` line**; that is that marker's job, and the two never appear together.
