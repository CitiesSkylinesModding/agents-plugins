...
- [ ] The orchestrator appended the reference's line — link plus a coarse one-line description — to the going-deeper list in the owning skill's `SKILL.md`: `plugins/cs2-modding/skills/cs2-modding/SKILL.md` for the two trunk families, `plugins/cs2-modding/skills/cs2-modding-ui/SKILL.md` for a UI reference. That file is shared and outside the authoring agent's folder, so it is the orchestrator's write rather than the agent's.
- [ ] The reference names no mod. It names no library except Harmony and PolySharp — under the plugin `AGENTS.md`'s carve-outs, which also name plainly the components the game or the official toolchain ships — with Harmony's package id and pinned version staying in `cs2-mod-project`.
- [ ] The reference writes its bridges as markdown links into the target topic's entry file, falling back to a backticked slug only where the target topic is not yet authored.
...
A ticket still states what these cannot: which claims in its topic rot, its own bridge targets by name, and whatever its subject makes non-obvious.
...
- **Bridges are markdown links into the target topic's entry file.** The lint asserts that every markdown link in the shipped tree resolves on disk and stays inside the plugin — relative paths cross families and skills freely. Link the entry file, never a sibling inside another topic's folder: the entry file is the only door its own header guards. A target topic not yet authored cannot be linked — the lint fails the dead link at edit time — so that bridge stays a backticked slug, and the run's closing coherence pass converts it once the topic lands.
...
**Check every bridge in the research file against the reference folders on disk, plus the structure document the run's tickets are scoped by for topics not yet authored, before authoring.** A discovery agent asked for a `## Bridge` section will name a topic that reads right and does not exist — the placement-definitions pass asserted `terrain-and-water`, whose material belongs to `environment-and-pollution`, and the same phantom had been sitting in the custom-tools research file undetected. The research file's slugs are what nothing checks, and an unchecked phantom there steers the authoring pass before any lint sees a link. Check the finished reference the same way — diff its bridge targets against that same approved set — since a link to a real but wrong topic resolves on disk and passes the lint.
