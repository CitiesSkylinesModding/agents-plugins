...
- **`## The map`** — concept to component, field and system, in one table or several where the concepts group, each row carrying the access shape the prefab-value rule requires.
  ...
  The same test rules a declared constant nothing reads: it ships as no number, and the literal the consuming line carries ships cited to that line (the 2026-08-18 ruling there).
