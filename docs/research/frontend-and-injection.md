...
Scaffold citations are to `@colossalorder/create-csii-ui-mod` version `1.0.0` (`create-csii-ui-mod/package.json:3`).
...
**`find` never evaluates an accessor and `get` always does.** `find` walks the map with `for (const t in s)` over property *names* (`:13462`), so a getter is never invoked; `get` reads `n[t]` (`:13376`). That is the mechanism behind `docs/SOURCES.md:111`.
