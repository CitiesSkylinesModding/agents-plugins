...
- `utilities-and-flow-networks` — `ElectricityFlowNode`, `ElectricityFlowEdge`, `WaterPipeNode`, `WaterPipeEdge` are four of the eighteen fixed anchors in the save query and four of the nineteen in `ClearSystem`, and their graphs are rebuilt on load by `ElectricityGraphSystem` and `ConnectedFlowEdgeSystem`.
