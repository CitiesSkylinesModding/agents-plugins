...
**Baseline.** Decompiled game version 1.6.2f1, read at the checkout `C:\Users\Morgan\Documents\Projets\DecompiledCitiesSkylines2`. Mod corpus (22 repositories under `C:\Users\Morgan\Documents\Projets\cs2-third-party-mods\`) read 2026-08-04. Wiki not fetched: it has no page on this subject (see `## Dead ends`).
A fourth source appears in three findings: **the running game**, a 1.6.0f1 development build driven over the Mono soft debugger on 2026-08-04, with a city loaded that was written by an older build. Its results are cited as `live 1.6.0f1` and carry no path; where the game-update sweep re-read one against a 1.6.2f1 build, the reading is stamped `Live 1.6.2f1` instead.
...
1. **`IEmptySerializable`** (`IEmptySerializable.cs` — a marker with no members). Not enableable, or enableable but also `ISerializeAsEnabled` → `EmptyComponentSerializer` (`ComponentSerializerLibrary.cs:57-63`). **Writes zero bytes of payload** (`EmptyComponentSerializer.cs:63-67` schedules no job at all, only accounting overhead). Presence is carried entirely by the archetype's serializer-index list.
2. **`IEmptySerializable` + `IEnableableComponent`** → `EnableableEmptyComponentSerializer<T>` for a component, `EnableableEmptyBufferElementSerializer<T>` for a buffer (`ComponentSerializerLibrary.cs:64-71`). Cost: **one bit per entity**, packed eight to a byte (`EnableableEmptyComponentSerializer.cs:29-54`).
3. **`ISerializable` on an `IComponentData`** → `ComponentDataSerializer<T>`, or `EnableableComponentDataSerializer<T>` when the type is enableable and not `ISerializeAsEnabled` (`ComponentSerializerLibrary.cs:80-83`).
4. **`ISerializable` on an `IBufferElementData`** → `BufferElementDataSerializer<T>` / `EnableableBufferElementDataSerializer<T>` (`ComponentSerializerLibrary.cs:84-87`).
5. **`ISerializable` on an `ISharedComponentData`** → `SharedComponentDataSerializer<T>` (`ComponentSerializerLibrary.cs:88-91`).
...
- **`IStrideSerializable`** (`IStrideSerializable.cs`) adds `int GetStride(Context)`. A non-zero stride declares the fixed byte size of one element, which switches the writer into a **column-filtered** mode: the payload is written through `NativeCompression` byte-plane filtering before compression (`ComponentDataSerializer.cs:34-56`, `BinaryWriter` / `BinaryReader.Read(NativeArray<byte>, int stride)` at `BinaryReader.cs:103-107`). It buys compression ratio. Returning 0 disables it. **The stride is also read on load**: `ComponentDataSerializer.Update` calls `GetStride(context)` with the load context (`:217`) and the deserialize job passes it to `reader.Read(nativeArray, m_Stride)` (`:163`), which de-interleaves the byte planes before any element is read (`BinaryReader.cs:105`). A stride that no longer matches what wrote the save yields garbage silently: the only guard is the byte-count check at `:171`, which a wrong-but-same-length stride passes. Verdict: corrected at the review gate of 2026-08-04; the original claim generalised from the write path.
...
That is the whole declaration as the corpus writes it — though `IQueryTypeParameter` is redundant in it, since `IComponentData` already derives from that interface (`src/Unity.Entities/Unity.Entities/IComponentData.cs:6`) and the decompile lists it only because ILSpy prints the transitive closure; a mod's own declaration wants `IComponentData, IEmptySerializable` alone (`PlopTheGrowables/Code/Systems/LevelLocked.cs:15`; the same shape at `PlopTheGrowables/Code/Systems/PloppedBuilding.cs:15`, `Anarchy/Anarchy/Components/PreventOverride.cs:13`, `Anarchy/Anarchy/Components/DoNotForceUpdate.cs`, `BetterBulldozer/BetterBulldozer/Components/PermanentlyRemovedSubElementPrefab.cs`, `CS2-Platter/Platter/Components/Initialized.cs`).
...
- **Every component's data for one archetype is one length-prefixed block.** `writer.Begin()` / `writer.End(block, out size)` and `reader.Begin(out size)` / `reader.End(block)`; `Begin` reads a four-byte size and `End` compares the position against it, resyncing and returning false on a mismatch (`BinaryReader.cs:30-48`). The caller turns false into `ComponentSerializerException("Data size mismatch when deserializing component ...")` (`ComponentDataSerializer.cs:185-188`). **So writing and reading different byte counts is detected, but only at the end of the whole block** — after every entity of that archetype has been read past the end of its own record.
- **Read and write must be exactly symmetric per entity.** There is no per-entity framing. `Read(out uint)` indexes the buffer directly and advances (`BinaryReader.cs:341-349`), and a read past your own record silently returns the next entity's bytes. **Corrected (review gate, 2026-08-26): the "in a Burst release build" qualifier was wrong and is retired — see the 2026-08-26 re-sweep entry below.**
...
`m_PreZoneType` is `Game.Zones.ZoneType`, which is itself `ISerializable` (`src/Game/Game.Zones/ZoneType.cs:6`), so it binds the nested-serializable overload and needs no unpacking; that it is also `IStrideSerializable` contributes nothing here, because a nested type is reached only through `Serialize`/`Deserialize` and the stride is asked of the component type alone (`ComponentDataSerializer.cs:217`).
...
- `IDefaultSerializable : ISerializable` adds `void SetDefaults(Context)` (`IDefaultSerializable.cs`). **`SetDefaults` is called for every registered system serializer whose type was absent from the save** (`EntityDeserializer.cs:419-427/608-612`) — which is exactly the case of a save written before the mod existed.
...
**Registration into an update phase is irrelevant here; existing when the library is built is not.** `SystemSerializerLibrary.Initialize` enumerates `world.Systems` once (`SystemSerializerLibrary.cs:32`) and ends `isDirty = false` (`:63`); the rebuild is gated on `isDirty` in `SerializerSystem.OnUpdate` (`SerializerSystem.cs:89-92`), and the sole vanilla trigger is `ModManager.AfterLoadAssembly` (`ModManager.cs:150`), fired before `IMod.OnLoad` (`:124`). So a system created during `OnLoad` gets its save section with or without a phase, while one created lazily later gets none — neither `Serialize` nor `SetDefaults` is ever called on it — unless the mod calls the public `SerializerSystem.SetDirty()` or `SystemSerializerLibrary.SetDirty()` (`SerializerSystem.cs:159` and `SystemSerializerLibrary.cs:66`) itself. Verdict: scoped at the review gate of 2026-08-04.
...
2. At the end of the block, `reader.End(block)` returns false and the job throws `ComponentSerializerException` (`ComponentDataSerializer.cs:185-188`).
3. The throw is raised inside `DeserializeComponentDataJob<TReader>` (`ComponentDataSerializer.cs:185-188`), a `[BurstCompile] IJob` scheduled off the main thread (`:251-259`) — **the attribute does not take (corrected 2026-08-26, ticket 38)**: the job's concrete type is closed only at runtime, by `MakeGenericType` plus `Activator.CreateInstance` in `ComponentSerializerLibrary.Initialize` (`src/Colossal.Core/Colossal.Serialization.Entities/ComponentSerializerLibrary.cs:82/96`), and Burst compiles no generic job it cannot see closed at build time; no `RegisterGenericJobType` exists anywhere in `src/Colossal.Core` or `src/Game`. So the throw is an ordinary managed exception, which is what keeps the shipped outcome a populated city with one component garbled rather than the process abort a Burst-compiled throw is in a player build. So `UpdateSystem`'s per-system try/catch (`src/Game/Game/UpdateSystem.cs:184-197`) is not what handles it — that catch wraps a system's own `Update()`. Verdict: corrected at the review gate of 2026-08-04; the original chain was inferred from the diagnostic rather than derived.
...
Note the asymmetry: a **system** section is deserialized on the main thread inside a per-section catch, so one bad system section is logged and the others still load (`EntityDeserializer.cs:570-573/591-598`). A component's throw is raised inside a scheduled job and does not travel that path at all.
...
Each is a `Colossal.Version`, and comparison is a single packed `long` ordered major, minor, build, release type, incremental, build date, build time (`src/Colossal.Core/Colossal/Version.cs:315-318/383-401`, layout at `:46`), so `>=` is a chronological test. On the wire a version is 13 bytes (`:403-411`).

Live 1.6.2f1 reports `current` as `1.6.2f1 (767.21d1) [6300.26419]`.

**Beside the version constants there is a second, coarser mechanism the game actually prefers for migrations: format tags.**
`Game.FormatTags` is a flat enum of 42 members in 1.6.2f1 (`src/Game/Game/FormatTags.cs`, count confirmed live), each naming a format change: `ShortLaneOptimization`, `HomelessAndWorkerFix`, `CargoPortCleanup`, `HouseholdPetLimit`, `TripPriority` and so on.
...
- **Older save, newer game** — the new build's tags are simply absent from the save, so `context.format.Has(tag)` is false and the migration systems gated on it run. Confirmed live: the loaded city was written by `1.3.3f1 (1166.85c0) [5841.24389]`, is loading under 1.6.2f1 with `purpose = LoadGame`, and `context.format.Has<FormatTags>(FormatTags.TripPriority)` is `false`.
...
`SystemUpdatePhase.Serialize` and `SystemUpdatePhase.Deserialize` (`src/Game/Game/SystemUpdatePhase.cs:26-27`) each fire exactly once, driven by `SaveGameSystem.OnUpdate` and `LoadGameSystem.OnUpdate` respectively (`SaveGameSystem.cs:67-84`, `LoadGameSystem.cs:50-57`). `mod-lifecycle-and-ordering.md` owns their position in the tree and their once-per-load firing; this file owns their contents.

**Serialize**, seven registrations (`src/Game/Game.Common/SystemOrder.cs:731-737`):
...
**Deserialize**, three bands used deliberately (`SystemOrder.cs:738-898`):

- Front band (`UpdateBefore`): `AllowBarrier<DeserializationBarrier>` (`:738`), then 57 `PreDeserialize<T>` wrappers, then `ClearSystem` and `GameModeSystem` (`:796-797`).
- Middle band (`UpdateAt`, `:799-853`): `SerializerSystem`, `ReadSystem`, then the rebuilders — `FilterLoadedSystem`, `ResolvePrefabsSystem`, `RequiredComponentSystem`, and some thirty systems that reconstruct derived state that is deliberately not saved (`SubLaneSystem`, `SubObjectSystem`, `HouseholdCitizenSystem`, `OwnedVehicleSystem`, `ConnectedRouteSystem`, `ElectricityGraphSystem` …), with the shipped migration systems last (`:843-853`).
- Back band (`UpdateAfter`, `:854-898`): the resets, then 32 `PostDeserialize<T>` wrappers.
...
The world is created once at boot and destroyed only at shutdown (`src/Game/Game.SceneFlow/GameManager.cs:598/765`), so every load runs against the world the previous city left behind. `ClearSystem.OnUpdate` is a single `EntityManager.DestroyEntity(m_ClearQuery)` (`src/Game/Game.Serialization/ClearSystem.cs:57-60`) over a query whose `Any` list is **twenty fixed vanilla types and nothing else** (`:23-53`) — it does not gain the mod types the save query gains.
...
an entity created carrying only `PlopTheGrowables.LevelLocked` (plus Unity's automatic `Simulate`) was matched by `SerializerSystem.m_Query` (26 query types) and **not** matched by `ClearSystem.m_ClearQuery` (21 query types at 1.6.0f1; 1.6.2f1 adds `MeshColorPalette` to the `Any` list, `ClearSystem.cs:46`);
...
`src/Game/Game.Serialization.DataMigration/` holds twelve systems. Eleven are registered `UpdateAt` in `Deserialize` (`SystemOrder.cs:843-853`); `ResidentPseudoRandomSystem` alone sits in the back band (`:860`):
...
2. Failing that, the stored name is looked up in a **type table** built from `[FormerlySerializedAs]` attributes (`ComponentSerializerLibrary.cs:106-115`, `SystemSerializerLibrary.cs:71-79`, attribute at `FormerlySerializedAsAttribute.cs`), **progressively trimming from the last comma** — so `Ns.Type, Asm, Version=…, Culture=…, PublicKeyToken=…` is retried as `Ns.Type, Asm, Version=…`, then `Ns.Type, Asm`, then `Ns.Type`. A `[FormerlySerializedAs]` may therefore name the old type at whatever precision is convenient. **The trimming does nothing for a type carrying no such attribute**, though, since the table is built from attribute old names alone (`ComponentSerializerLibrary.cs:113-120`, `UpdateTypeTable`) — so it is not what makes an assembly version bump safe. **Stage 1 is.** The engine registers a blanket `AssemblyResolve` handler before any managed code runs, retrying a failed bind on `AssemblyName.Name` alone and dropping version, culture and token (`src/UnityEngine.CoreModule/UnityEngine/ClassLibraryInitializer.cs:32-45`, `InitAssemblyRedirections`), so a `Type.GetType` on a stored name whose version no longer matches resolves without ever reaching the table.
...
A saved city is **not one file**. `GameManager.Save` builds a transient asset database, fills it, and packages the whole of it as one `PackageAsset` under `Saves/<user path>` (`src/Game/Game.SceneFlow/GameManager.cs:950-1012`, path from `src/Game/Game.Assets/SaveGameMetadata.cs:14`). Inside it:

- **The entity stream**, a `SaveGameData` asset — extensions `.SaveGameData` and `.cds` (`src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/SaveGameData.cs:5-7`), written to `saveGameData.GetWriteStream()` (`GameManager.cs:965`).
...
- **A preview texture**, captured and compressed into the same package (`GameManager.cs:967-974`).
...
- **`contentPrerequisites`** — the DLC and content packs. Filled from `SaveGameSystem.referencedContent`, mapped to prefab names (`GameManager.cs:934-942`), and checked before load by `ArePrerequisitesMet` (`GameManager.cs:1846-1865`).
...
**Technique siblings.**
`ecs-in-this-game` owns the component kinds this file's mechanisms attach to — `IComponentData`, `IBufferElementData`, `ISharedComponentData`, `IEnableableComponent` — and the archetype and chunk model that `CreateEntities` reconstructs on load (`EntityDeserializer.cs:466-525`). It already carries the `ISerializeAsEnabled`-adjacent enableable-bit story and the `UpdateFrame` shared component, whose serializer is the only `SharedComponentDataSerializer` the game exercises.
...
Tier 2, engine claims only. Unity docs fetched live 2026-08-26 at the version-pinned URLs `docs/SOURCES.md` entry 13 fixes; decompile read the same day at 1.6.0f1; Unity package sources (entry 15) read at `com.unity.entities@1.3.10`. No live game was used. The save format, the version constants, the format tags, the phase bands and the whole census file are Colossal's and were left alone.

- **"Burst-compiled" was one word that would have made its own paragraph's conclusion false, and it is gone.** Corrected in place at the finding above. Burst 1.8's BC1370 entry states that in a Player build a `throw` in Burst-compiled code "becomes an abort, which crashes your application" — so if the deserialize job really were Burst-compiled, a mod that skipped its version int would crash the player rather than load a city with one component garbled, which is the opposite of what the reference teaches and of what the paragraph concludes two lines later. The decompile settles that it is not: the job carries `[BurstCompile]` (`src/Colossal.Core/Colossal.Serialization.Entities/ComponentDataSerializer.cs:83`), **but that attribute is a request rather than a record**, and the concrete specialization exists only at runtime — `ComponentSerializerLibrary.Initialize` builds each serializer with `MakeGenericType` plus `Activator.CreateInstance` (`ComponentSerializerLibrary.cs:82/96`), then schedules through a generic method. [Burst's generic-jobs page](https://docs.unity3d.com/Packages/com.unity.burst@1.8/manual/compilation-generic-jobs.html) states the two conditions this fails, and no `RegisterGenericJobType` — the escape hatch that would register the specializations — exists anywhere in `src/Colossal.Core` or `src/Game`. The shipped Burst library corroborates and no more: `Cities2_Data/Plugins/x86_64/lib_burst_generated.dll` carries two vanilla flow jobs' error literals verbatim and none of this serializer's three, but it holds only 43 distinct `.cs` paths for a game with hundreds of Burst jobs, so an absence there is not proof — the reflection derivation is what settles the finding, and the grep only agrees with it. Consistency check: the game demonstrably saves and loads, which it could not do if these jobs aborted. The `UNVERIFIED:` marker on which log the exception arrives under is untouched — the Burst page answers only for Burst-compiled throws, and the managed job wrapper is a bare `data.Execute()` with no `try`/`catch`.
- **"In a Burst release build" attributed to the build a silence the format itself produces.** The qualifier was wrong twice: the shipped `NativeArray` indexer is a bare `UnsafeUtility.ReadArrayElement` in every path, and — the half that settles it — a read past your own record is still **inside the same buffer**, so no bounds check in any build could fire. The silence is the format's, not the build's, and the corrected sentence says so.
- **A managed component is the one thing that fails loudly, and the drop-without-a-word sentence did not carry the exception.** [The managed-components page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/manual/components-managed.html) establishes that a managed component is a `class` inheriting `IComponentData`; every generic serializer in this file's mechanisms table is constrained to an unmanaged struct — **corrected in place (review gate, 2026-08-26): this read "every serializer", and the plain non-enableable `IEmptySerializable` row takes the non-generic `EmptyComponentSerializer` instead (`ComponentSerializerLibrary.cs:57-62`), so a managed component implementing only that interface registers cleanly. Add `IEnableableComponent` and it goes back through `EnableableEmptyComponentSerializer<T>`, which is `where T : unmanaged`, so the library falls over as before**, so declaring a managed component that implements one of the save interfaces takes the whole serializer library down rather than dropping just that type. Placed beside the sentence that tells a reader what happens to a component the library will not serialize, rather than as a new table row. **Decompile side, added at the review gate of 2026-08-26 — this bullet shipped a doc page as its only citation, which is the one thing entry 13's precedence forbids:** the constraint is `ComponentDataSerializer<TComponentData> where TComponentData : unmanaged, IComponentData, ISerializable` (`src/Colossal.Core/Colossal.Serialization.Entities/ComponentDataSerializer.cs:11`), and the whole-library half is that the constraint is met through an unguarded `MakeGenericType`/`Activator.CreateInstance` loop with no `try`/`catch` around it (`ComponentSerializerLibrary.cs:60-105`), so the first bad type takes the loop down and every serializer after it goes unregistered.
- **"A mod component is enough to get its entity saved" needed one qualification.** [The enableable page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/manual/components-enableable-use.html) states that an entity with `T` disabled matches queries as if it does not have `T` at all, and [the chunk-list page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/manual/systems-looking-up-data.html) that chunks with zero relevant entities enabled are excluded from the output list. The shipped sentence now stops at the query rule the decompile settles. **It deliberately says nothing about whether a disabled entity rides in on an archetype-mate's chunk** — the two code paths agree that `GatherChunksJob` admits a chunk on a non-zero mask and admits it whole, and that Colossal then discard the query's mask and rebuild it as all-ones ANDed with `PrefabData` alone, but whether that happens is a runtime layout fact and no live run was made. Not shipped without one.
- **A mechanism no shipped reference owned, and it is a correctness trap.** `EntityDeserializer` holds an `ExclusiveEntityTransaction` across every system section (`src/Colossal.Core/Colossal.Serialization.Entities/EntityDeserializer.cs:184`, released at `:199`), so a mod's `Deserialize` and `SetDefaults` run with the `EntityManager` off limits — and `AssertIsExclusiveTransaction`, whose package-source body throws `InvalidOperationException`, is an empty `[Conditional]` method here (`src/Unity.Entities/Unity.Entities/EntityManager.cs:809-813`). So a read there neither throws nor waits for the loader's own jobs. Landed as a three-line block in `## Whole-system state`, pointing at the migration split the file already carries.
- **Dead ends, including the boundary check the ticket pointed this sweep at.** The reference attributes entity remapping entirely to Colossal's writer and reader, **correctly**: `SerializeUtility`, `EntityRemapUtility` and `IsChunkSerializable` have zero hits across `src/Game` and `src/Colossal.Core`. Blob assets have no reader/writer overload and no vanilla component holds one, so the file's silence about them is right rather than a gap. Cleanup components against the game's own destroy pass is `ecs-in-this-game`'s boundary and no edit was proposed there.
- **Source-list feedback, including one entry-13 contradiction this pass would not decide.** Entry 13 held on how to read the docs, and its precedence rule did real work — read alone, the Burst page would have turned a right sentence into a crash claim. Two amendments proposed. First, no entry on the list names an artifact that can settle whether a `[BurstCompile]` job was actually compiled for this build; `Cities2_Data/Plugins/x86_64/lib_burst_generated.dll` can, and entry 5 stops at `Content/` and never reaches `Plugins/`. Second, and larger: **that same library carries six absolute build-machine source paths, all under `ColossalPackages\com.unity.entities@1.3.10` — the declared version — against entry 13's "The shipped `Unity.Entities.dll` is 1.3.5 to 1.3.8".** This pass did not rule on it and appended it to `conflicts.md` instead: a vendored directory name records where source sat rather than what was in it, and settling it needs the dating method entry 13 points at. If 1.3.10 wins, entry 13's "the declared version is not the version that runs" paragraph loses its worked example and needs rewriting rather than patching.
  **Ruled (2026-09-01, the maintainer's grilling; conflicts.md).** Entry 13's 1.3.5–1.3.8 range survives: a build path dates a vendored directory's name and not the compiled assembly, so the six `@1.3.10` paths do not contest a derivation that compared the shipped binary itself — the number's first contestation, rejected. The library is listed as `SOURCES.md` entry 16, and no shipped sentence changes.

## Review gate 2026-08-26: the enableable exclusion shipped as the answer this file said it was withholding

The re-sweep bullet above records "whether that happens is a runtime layout fact and no live run was made. Not shipped without one" — and the shipped line went out as "unless it is enableable and disabled, since a disabled enableable component matches nothing", the strongest possible answer to the question the bullet claims to leave open, and the wrong one.

**The decompile settles it, and no live run could have.** Admission is per chunk: `GatherChunksJob.Execute` pushes the whole `ArchetypeChunk` on a non-zero mask (`src/Unity.Entities/Unity.Entities/GatherChunksJob.cs:59-63`), and `ToArchetypeChunkListAsync` returns only the chunk list (`EntityQueryImpl.cs:309-341`), so the mask never leaves that job. Colossal then rebuild it as every entity in the chunk — `reference.ULong0 = ulong.MaxValue >> 64 - archetypeChunk.Count` (`src/Colossal.Core/Colossal.Serialization.Entities/EntitySerializer.cs:43-44`) — and `CheckEnabledMaskJob` ANDs back only the `requireEnabled` types, which the one call site fixes to `PrefabData` alone (`src/Game/Game.Serialization/SerializerSystem.cs:102`). The query passes no `IgnoreComponentEnabledState` (`SerializerSystem.cs:198-209`), so enabled state is consulted at gather time and discarded immediately after.

So a disabled entity is written whenever any archetype-mate in its chunk matches. What no run could settle is which entities happen to share a chunk — a per-case fact a single run would exhibit one instance of. "Not shipped without one" set a bar the evidence could never clear and the code did not need, which is [a runtime question answered from first principles](../solutions/a-runtime-question-answered-from-first-principles.md) in its inverse form.

There is a second, unconditional path: where the archetype also carries a non-enableable type from the `Any` list — a `PrefabRef`, a vanilla anchor, any non-enableable mod serializable — the enableable `Any` count is zero (`MatchingArchetype.cs:86-112`) and enabled state is never consulted at all.

**The empty-flag recommendation nine lines above the corrected sentence needs no edit.** `EnableableEmptyComponentSerializer` writes one bit per entity and reads it back as the disabled state (`EnableableEmptyComponentSerializer.cs:42-48`, `:92-95`), so whenever the entity is in the save, false round-trips as false. The failure case — a mod-owned entity whose flag is its only `Any` match, in a chunk where every entity is disabled — is a per-case layout fact rather than a rule, and correcting the exclusion sentence covers the reader who would have met it.

**Second pass: the correction was itself absolute in the other direction.** It shipped as "disabling an enableable one does not take it back out", which is false exactly where the first version was true — a chunk in which *no* entity matches is never gathered at all (`GatherChunksJob.cs:59-63`), so a mod-only archetype whose sole serializable enableable type is disabled everywhere is silently dropped. Both directions depend on chunk co-tenancy, which is nothing a mod controls: disable one of a hundred such entities and it is saved, disable all of them and they are gone. That is the claim's third wrong version, so it now states the thing a reader acts on — the bit is not a lever on save membership either way — and routes them to the exclusion recipe instead of describing an outcome that varies per chunk.
