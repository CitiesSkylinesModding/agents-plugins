...
Not every vanilla system is created by the registration pass. That pass does precede mods (`src/Game/Game.SceneFlow/GameManager.cs:615` `CreateSystems()` then `:618` `InitializeModManager()`, with `CreateSystems` being `SystemOrder.Initialize`), but a residue of vanilla systems is reached only through a lazy `GetOrCreateSystemManaged` outside it — established by enumerating every registered type in `SystemOrder` against every `GetOrCreateSystemManaged`/`CreateSystemManaged` call site under `src/Game`, then classifying each unregistered type by its enclosing method, which puts the boot-time ones (an `OnCreate`, or a method one calls) aside. What is left is a small class rather than one system: `CameraCollisionSystem` from `CameraController.Awake` after `await GameManager.instance.WaitForReadyState()` (`src/Game/Game/CameraController.cs:306-308`, `:333`), `CinemachineRestrictToTerrain` from `Start()`, `UpkeepPropertyBinderSystem` from `PrefabUISystem.OnGameLoaded` (`:1343`, `:1592`), `ReplacePrefabSystem` from `PrefabSystem.OnUpdate` (`:762`, `:805`), `TerrainDebugSystem` from `GameManager.OnGUI` behind `shouldUpdateWorld`, and the debug generics.
