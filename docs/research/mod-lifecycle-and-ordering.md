...
`VOLATILE:` goes on the per-phase system names and counts, which is where the version moves; the tree's shape is architecture and carries no marker.
...
- **`PrefabUpdate`** — 23 [6]. `TextureStreamingSystem`, `GeometryAssetLoadingSystem`, `PrefabInitializeSystem`, `MeshSystem`, `UIInitializeSystem`, `ObjectInitializeSystem`, `ZoneSystem`, `ZonePrefabInitializeSystem`, `TriggerPrefabSystem` (`:1004-1026`). Runs whenever `PrefabSystem` has pending prefab updates (`PrefabSystem.cs:757-763`). Where prefab-shaping systems go.
...
Vanilla registers nine `PreDeserialize<T>` in the Deserialize front band — the six spatial `SearchSystem`s, `InstanceCountSystem`, `PathfindQueueSystem`, `PathfindResultSystem` (`SystemOrder.cs:738-746`) — which is the pattern's purpose: **clear a mod's index or cache before the loader starts writing entities into it.**
...
- **`citizens-and-households`**, **`economy-and-companies`** and **`city-services-and-coverage`** all sit in `GameSimulation`, the 297-registration phase, and all three are where forking a vanilla simulation system is the realistic change. Anything written there needs the fork recipe from this file — disable, anchor with the two-type form, and copy the original's interval _and_ offset (`CitizenBehaviorSystem.cs:1016-1024` against `Time2WorkCitizenBehaviorSystem.cs:63/65`) — plus the fact that a `GameSimulation` change is not visible to the modification phases until the following frame.
