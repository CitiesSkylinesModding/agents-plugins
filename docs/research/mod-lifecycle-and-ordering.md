...
**Baseline.** Decompiled game version 1.6.2f1. Mod corpus (20 repositories under `C:\Users\Morgan\Documents\Projets\cs2-third-party-mods\`) read 2026-08-02. Wiki fetched live 2026-08-02 — the bot challenge did not win this time, so the `Systems` and `ECS - Entity Component System` pages are cited from the live pages rather than through `survey-wiki-inventory.md`'s snapshot.

**Re-swept 2026-08-03 (the new-sources resweep) against `docs/SOURCES.md`, which did not exist when this pass ran.** This topic makes no frontend claim, so the two sources the original pass could not have reached bear on it at exactly one seam: `AddUIModule`, where `@colossalorder/create-csii-ui-mod/template/types/modding.d.ts` — the scaffold cite expanding through the npm-global junction to `<install>/Cities2_Data/Content/Game/.ModdingToolchain/npx-create-csii-ui-mod/`, the game's own files, versioned by the install and not by npm (conflicts.md's ruled scaffold-citation entry) — declares the `ModuleRegistry` a loaded module talks to — `get`, `add`, `override`, `extend`, `append`, plus `hasAppend`, `find` and `reset`, and seven `AppendHookTargets`. That surface belongs to `frontend-and-injection`; what belongs here is the C# gate, amended at the deferral finding below. Nothing else in this file needed either source.
...
**Verdict: the wiki's `OnCreateWorld()` does not exist on `IMod` at 1.6.2f1.**
...
**Instantiation.** `ModInfo.Load` then does `asset.assembly.GetTypesDerivedFrom<IMod>()` (`src/Game/Game.Modding/ModManager.cs:120`), which is `assembly.GetTypes().Where(t => typeof(IMod).IsAssignableFrom(t))` (`src/Colossal.Core/Colossal.Reflection/ReflectionUtils.cs:508-513`) — reflection over every type, transitive, nested types included, and with no abstract-class filter.
So the loader instantiates **every** `IMod` implementation in the assembly, not one, and calls `OnLoad` on each in turn (`ModManager.cs:153-159`).
...
Other loading gates on the same asset, all read before `Load` proceeds (`ModManager.cs:96-117`):
...
Failing each sets a distinct `ModInfo.State` and stops (`ModManager.cs:100-117`).
...
Rots: the `ModInfo.State` member names — `Unknown`, `Loaded`, `Disposed`, `IsNotModWarning`, `IsNotUniqueWarning`, `GeneralError`, `MissedDependenciesError`, `LoadAssemblyError`, `LoadAssemblyReferenceError` (`ModManager.cs:33-44`); they are also the localisation keys the failure dialog interpolates (`:296`).

### The mod class is never constructed, but its systems are

`ModInfo.Load` builds each instance with `FormatterServices.GetUninitializedObject(item)` (`ModManager.cs:122`).
...
Unity constructs a system with `Activator.CreateInstance(systemType)` (`src/Unity.Entities/Unity.Entities/TypeManager.cs:2988-2996`), so a mod's own systems **do** get their parameterless constructor and their field initialisers, and a system without a parameterless constructor throws at creation.
...
The whole of `GameManager.Initialize()` is one async method (`src/Game/Game.SceneFlow/GameManager.cs:577-672`), and the relevant order is fixed by its statement order:

1. `CreateWorld()` (`:598`, body at `:2378-2389`) — `new World("Game")`, then `PrefabSystem`, `UpdateSystem`, `LoadGameSystem`, `SaveGameSystem` created by hand.
2. `new ModManager(configuration.disableCodeModding)` (`:611`).
3. `CreateSystems()` (`:621`, body at `:2391-2398`) — `SystemOrder.Initialize(m_UpdateSystem)` (`:2395`), which is where every vanilla system is created and registered.
4. `InitializeModManager()` (`:624`, body at `:674-680`) — `m_ModManager.Initialize(m_UpdateSystem)`, which registers, loads and calls `OnLoad` on every mod (`ModManager.cs:245-365`, `:434-462`).
5. `LoadPrefabs(AssetDatabase.global)` (`GameManager.cs:630`).
6. `m_State = State.WorldReady` (`:632`), then `onWorldReady?.Invoke()` (`:635`).
...
- **Nothing has updated yet.** `shouldUpdateWorld => m_State >= State.WorldReady` (`:296`), and that state is only set at `:632`, after `OnLoad`. So no phase has run a single time when `OnLoad` executes: a mod may not read simulation state, query a populated world, or assume any vanilla system has done its `OnUpdate` work.
...
Rots: the `GameManager.State` member names and their declaration order — re-read `src/Game/Game.SceneFlow/GameManager.cs:124-134`.
...
`m_ModManager?.Initialize(m_UpdateSystem, reinitialize: true)` (`:1643`) runs on a playset or mod-status change and re-runs `RegisterMods()` then `InitializeMods()` (`ModManager.cs:264-265`).
`RegisterMods` keeps an existing entry's state through `ModInfo.TransferState` and gives only a genuinely new asset a fresh `ModInfo` (`ModManager.cs:400-431`, the transfer at `:417`), and `ModInfo.Load` returns immediately unless `state == State.Unknown` (`:96-99`).
So an already-loaded mod is skipped rather than disposed and re-loaded, and `OnLoad` runs exactly once per mod per process.
Disabling a loaded IL assembly mid-session cannot unload it: that branch calls `RequireRestart()` (`GameManager.cs:1587`, the notification at `ModManager.cs:527-548`), while `!isLoaded && isInActivePlayset` queues the assembly for a first load (`GameManager.cs:1591`).
...
This is the first of the two conflict entries that bound this topic, since ruled (`conflicts.md:787`), whose claim originates in hand-written orientation prose at `DecompiledCitiesSkylines2/AGENTS.md:56`.
...
- **`SystemOrder.cs` is the only vanilla registrar.** A count of `\.UpdateAt<|\.UpdateBefore<|\.UpdateAfter<|RegisterGPUSystem` over `src/Game/` returns 1015 occurrences in `Game.Common/SystemOrder.cs` and 3 in `Game/UpdateSystem.cs` (its own internal call sites), and nothing anywhere else. There is no second ordering mechanism to have missed.
...
`UpdateSystem` exposes five **phase** registration methods (`src/Game/Game/UpdateSystem.cs:141-164`), plus a `RegisterGPUSystem` pair (`:128`, `:133`) that drives `IGPUSystem.OnSimulateGPU` off the render pipeline's begin-frame event rather than off a phase. No phase method reaches that drive, and it registers nothing into a phase, so the two are complementary: the one vanilla GPU system takes both, `RegisterGPUSystem<WaterSystem>()` (`SystemOrder.cs:46`) and `UpdateAt<WaterSystem>(SystemUpdatePhase.PostSimulation)` (`:631`), the first for `OnSimulateGPU` (`WaterSystem.cs:1578`) and the second for `OnUpdate` (`:1609`).
...
`Refresh()` sorts `m_Systems` by `(phase, addIndex)` (`UpdateSystem.cs:29-37`, `:299`) and walks each phase's run building `m_Updates`, splicing in the `m_RefMap` entries recursively as it goes (`:292-363`, `:365-438`); `Update(phase)` then walks one contiguous range of `m_Updates` and returns (`:172-183`).

Three rules follow, and they are the ones a reader needs:

1. **The bands never interleave.** The ±1,000,000 offsets are far larger than the registration counter will ever reach — vanilla makes 1015 registrations total — so every `UpdateBefore` in a phase runs before every `UpdateAt`, and every `UpdateAt` before every `UpdateAfter`. Nothing runs "before the phase" or "after the phase" despite the names; all three land inside it. The wiki says otherwise ("`UpdateBefore<T>()` - registers a system to run before a phase", https://cs2.paradoxwikis.com/Systems). **Verdict: the decompile wins; the three methods choose a band within one phase, and a mod that reads the wiki literally will expect its system to run outside the phase it named.**
...
3. **A mod always registers after all of vanilla**, because `SystemOrder.Initialize` runs at `GameManager.cs:621` and mod `OnLoad` at `:624`. So a mod's `UpdateAt` lands after every vanilla `UpdateAt` in that phase, its `UpdateBefore` after every vanilla `UpdateBefore` but ahead of all vanilla `UpdateAt`, and its `UpdateAfter` last of everything. This is the only lever a mod has over vanilla ordering short of anchoring.
...
The `Deserialize` phase uses all three bands as a designed sandwich: `UpdateBefore<PreDeserialize<T>>` (`:739-747`), then `UpdateAt` readers and migrations (`:802` among them), then `UpdateAfter<PostDeserialize<T>>` (`:864`, `:893-898`).
...
- `Traffic/Code/Mod.cs:93/94` → `ApplyNetSystem` at `:716` (ApplyTool). ✓
- `Recolor/Recolor/Mod.cs:149-154` is the deepest chain: three systems anchored after vanilla `MeshColorSystem` (`:649`, PreCulling), then `CustomMeshColorSystem` anchored after one of _those_ — anchoring to the mod's own system, resolved by the recursion.
...
- `CS2-MoveIt/Code/MoveIt/Mod.cs:92` → `ApplyPrefabsSystem`, which vanilla registers with `UpdateAfter` (`:299`) rather than `UpdateAt`; the splice works regardless of which band the anchor sits in.
...
**Root.** `GameManager` is a `MonoBehaviour` (`GameManager.cs:61`) and makes the only four calls that are not nested inside another system's update: `Update()` → `UpdateWorld()` → `MainLoop` (`:2405`), then `UpdateUI()`, then `PostUpdateWorld()` → `Cleanup` (`:2413`); and `LateUpdate()` → `LateUpdateWorld()` → `LateUpdate` (`:2421`) then `DebugGizmos` (`:2422`).
`MainThreadDispatcher.UpdateUpdaters()` sits at `:723`, outside the `shouldUpdateManager` guard, so deferred callbacks run even when the world does not.
...
```
Unity Update()  →  GameManager.Update()                         GameManager.cs:712-725
  MainLoop                                                      GameManager.cs:2405
    ...
    (UpdateAt band, in registration order)
      ...
      SaveGameSystem           :69   →  Serialize               SaveGameSystem.cs:82
                                          BeginPrefabSerializationSystem :734
                                            →  PrefabReferences BeginPrefabSerializationSystem.cs:272
      DebugWatchSystem         :71
    (UpdateAfter band)
      PrepareCleanUpSystem     :50
  (GameManager.UpdateUI)
  Cleanup                                                       GameManager.cs:2413

Unity LateUpdate()  →  GameManager.LateUpdate()                 GameManager.cs:727-733
  LateUpdate                                                    GameManager.cs:2421
      DebugSystem              :70
      ...
      CompleteRenderingSystem  :75   →  CompleteRendering       CompleteRenderingSystem.cs:51
      GizmosSystem             :76
      AutoSaveSystem           :77
  DebugGizmos                                                   GameManager.cs:2422
```
...
- **`MainLoop`** — 20 [3]. The frame's spine, and the phase most of the others are driven from — not all, since `LateUpdate`'s `SimulationSystem` drives the simulation phases and `CompleteRenderingSystem` the rendering-completion one. A mod registering here (`FindIt-CSII/FindIt/Mod.cs:70/71`, `RoadBuilder-CSII/RoadBuilder/Mod.cs:65/74`, `ExtraAssetsImporter/EAI.cs:162`) lands after `RenderingSystem` and `SaveGameSystem` and before `PrepareCleanUpSystem`. By the time it fires, `Cleanup`, `LateUpdate` and `DebugGizmos` are all still ahead of it — so the whole simulation, driven from `LateUpdate`, has not run for that frame.
- **`Raycast`** — 1 [3], `ToolRaycastSystem` (`:1058`). First of the `UpdateAt` band in MainLoop, so nothing else in that band has run this frame — the front band has, including `EndFrameBarrier` (`SystemOrder.cs:49`), whose playback of the previous frame's deferred structural changes therefore precedes any `Raycast` system. The barrier is also shut at that point, and `AllowBarrier<EndFrameBarrier>` reopens it at `SystemOrder.cs:62` — after `UpdateAt<RaycastSystem>` at `:55` in the same band. Two cautions on the mechanism, each an easy over-claim: the shut is `m_IsAllowed = false;` on the first line of `SafeCommandBufferSystem.OnUpdate`, unconditional and ahead of `base.OnUpdate()`, so it is not the playback that causes it (the playback itself returns early when `m_PendingBuffers->IsEmpty`, `EntityCommandBufferSystem.cs:58-61`); and the guarded `public new EntityCommandBuffer CreateCommandBuffer()` (`SafeCommandBufferSystem.cs:16`) **hides** rather than overrides the non-virtual base (`EntityCommandBufferSystem.cs:18`), so the throw binds on the reference's static type — a caller holding the barrier as `EntityCommandBufferSystem` gets a buffer with no throw and no log, playing back one frame late. `UpdateSystem.Update` does swallow and log the throw when it happens (`UpdateSystem.cs:189`). A mod raycast system belongs here (`Traffic/Code/Mod.cs:88`, `NodeController/NodeController/Mod.cs:69`).
- **`PrefabUpdate`** — 23 [6]. `TextureStreamingSystem`, `GeometryAssetLoadingSystem`, `PrefabInitializeSystem`, `MeshSystem`, `UIInitializeSystem`, `ObjectInitializeSystem`, `ZoneSystem`, `ZonePrefabInitializeSystem`, `TriggerPrefabSystem` (`SystemOrder.cs:1005-1027`). Driven every `MainLoop` frame, unconditionally: `PrefabSystem.OnUpdate` calls `UpdatePrefabs()` — which returns early on an empty update map — then drives the phase regardless, and gates only `ReplacePrefabSystem.FinalizeReplaces()` on there having been updates (`PrefabSystem.cs:755-763`). `PrefabSystem` registers no `RequireForUpdate`, so nothing suppresses its own update either. Verdict: an earlier pass of this file read the early return in `UpdatePrefabs()` as gating the phase drive; it does not, and the corrected reading is what `prefabs-and-assets.md` establishes independently. Where prefab-shaping systems go.
- **`PreTool`** — 1 [1], `OriginalDeletedSystem` (`SystemOrder.cs:699`).
- **`ToolUpdate`** — 15 [15]. The eleven vanilla tools (`:700-710`) plus `UpgradeDeletedSystem`, bracketed by `AllowBarrier<ToolOutputBarrier>` and `ToolOutputBarrier` (`:694/696`). The most-used phase in the corpus after `UIUpdate`. **`ToolSystem` sets `m_LastTool.Enabled = true` immediately before driving this phase and `false` when a tool stops being active** (`ToolSystem.cs:314/323/325`), which is the mechanism behind the wiki's "`ToolBaseSystem` uses `ToolUpdate`" — a tool elsewhere would still be enable-gated by the tool system but would run at the wrong moment.
- **`ClearTool`** / **`ApplyTool`** — 1 [2] / 9 [3]. Driven from `ToolOutputSystem` at the tail of `ToolUpdate`, mutually exclusive on `ToolSystem.applyMode` (`ToolOutputSystem.cs:22-30`). `ApplyTool` holds the nine `Apply*System` consumers (`SystemOrder.cs:713-721`).
- **`PostTool`** — 7 [1]. `ToolFeedbackSystem`, `SelectedUpdateSystem`, `CourseSplitSystem`, `SubElementDeleteSystem`, `MapTileSystem` (`:722-726`).
...
- **`PrefabReferences`** — 4 [1]. `PrimaryPrefabReferencesSystem`, `CheckPrefabReferencesSystem`, `SecondaryPrefabReferencesSystem`, `CheckPrefabReferencesSystem` again (`:727-730`). Reached from inside both `Deserialize` and `Serialize`.
...
- **`ModificationEnd`** — 61 [10]. `InstanceCountSystem`, `LaneDataSystem`, `ZoneCheckSystem` (`:258`), `ValidationSystem` (`:264`), `ApplyPrefabsSystem` (`:299`), notification triggers (`:306`). The last chance to touch an entity before the frame's tool and render work.
- **`PreCulling`** — 19 [3]. `CameraUpdateSystem`, `PreCullingSystem`, `OverlayInfomodeSystem`, `MeshColorSystem` (`:649`), `WindTextureSystem` (`:632-650`). Where per-instance colour work goes.
...
- **`LateUpdate`** — 5 [0]. `DebugSystem`, `SimulationSystem`, `CompleteRenderingSystem`, `GizmosSystem`, `AutoSaveSystem` (`:70-77`). **No corpus mod uses it**, which is unsurprising: a mod's `UpdateAt` here lands after all five, the whole simulation included.
- **`PreSimulation`** — **0** [0]. Driven at most once per frame from one of two mutually exclusive sites (`SimulationSystem.cs:168/272`) and occupied by nobody, vanilla or corpus. The one genuinely empty phase, and the only place to run at most once per frame immediately before the frame's simulation steps.
...
- **`EditorSimulation`** — 9 [2]. `TimeSystem`, `ClimateSystem`, `SnowSystem`, `WindSimulationSystem`, `WindSystem`, `NaturalResourceSystem`, `FireSimulationSystem`, `StreetLightSystem` (`SystemOrder.cs:603-610`) — environment only, no city. A mod wanting to work in the editor registers into both this and `GameSimulation` (`Water_Features/Water_Features/WaterFeaturesMod.cs:149-158`).
...
- **`PostSimulation`** — 1 [0], `WaterSystem` (`SystemOrder.cs:631`). Runs once per frame after the steps.
- **`CompleteRendering`** — 1 [0], `NotificationIconRenderSystem` (`SystemOrder.cs:693`). Driven after `CompleteRenderingSystem` has completed every GPU upload (`CompleteRenderingSystem.cs:41-51`).
- **`DebugGizmos`** — 32 [1]. The `*DebugSystem` family (`SystemOrder.cs:1028-1059`). Last phase of the frame; `Traffic/Code/Mod.cs:71` puts its gizmo system here behind `#if DEBUG_GIZMO`.
...
The game itself ships a dead one: `Game.Audio/WeatherAudioSystem.cs:135-138` returns 16 unconditionally and is registered only at `Modification2` (`SystemOrder.cs:115`). Where a vanilla system sits in more than one phase, the override branches — `NaturalResourceSystem.cs:334-341` and `WindSimulationSystem.cs:166-173` return 1 outside `GameSimulation`; `RandomTrafficDispatchSystem.cs:387-394` and `TrafficSpawnerAISystem.cs:645-652` return a smaller interval during `LoadSimulation`.

The mask is `(updateIndex & (interval - 1)) != offset → skip` where `updateIndex` is `SimulationSystem.frameIndex` (`UpdateSystem.cs:224`, `SimulationSystem.cs:279`). `Refresh()` auto-assigns an offset to spread same-interval systems across frames, but only where the system returned a negative one (`UpdateSystem.cs:326-351`, `:397-407`).
...
- **`OnCreate` throwing kills the whole mod.** Unity's `AddSystem_OnCreate_Internal` catches, removes the half-built system from the world, and **rethrows** (`src/Unity.Entities/Unity.Entities/World.cs:303-316`). The throw propagates out of `GetOrCreateSystemManaged` inside `UpdateAt<T>` (`UpdateSystem.cs:143`), out of `OnLoad`, and into `ModManager.InitializeMods`'s catch — which disposes the mod and logs `"Error initializing mod {0} ({1})"` (`ModManager.cs:454-458`).
- **`OnUpdate` throwing disables nothing and repeats forever.** `UpdateSystem.Update` wraps each system's `Update()` and logs `"System update error during {0}->{1}:"` with the phase name and the system's type name (`UpdateSystem.cs:188-197`, and identically in the three-argument overload at `:236-245`). The loop continues to the next system; the throwing one runs again next frame. Errors are suppressed from the UI only in editor mode (`:191-193`).
...
The `OnLoad` row is quieter still: `ModInfo.Dispose()` overwrites the error state with `Disposed` before the notification pass tests `state >= IsNotModWarning` (`src/Game/Game.Modding/ModManager.cs:171-174`, `:265` against `:271`), so it pushes no notification either.
...
**A mod's `OnLoad` throwing is a different mechanism entirely.** `ModManager.InitializeMods` catches, calls `modInfo2.Dispose()` — which calls `OnDispose()` on every instance (`ModManager.cs:161-174`) — and logs (`:454-458`). The mod's state becomes `GeneralError` — and then `Disposed`, because `Dispose()` sets it (`:171-174`), which is what keeps it below the notification pass's `state >= IsNotModWarning` gate (`:271`). So **no notification is pushed for it**, corrected 2026-08-05 against the earlier reading of `:267-337`. The logger is `LogManager.GetLogger("Modding").SetShowsErrorsInUI(false)` (`:179`), so there is no dialog either, and `Modding.log` is the whole record.

**A corpus author found this by hand.** `ExtraAssetsImporter/EAI.cs:164-168` catches, logs with the comment "Doing this, because the game isn't logging any error", then `throw ex;` with "This should still send the error to the game and so start the OnDispose." The second half is exactly right — the rethrow is what triggers `Dispose()`. The first half is not quite: `ModManager.cs:457` does log it, to the `Modding` logger with UI errors suppressed.
...
`Enabled = false` on a `ComponentSystemBase` does not unregister it. `SystemBase.Update()` skips `OnUpdate` when `!Enabled` and the system stays in `m_Updates` (`src/Unity.Entities/Unity.Entities/SystemBase.cs:38-81`). **Corrected 2026-08-26 (ticket 38): it does not "still call `OnStopRunning()` once".** The stopping branch is gated on `if (ptr->PreviouslyEnabled)` (`:64`), a flag set only by the running branch on the first update that gets through (`:50-54`), so a system disabled before it has ever run never reaches the hook. The superseded claim cited `:64-68`, a range whose own first line is the guard that overturns it — the mid-range form of [a derivation stopped at the line that agreed with it](../solutions/decompile-read-stopped-at-the-confirming-line.md), where a range reads as diligence while holding the disproof. **That is what makes the pattern work: a disabled system is still a valid anchor**, so `UpdateBefore<Fork, Vanilla>(phase)` puts the fork in the dead original's exact slot.
...
- `WaitXFrames(int)` returns a `Task` completing after N ticks (`:44-49`); `GameManager.Initialize` uses it four times to pace its own boot (`GameManager.cs:615/618/623/634`).

The tick is `MainThreadDispatcher.UpdateUpdaters()` at `GameManager.cs:723`, called from `GameManager.Update()` **outside** the `shouldUpdateManager` guard — so deferred work runs even while the world is not updating.
...
- Forcing a UI module asset to load through `GameManager.instance.modManager.AddUIModule` — which requires `ModManager.m_Initialized`, so it cannot run inside `OnLoad` at all (`CS2-WriteEverywhere/BelzontWE/WriteEverywhereCS2Mod.cs:104-109`, gate at `ModManager.cs:478-486`).
  **Amended 2026-08-03 (the new-sources resweep): the gate is `if (m_Initialized)` with no `else`** (`src/Game/Game.Modding/ModManager.cs:480-485`), so an early call is a silent no-op and not a throw — which is the half a reader diagnosing a missing UI module needs, and which this file had left to inference. The body it skips is the one that registers the `"ui-mods"` host location for the module's directory and pushes the module's `coui` path into the frontend's app bindings, so nothing partial happens either.
...
- `PreSerialize<T>` and `IPreSerialize` (`SystemOrder.cs:732-733` uses it for `ClimateSystem` and `AudioManager`).
...
`ModInfo.Dispose()` calls `OnDispose()` on every instance, clears the list and sets `State.Disposed` (`ModManager.cs:161-174`). It is called from two places: `ModManager.Dispose()` at shutdown (`:498-525`) and `InitializeMods`'s catch when `OnLoad` threw (`:456`).
...
- **`zoning-buildings-and-land-value`** needs the `ModificationEnd` slot where `ZoneCheckSystem` lives (`:258`) and the `GameSimulation` slot where `BuildingConstructionSystem` lives (`:586`); the corpus's cleanest substitution example spans exactly those two (`PlopTheGrowables/Code/Mod.cs:74-82`).
- **`environment-and-pollution`** is the one mechanics area with a live `EditorSimulation` presence (`SystemOrder.cs:603-610`), so a change there needs the dual-registration pattern rather than a single `GameSimulation` registration.
...
- **`SystemUpdatePhase.PreSimulation` has zero registrations in vanilla and zero in the corpus.** Verified by grepping `SystemOrder.cs` (31 of 32 phases appear; `PreSimulation` does not) and the corpus. It is driven at most once per frame, from one of two mutually exclusive sites — `SimulationSystem.cs:168` inside the loading branch, whose caller returns before the main-path drive at `:272` (`OnUpdate`, `:202-208`) — and zero times on a loading frame whose pump count is already spent (`:160`, `:166`, the non-new-game load edge: one frame per load). The shipped once-per-frame statements stand as the approximation. Genuinely empty, not merely unpopular.
...
Nothing in this topic's entry file moved; one clause was cut from the phase catalogue sibling.

- **`PreSimulation`'s occupancy, re-derived — unchanged from the dead-ends entry above, which carries the derivation and its cites.** The catalogue's "occupied by nobody, vanilla or mod" held on the vanilla half and carried a corpus clause on the mod half — the shape the plugin's prose contract cuts, since no reader can re-derive it — so the line now reads "with no vanilla occupant".
- **The sweep's other lifecycle leads closed as dead ends.** SceneExplorer registers into `UIUpdate`, `ToolUpdate`, `UITooltip` and `MainLoop` only (`SceneExplorer/SceneExplorer/ModEntryPoint.cs:31-35`); its `MainLoop` splice ahead of the raycast system is the two-type anchored form the reference already states, and its one main-thread dispatch is `MainThreadDispatcher.RunOnMainThread` (`SceneExplorer/SceneExplorer/Settings.cs:130-141`), already in the reference's four-shape dispatcher table.

## Re-sweep 2026-08-26: Unity's documentation (ticket 38)

Tier 1, swept in full with `phase-catalogue.md`. Unity docs fetched live 2026-08-26 at the version-pinned URLs `docs/SOURCES.md` entry 13 fixes; decompile read the same day at 1.6.0f1. No live game was used. Both of this topic's ruled `conflicts.md` entries bound the sweep and neither was reopened — the stock ordering attributes stay inert, and the ordering stays a nesting tree with `VOLATILE:` on the per-phase names and not on the tree's shape.

**One search underwrites several negatives here, so its reach is stated once.** A `grep -rl` for `ComponentSystemGroup|IRateManager|RateUtils|ScriptBehaviourUpdateOrder|ICustomBootstrap` over every `src/` assembly whose directory does not begin `Unity.` returns no file at all. The pattern names types rather than call spellings, so it reaches a bare reference as well as a use, which is what makes the empty result evidence. Consequence for this whole sweep: every Entities doc page on group ordering, rate managers, fixed-step semantics and player-loop injection describes machinery this build does not run. `World.Update()` is never called either; only `World.Dispose` is, from `GameManager.DestroyWorld` on the quit paths.

- **`Enabled = false` does not call `OnStopRunning` on the path this reference's own recipe takes — and the superseded claim's citation contained its own disproof.** Corrected in place at the finding above. [The `SystemBase` page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/manual/systems-systembase.html) and [the `OnStopRunning` API page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/api/Unity.Entities.ComponentSystemBase.OnStopRunning.html) both frame the callback as a **transition** out of a running state, which is what prompted the read; neither states the precondition. `SystemBase.Update()` enters the stopping branch only when `Enabled && ShouldRunSystem()` is false **and** `ptr->PreviouslyEnabled` is true (`src/Unity.Entities/Unity.Entities/SystemBase.cs:64`), that flag being set by the running branch on the first update that gets through (`:50-54`); the same gate runs at teardown (`src/Unity.Entities/Unity.Entities/ComponentSystemBase.cs:224-232`). The fork recipe sets the flag from `OnLoad`, which this file places at boot step 4 before the world has ticked — so the vanilla system has never run, and the hook never fires. On the mid-session-enable path it does. The docs also supply a second trigger the shipped sentence omitted: an unmet `RequireForUpdate` stops a system running and gets the same callback.
- **"The attributes it knows" over-generalised from three to all, and the counterexample is inside the plugin.** The three ordering attributes are inert and the ruling is untouched; the defect was the reach of the instruction sentence. `[RequireMatchingQueriesForUpdate]` is read off the system type at construction — `TypeManager.GetSystemAttributes` in `SystemState.InitManaged` (`src/Unity.Entities/Unity.Entities/SystemState.cs:254-261`), the unmanaged path at `:292-295` — and `ShouldRunSystem()` honours it (`:434-446`) with no `ComponentSystemGroup` anywhere in the chain, so the ruling's proof does not reach it. **Its precondition ships with it**: `ShouldRunSystem` tests the explicitly required queries first and returns out of that branch (`:422-433`), so a system that calls `RequireForUpdate` at all never consults the attribute. `[BurstCompile]` is the second counterexample and needs no decompile — a reader who took the old sentence literally would strip it. Checked in the other direction and supporting the ruling: `[CreateBefore]`/`[CreateAfter]` **are** inert here, their only consumer being `TypeManager.SortSystemTypesInCreationOrder` (`src/Unity.Entities/Unity.Entities/TypeManager.cs:3087-3118`), whose call sites are all inside the Unity assemblies (`DefaultWorldInitialization.cs:167/249`, `Unity.Entities.Hybrid/BakingUtility.cs:113`).
- **Nothing shipped said what order systems come down in, and the answer is actionable.** [The system-groups page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/manual/systems-update-order.html) states that `World.Dispose` destroys systems in reverse creation order regardless of the create-order attributes. Confirmed here: `DestroyAllSystemsAndLogException` walks `sysHandlesInCreationOrder` backwards three separate times — `OnBeforeDestroyInternal`, then `OnDestroy`, then `OnAfterDestroyInternal` (`src/Unity.Entities/Unity.Entities/World.cs:544-619`). Creation order is the literal order of the game's own calls, since the attribute sort never runs: the four hand-created systems, then all of vanilla, then each mod's as its `OnLoad` creates them. **So every mod system's `OnDestroy` runs before any vanilla system the registration pass created**, and among mods the later-loaded goes first. **Corrected in place (review gate, 2026-08-26): this read "before any vanilla system's", full stop.** A residue of vanilla systems is created lazily *after* mods load and so comes down first — the review-gate section at the end of this file enumerates them. The more useful half for the section that already existed: `TerminateGame` calls `m_ModManager?.Dispose()` at `src/Game/Game.SceneFlow/GameManager.cs:802` and `DestroyWorld()` at `:803`, so every `IMod.OnDispose` runs with the world intact. (The `MonoBehaviour` `OnDestroy` fallback at `:760-767` destroys the world without disposing mods first, but is guarded on the state not already being `Quitting`/`Terminated`/`WorldDisposed`, so on any path a player takes `TerminateGame` has already run — which is why the shipped sentence is unqualified. Recorded so the next pass does not re-derive it.) This is a bridge to what ticket 37 landed in `performance-and-memory` — that `World.Dispose` completes every published job before the first `OnDestroy` — and not a restatement: that file owns the jobs, this one owns which `OnDestroy` runs first.
- **`ResetDependency` does not do what a downstream sentence says, and the shipped sentence here is accurate as written.** [The dependencies page](https://docs.unity3d.com/Packages/com.unity.entities@1.3/manual/scheduling-jobs-dependencies.html) reads as a flat contradiction of "reset to `default`", which is why it was chased. `UpdateSystem` calls `ResetDependency()` when `m_ResetInterval <= iterationIndex`, immediately before the system's `Update()` (`src/Game/Game/UpdateSystem.cs:230-234`), and that is `base.Dependency = default(JobHandle)` (`src/Game/Game/GameSystemBase.cs:141-144`). But `BeforeOnUpdate()` then sets `NeedToGetDependencyFromSafetyManager = true` (`src/Unity.Entities/Unity.Entities/SystemState.cs:403-408`), so the first read of `base.Dependency` inside `OnUpdate` re-derives from the graph either way (`:192-203`). **The reset's only net effect is to make `BeforeOnUpdate`'s `m_JobHandle.Complete()` a no-op** — that completion being, on every other update, a main-thread wait on the handle the system published last time it ran. How much of its own previous handle a system gets back through the graph depends on its access modes: `AddDependency` stores the handle as the write fence of every type written and appends it to the read fences of every type read (`src/Unity.Entities/Unity.Entities/ComponentDependencyManager.cs:252-265`), while `GetDependency` returns the write fence for every type read plus both fences for every type written (`:198-237`). **No sentence in this file moved.** What the evidence bears on is `performance-and-memory`'s handle-discipline item 6 — "so a handle you published last iteration is no longer chained to anything" — which is unqualified where the decompile makes it conditional on access modes, and which attributes the effect to the reset rather than to the completion. That file is ticket 37's; handed across as evidence, not decided, and not a `conflicts.md` entry since the decompile settles it and what remains is a scoping question inside one file. **Superseded in place (review gate, 2026-08-26): it was decided.** Ticket 37 resolved, so the file was free, and the clause was cut rather than rephrased — see the review-gate sections at the end of this file.
- **The tree's one explicitly unverified weak point is now closed and no prose moved.** The Editor manual's `MonoBehaviour.LateUpdate` page confirms the ordering the derivation had assumed, which is the central consequence the nesting tree turns on.
- **Source-list feedback.** Entry 13 held on everything it claims; all three URL shapes resolved, `@1.3` served the right branch, and the search-don't-compose instruction was necessary twice. Two amendments went in: for Entities the `/api/` page is often a one-line stub with the mechanism on a manual page about something else — `World.Dispose`'s destruction order is on the *system-groups* page, nowhere near the API entry, which cost four fetches — and the entry's trap list named three container-and-job facts while missing the largest one for this half of the engine, **the default world bootstrap**, which the grep above settles for good.

## Review gate 2026-08-26: the handle-discipline finding was decided rather than handed across

The re-sweep bullet above closes with "That file is ticket 37's; handed across as evidence, not decided." Ticket 37 is resolved, so the file was free, and `plugins/cs2-modding/AGENTS.md` forbids the alternative: "Fix it in the pass that found it … Deferring the correction to a follow-up drops it."

Re-derived before the correction landed. The reset happens where the prose said — `UpdateSystem.cs:230-233` gates `ResetDependency()` on `m_ResetInterval <= iterationIndex`, and `GameSystemBase.cs:141-143` sets `base.Dependency = default`. The causal clause after it was false: `BeforeOnUpdate()` sets `NeedToGetDependencyFromSafetyManager = true` unconditionally (`src/Unity.Entities/Unity.Entities/SystemState.cs:403-408`), so the first read of `base.Dependency` inside `OnUpdate` re-derives from `ComponentDependencyManager.GetDependency` over the declared reader/writer lists (`SystemState.cs:192-203`) whether or not the reset ran. The reset's only net effect is that `m_JobHandle` is already `default` when `BeforeOnUpdate` calls `Complete()`, making that main-thread wait a no-op — the reverse of what the sentence implied, since it invited the defensive `Complete()` that rule 5 on the same list forbids.

The access-mode half is what the corrected prose keeps. `AddDependency` files the published handle as the **write fence** of every written type (`ComponentDependencyManager.cs:254`) and appends it to the **read fences** of every read type (`:259`); `GetDependency` returns, for a type the system reads, only the write fence (`:207`), and for a type it writes, the write fence plus every read fence (`:215-221`). So a handle comes back through what you declared read-write and not through what you declared read-only — and never at all for a container the graph does not track, since `AddDependency` indexes purely by `TypeIndex`.

**The rule's instruction stands, on a stronger footing than the reason it was given.** `base.Dependency` is never a carrier of the handle you assigned; a field of your own, combined per rule 1, is the only thing that survives an iteration.

**The teardown sentence this sweep wrote generalised past what creation order supports.** It shipped as "reverse creation order, which puts every mod's ahead of every vanilla one — a mod system's `OnDestroy` still sees the whole vanilla world, and no vanilla system's ever sees the mod's." The mechanism is right and the universal is not: `World.Dispose` walks `sysHandlesInCreationOrder` backwards in all three teardown loops (`src/Unity.Entities/Unity.Entities/World.cs:545`, `:570`, `:594`), and the handle is appended before `OnCreate` runs (`:255-257`, the append at `:300`) — so anything created *later* comes down *first*.

Not every vanilla system is created by the registration pass. That pass does precede mods (`src/Game/Game.SceneFlow/GameManager.cs:621` `CreateSystems()` then `:624` `InitializeModManager()`, with `CreateSystems` being `SystemOrder.Initialize`), but a residue of vanilla systems is reached only through a lazy `GetOrCreateSystemManaged` outside it — established by enumerating every registered type in `SystemOrder` against every `GetOrCreateSystemManaged`/`CreateSystemManaged` call site under `src/Game`, then classifying each unregistered type by its enclosing method, which puts the boot-time ones (an `OnCreate`, or a method one calls) aside. What is left is a small class rather than one system: `CameraCollisionSystem` from `CameraController.Awake` after `await GameManager.instance.WaitForReadyState()` (`src/Game/Game/CameraController.cs:306-308`, `:333`), `CinemachineRestrictToTerrain` from `Start()`, `UpkeepPropertyBinderSystem` from `PrefabUISystem.OnGameLoaded` (`PrefabUISystem.cs:1343`, `:1592`), `ReplacePrefabSystem` from `PrefabSystem.OnUpdate` (`PrefabSystem.cs:762`, `:805`), `TerrainDebugSystem` from `GameManager.OnGUI` behind `shouldUpdateWorld`, and the debug generics.

Both halves of the shipped sentence failed on those. The correction scopes it to the systems the registration pass created and names the lazily-created residue as the exception — and adds the trap the mechanism leaves behind, which no finder raised: `m_SystemLookup.Clear()` runs only after all three loops (`World.cs:622`), so an existing-only lookup during teardown still returns a system whose `OnDestroy` has already run. The lookup does not tell the reader it is gone.

**Second pass: the handle-discipline correction was over-corrected and the claim was dropped instead.** The replacement reason said what comes back from `base.Dependency` is "the graph's fence for those types and never the handle you assigned" — false for anything declared read-write, where `AddDependency` stores that very handle as the type's `WriteFence` (`ComponentDependencyManager.cs`) and `GetDependency` reads it straight back, returning it verbatim for a system declaring one read-write type and no other reader. That is the same clause wrong twice, so `plugins/cs2-modding/AGENTS.md`'s rule applied — "A claim that comes back wrong twice is a claim to delete rather than rephrase. Ask what the surrounding prose loses if the claim simply goes; where the answer is nothing, that is the fix." The rule keeps its interval mechanics and its instruction, and now states no reason at all; its title was rewritten to match what it actually forbids, since "do not carry a handle" contradicted a body telling the reader to hold one in a field.

**Third pass: deleting the reason outright was itself wrong, and a reason went back in.** This is the one deletion shape `plugins/cs2-modding/AGENTS.md` says does not survive its own review — "a cut of a trap's consequence clause, which reads as narration to a deletion pass and is the half a reader acts on". Stripped to its mechanism and its instruction, the rule read as though `base.Dependency` comes back *empty* on a reset iteration, which contradicts the same file's own account of the graph and would send a reader to hand-carry every vanilla type's dependency or to insert the defensive `Complete()` rule 5 forbids. The half that makes the instruction necessary is neither the severing (there is none) nor the derivation (that holds either way): it is that the reset **pre-empts the completion stock `SystemBase.Update` performs**, so last iteration's handle is not completed for you before the next run. `docs/research/performance-and-memory.md` had that paragraph established all along, one file away from the rule it explains.
