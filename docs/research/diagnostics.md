...
4. **Did a system fail to construct?** A throw from any system's `OnCreate` propagates out of `UpdateAt<T>` and takes the **whole mod** down — `GeneralError` is set and then overwritten by `Disposed`, per the state table above — so this presents exactly as step 3, and the stack trace is what separates them.
