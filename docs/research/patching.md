...
**Baseline.** Decompiled game 1.6.2f1, read 2026-08-04.
...
Installed game read at 1.6.0f1, its cites carried onto 1.6.2f1 by the game-update sweep — `Cities2_Data/Managed/` under `%CSII_INSTALLATIONPATH%`, the toolchain cache at `%CSII_TOOLPATH%`, and, under `%CSII_USERDATAPATH%`, both the log directory (`Logs/`) and the Paradox mods cache (`.cache/Mods/pdx_mods/`).
...
`GameManager.ListHarmonyPatches()` exists and is thorough: it finds the first loaded assembly whose name contains "Harmony", resolves `HarmonyLib.Harmony`, calls the static `GetAllPatchedMethods()` and `GetPatchInfo(...)`, and logs every patched method followed by its prefixes, postfixes, transpilers and finalizers by declaring type and name (`src/Game/Game.SceneFlow/GameManager.cs:2170-2221`, `PrintPatchDetails` at `:2225-2242`, `PrintIndividualPatches` at `:2244-2263`).
It logs to the `Modding` logger, which is the same one `ModManager` writes to (`src/Game/Game.Modding/ModManager.cs:179`).

**It runs before any mod is loaded, so it always finds nothing.**
The call sits at `GameManager.cs:590`, inside `Initialize()`. `m_ModManager` is not even constructed until `:611`, and `InitializeModManager()` — which is what eventually reaches `IMod.OnLoad` and therefore `PatchAll` — is at `:624`.
...
`%CSII_USERDATAPATH%/Logs/Modding.log` opens on `Modding runtime: Builtin`, which is `ListHarmonyPatches`' own first `InfoFormat` (`GameManager.cs:2173`), and then contains no `Harmony found.` line and zero `Patched Method:` lines across a session that loaded nine mods.
...
Rots: the position of `ListHarmonyPatches()` relative to `InitializeModManager()` — re-check `src/Game/Game.SceneFlow/GameManager.cs:590/624`.
...
`isLoaded` bites only on a mid-session re-initialization (`ModManager.cs:245`, `GameManager.cs:1643`), where a copy already in the process outranks a local one and a higher-versioned one alike; `mod-compatibility.md` carries the full derivation at its first finding.
...
Traffic appends `ModifiedLaneConnections` to `LaneSystem.m_OwnerQuery` (`src/Game/Game.Net/LaneSystem.cs:9197`, built at `:9220` and required at `:9240`).
...
`InitializeRaycast` on three different tools — `BulldozeToolSystem` (`src/Game/Game.Tools/BulldozeToolSystem.cs:1457`), `NetToolSystem` (`NetToolSystem.cs:5890`), `DefaultToolSystem` (`DefaultToolSystem.cs:776`) — patched by `BetterBulldozer/BetterBulldozer/Patches/BulldozeToolSystemInitializeRaycastPatch.cs:20`, `Anarchy/Anarchy/Patches/NetToolSystem_InitializeRaycast.cs:31`, `ExtraDetailingTools/MOD/Patches/DefaultToolSystem.cs:10`, and both bulldoze and default by `CS2-Platter/Platter/Patches/MarkerPatches.cs:45-46/105-106`.
Both `GetRaycastResult` overload families on `ToolBaseSystem` (`ToolBaseSystem.cs:542/555`) and on `BulldozeToolSystem` (`BulldozeToolSystem.cs:1653`), by `BetterBulldozer/.../ToolBaseSystemGetRaycastResultPatch.cs:20` and `MarkerPatches.cs:69-71/84-86/128-130/147-149`.
The snap surface: `ToolBaseSystem.GetActualSnap(Snap, Snap, Snap)` (`ToolBaseSystem.cs:467`) at `CS2-Platter/Platter/Patches/ToolSystemPatch.cs:31-33`; the private static `GetAvailableSnapMask` overloads on `AreaToolSystem` (`AreaToolSystem.cs:3268`) and `ObjectToolSystem` (`ObjectToolSystem.cs:3627`) at `ExtraDetailingTools/MOD/Patches/AreaToolSystem.cs:10-12` — the sibling `Patches/ObjectToolSystem.cs` names the same target but its whole `[HarmonyPatch]` is commented out (`:27-33`), so nothing patches it; `ObjectToolSystem.GetAllowRotation` (`ObjectToolSystem.cs:2934`) and the private `SnapControlPoint(JobHandle)` (`:4252`, called from seven places across `Cancel`, `Apply` and `Update`, none of them `OnUpdate`, the first at `:3783`) at `ToolSystemPatch.cs:55-101`.
...
`ToolUISystem.GetElevationRange` (`src/Game/Game.UI.InGame/ToolUISystem.cs:511`, bound at `:148`), `AllowBrush` (`:639`, bound at `:180`), `SetBrushStrength` (`:662`, bound at `:194`); `ToolbarUISystem.Apply` (`ToolbarUISystem.cs:856`) and `BindAssets` (`:410`); `TimeUISystem.GetDay` and `GetTicks` (`TimeUISystem.cs:163/157`, bound at `:86/85`); `CityInfoUISystem.WriteDemandFactors` (`src/Game/Game.UI.InGame/CityInfoUISystem.cs:271`, called six times, `:238/244/250` among them); `ActionsSection.OnProcess` and `OnDelete` (`src/Game/Game.UI.InGame/ActionsSection.cs:299/244`, the latter bound at `:153`).
...
`ClimateSystem.SampleClimate(ClimatePrefab, float)` (`src/Game/Game.Simulation/ClimateSystem.cs:605`, against a one-argument overload at `:623`), the public static `CityServiceUpkeepSystem.CalculateUpkeep` (`src/Game/Game.Simulation/CityServiceUpkeepSystem.cs:593`), and `CityServiceBudgetSystem.OnUpdate` (`src/Game/Game.Simulation/CityServiceBudgetSystem.cs:854`) — patched at `Time2WorkPatches.cs:171/193-194/226`.
...
**Every one of these targets was checked against the decompile and exists at 1.6.2f1**, with the signature the patch claims, including every one whose `Type[]` array turns out to be load-bearing.
...
**No corpus repository uses any of the three.** Combined with mod load order being dictionary iteration order (`ModManager.cs:181/441`), the practical position is that cross-mod patch order on a shared target is unspecified, and every corpus mod that cares handles it by making its patch order-independent instead.
...
`AreaToolSystem.GetAvailableSnapMask` has two — the public override `(out Snap, out Snap)` at `src/Game/Game.Tools/AreaToolSystem.cs:3256` and the private static four-parameter one at `:3268` carrying the logic. `ExtraDetailingTools/MOD/Patches/AreaToolSystem.cs:10-12` targets the second.
`ObjectToolSystem.GetAvailableSnapMask` likewise, the seven-parameter private static at `ObjectToolSystem.cs:3627` against the override at `:3611`; the `ExtraDetailingTools/MOD/Patches/ObjectToolSystem.cs` class that named it is commented out (`:27-33`), so no live patch takes this target.
...
`ClimatePrefab.SimplexNoise(float, float)` carries `[BurstCompile]`, and its entire body is `return SimplexNoise_000094ED_0024BurstDirectCall.Invoke(x, y);` (`src/Game/Game.Prefabs.Climate/ClimatePrefab.cs:1105-1109`).
...
**Application.** Every corpus mod patches from `IMod.OnLoad`, and `OnLoad` is the last thing `ModInfo.Load` does before marking the mod loaded (`src/Game/Game.Modding/ModManager.cs:118-125`, dispatching to each `IMod` at `:153-159`).
Two things happen before it and both matter.
`FormatterServices.GetUninitializedObject(item)` constructs the `IMod` **without running any constructor** (`:122`), so field initialisers on the mod class do not run and a `Harmony` instance cannot be created there.
...
A mod whose Harmony reference cannot be resolved never reaches `OnLoad` at all: `canBeLoaded` is false, and the mod's state becomes `MissedDependenciesError` with the unresolved names listed (`ModManager.cs:110-117`, `ExecutableAsset.cs:175`).
...
It runs at process shutdown, from `ModManager.Dispose()` called by `GameManager` on destroy (`GameManager.cs:802`, `ModManager.cs:498-525`), where the AppDomain is going away regardless.
And it runs per-mod when that mod's own load throws: `InitializeMods` catches, calls `modInfo.Dispose()`, and logs (`ModManager.cs:453-458`) — which is the case where unpatching genuinely buys something, because a mod that patched and then failed halfway through `OnLoad` would otherwise leave live patches behind a mod that is not there.
It does **not** run when a code mod is disabled mid-session: that path calls `RequireRestart()` and leaves the mod loaded (`GameManager.cs:1582-1588`), and re-initialisation skips any mod already in a non-`Unknown` state (`ModManager.cs:96-99`), so a mod is never unloaded and re-patched inside one run.
...
Rots: the `OnDispose` trigger set — re-check `src/Game/Game.SceneFlow/GameManager.cs:802/1582-1588` and `src/Game/Game.Modding/ModManager.cs:96-99/453-458`.
