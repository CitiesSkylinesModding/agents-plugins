...
- **A citation on every claim.** `src/<assembly>/<namespace>/<Type>.cs:<line>` for decompiled source, `<checkout>/<path>:<line>` for anything else inside a decompile checkout, `<repo>/<path>:<line>` for a mod, an install-relative path for a file in the installed game, `<package>/<path>:<line>` for a file in an installed toolchain package — the official UI mod scaffold keeps this form as `create-csii-ui-mod/<path>:<line>`, an alias for the npm-global junction whose target the citing file's baseline expands once, since the files behind it are the game's own and version with the install rather than with npm — a path from this repository's root for a file in it, `<file>:<line>` for a sibling file here, the full URL for a wiki page.
  ...
  A line cite into a living sibling — `SOURCES.md` above all — rots the moment that file gains a line above it: amend on the existing line where possible, and otherwise the sweep of every cite below the insertion is part of the amendment ([an amendment that shifted every cite below it](../solutions/an-amendment-that-shifted-every-cite-below-it.md)).
