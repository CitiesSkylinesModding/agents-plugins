...
- `method-<slug>.md` — how a source was obtained, where obtaining it was itself an investigation.
  ...
  One exists: [`method-decoding-shipped-locale-data.md`](method-decoding-shipped-locale-data.md), which gets the vanilla strings out of the game's compiled `.loc` assets; its working scripts sit beside it in [`loc-parse/`](loc-parse/). Read it before decoding one yourself — it carries its traps, including the content packs' own locale packages, silently missed by reading the base game's package alone.
...
- **A citation on every claim.** `src/<assembly>/<namespace>/<Type>.cs:<line>` for decompiled source, `<checkout>/<path>:<line>` for anything else inside a decompile checkout, `<repo>/<path>:<line>` for a mod, an install-relative path for a file in the installed game, `%CSII_TOOLPATH%/<path>:<line>` for a file the toolchain installed, `<package>/<path>:<line>` for a package in the Unity mod project's package cache, `create-csii-ui-mod/<path>:<line>` for the official UI mod scaffold — an alias for the npm-global junction whose target the citing file's baseline expands once, since the files behind it are the game's own and version with the install rather than with npm — a path from this repository's root for a file in it, `<file>:<line>` for a sibling file here, the full URL for a wiki page.
  ...
  Once a path is cited in full, later mentions may shorten to `<Type>.cs:<line>`, and to a bare `:<line>` only where the full path is the nearest one cited, and a bare cite that follows prose naming another type names its own file instead.
  A cite inserted into existing prose re-anchors every bare cite after it, so the bare cites that follow it name their own file.
  A `<name>.md:<line>` cite names the sibling in the citing file's own directory, and several basenames exist both here and under `plugins/cs2-modding/skills/` — check which tree you opened before calling such a cite right or wrong.
  A line cite into the decompile is read at its HEAD, so where that HEAD is newer than the commit the last cite re-point names (the `Decompile:` line of that commit's message), bring the cites onto it before citing anything new: the `cs2-game-update` skill for a new game version, its cite step alone for a re-decompile.
  ...
  A line cite into a living file — a sibling, `SOURCES.md` above all, or a shipped reference — rots the moment that file gains a line above it: amend on the existing line where possible, and otherwise the sweep of every cite below the insertion is part of the amendment ([an amendment that shifted every cite below it](../solutions/an-amendment-that-shifted-every-cite-below-it.md)).
...
Nothing downstream catches either: the content lint checks the shapes a script can see, which `scripts/check-skill-content.ts` enumerates, and none of those is coverage.
