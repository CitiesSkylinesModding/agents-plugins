...
`Error` is another zero-size tag (`src/Game/Game.Tools/Error.cs:7`), added by `ValidationSystem` (`src/Game/Game.Tools/ValidationSystem.cs:468`, registered at `ModificationEnd`, `SystemOrder.cs:264`), and the only thing `ToolBaseSystem` queries for it (`m_ErrorQuery`, `ToolBaseSystem.cs:313`).
...
`Apply(JobHandle)` (`:1618-1645`) checks `GetAllowApply()`, plays a sound, sets `ApplyMode.Apply`, clears its control points and calls `DestroyDefinitions` — so a commit destroys the definition entities in the same frame the `Apply*System` family consumes them.
