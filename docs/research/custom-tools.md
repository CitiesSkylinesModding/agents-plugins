...
The frontend claims added in the 2026-08-03 re-sweep read the user's own installed copy at 1.6.0f1 — the same build the decompile was taken from — and cite it through a copy of `Cities2_Data/Content/Game/UI/index.js` reformatted with prettier at its defaults and read at `DecompiledCitiesSkylines2/src-ui/source.js`, **135,021 lines**; check a fresh reformat against that count before trusting a line number from it.
The original pass predates `docs/SOURCES.md` and ran against the decompile, the wiki and the corpus alone, so a claim below with no install citation is one nothing first-party outside `src/` has been checked against.
...
**Verdict: the list order is construction order, not registration order, and `SystemOrder.cs:699-709` does not settle it.** `ToolBaseSystem.OnCreate` appends at `:315` but resolves `ToolSystem` and `DefaultToolSystem` first at `:294-297`, and `ToolSystem.OnCreate` itself creates `DefaultToolSystem` at `ToolSystem.cs:192` — so the default tool is appended before any other whichever tool is constructed first. Construction also begins well before `:699`, since `UpdateSystem.UpdateAt` calls `GetOrCreateSystemManaged` immediately (`UpdateSystem.cs:143`) and `SystemOrder.cs:666` (`NetColorSystem`) and `:680` (`GuideLinesSystem`) resolve seven of the eleven tools between them. An earlier pass of this file read the registration span as the list order and produced an alphabetical table that matches the live game at no index.

**Settled 2026-08-03 against the running game (source 8), no mods loaded, `ToolSystem.tools` read by index.** `n=11`, and the order is `DefaultToolSystem`, `SelectionToolSystem`, `ObjectToolSystem`, `AreaToolSystem`, `UpgradeToolSystem`, `BulldozeToolSystem`, `NetToolSystem`, `RouteToolSystem`, `ZoneToolSystem`, `WaterToolSystem`, `TerrainToolSystem`. A mod tool registered from `IMod.OnLoad` lands after all eleven and never sees a prefab first. Because the order falls out of construction, another mod constructing a tool early shifts it, so the reference tells a reader to read the list back rather than trust the table.
...
| `SetUnderground(bool)`, `ElevationUp()`, `ElevationDown()`, `ElevationScroll()` | `public virtual void` (`:451/455/459/463`) | Empty hooks. Superseded on the elevation half by the 2026-08-25 re-sweep below: in game only the net tool's arrows render, and `ElevationScroll` has no caller |
...
**Verdict (2026-08-03 re-sweep): confirmed against the install, where the original pass had inferred it.** The path is built C#-side in `ToolUISystem.BindToolModes` rather than in the frontend, and `Cities2_Data/Content/Game/UI/Media/Tools/` holds exactly eight directories — `Area Tool`, `Bulldoze Tool`, `Net Tool`, `Object Tool`, `Snap Options`, `Terrain Tool`, `Vegetation Options`, `Zone Tool` — one per vanilla tool id that publishes modes, and none for any mod. `index.html` loads from the `gameui` host, so a bare `Media/...` src resolves under `assetdb://gameui/` (the page's base; `coui://gameui/` answers 404 — the 2026-08-23 verdict in `prefabs-and-assets.md`), which is the game's own UI directory and not one a mod writes into.
...
Verdict: this file's own earlier claim that `ValidationSystem` adds the tag is wrong, re-derived from the decompile under the placement-definitions pass and corrected here.
...
Verdict: this file's own earlier claim that a commit destroys the definitions "in the same frame the `Apply*System` family consumes them" is wrong, re-derived under the placement-definitions pass and corrected here.
...
`public const Snap kSnapAllIgnoredMask = Snap.AutoParent | Snap.PrefabType | Snap.ContourLines` (`ToolBaseSystem.cs:98`) — the three flags an "all snapping" UI toggle is meant to leave alone.

**Verdict: the earlier reading that "grepping `src/Game/` returns the declaration and no consumer, so it exists for the frontend and for mods" is wrong on its first clause, and the install overturns it.** `ToolUISystem`'s `tool.allSnapMask` binding returns `(uint)(onMask & offMask) & 0xFFF8FFFFu` (`src/Game/Game.UI.InGame/ToolUISystem.cs:135-143`), and `0xFFF8FFFF` is exactly `~(0x10000 | 0x20000 | 0x40000)` — the constant, inlined by the compiler because it is a `const`. So the grep came back empty for a decompilation reason and not a semantic one, which is the general trap `SOURCES.md` states for the frontend and which holds inside the decompile too.
The frontend consumes the binding as the "All" toggle's target set: the tool-options snapping panel reads `availableSnapMask`, `allSnapMask` and `selectedSnapMask`, and its "All" button computes `(selected & all) === all` and then ORs or clears `allSnapMask` wholesale (`DecompiledCitiesSkylines2/src-ui/source.js:80588-80605`, binding accessors at `:13091` and `:46089`). It splits `ContourLines` out of the icon row into a second titled group of its own (`:80606-80608/80656-80685`), which is why that flag is in the ignored mask at all: it is not part of "all snapping", it is a separate control. The icons are `Media/Tools/Snap Options/<Snap member name>.svg`, and the member names come over as the `tool.snapOptionNames` binding built from `Enum.GetName` over `Snap` (`ToolUISystem.cs:145/264-275`).
...
Corrected 2026-08-25, the performance-and-memory scheduler pass: that prefix's `Complete()` is not the shape to copy — vanilla's own `SnapControlPoint` schedules against `JobUtils.CombineDependencies(inputDeps, …)`, registers four readers and returns the handle uncompleted (`src/Game/Game.Tools/ObjectToolSystem.cs:4196-4271`), and all seven call sites chain it (`inputDeps = SnapControlPoint(inputDeps)`), which is also `patching`'s shipped rule for a prefix that schedules; the custom-tools recipe now says combine, register, assign the scheduled handle, never complete.
...
**Verdict (2026-08-03 re-sweep, settled against the running game): all of them.** `m_BuiltIn` defaults to `true` on `CompositeInstance` (`:20`) and the single site in the game that clears it is `src/Game/Game.Modding/ModSetting.cs:181`, inside a mod's own key-binding registration — so a composite is built-in unless a mod made it. Reading `InputManager.instance.FindAction(map, action).isBuiltIn` live at 1.6.0f1 returns `True` for every one of seventeen actions sampled across all eleven vanilla maps and `False` for the one mod map in that session; `settings-and-input.md`'s dead-end entry carries the full sample and the map list. So the built-in check is not a filter a vanilla action might slip past, which is what "cannot be read here" left open.
...
- **`frontend-and-injection`, in the UI skill**, is the tool options panel and it is a two-sided bridge. The C# side is `ToolUISystem`'s `"tool"` binding group (`src/Game/Game.UI.InGame/ToolUISystem.cs:44/125-187`), and the finding above matters to it: `availableSnapMask`, `allSnapMask`, `selectedSnapMask`, `setSelectedSnapMask`, `color`, `brushSize`, `brushStrength`, `brushAngle`, `undergroundModeSupported`, `undergroundMode` and `activeTool` all go through `ToolBaseSystem` virtuals and work for a mod tool, while `selectTool` and `selectToolMode` are hard-coded to the vanilla tools and do not. The TS side is `moduleRegistry.extend` on two paths, and the 2026-08-03 re-sweep re-derived both first-party from the shipped bundle, where the original pass had only the corpus: `game-ui/game/components/tool-options/mouse-tool-options/mouse-tool-options.tsx` → `MouseToolOptions` (`DecompiledCitiesSkylines2/src-ui/source.js:80891/80893`; corpus corroboration at `Anarchy/Anarchy/UI/src/index.tsx:22/27/41/43`, `BetterBulldozer/BetterBulldozer/UI/src/index.tsx:15`, `AreaBucket/UI/area-bucket/src/constants.ts:6`, `CS2-Platter/Platter/UI/src/components/vanilla/Components.tsx:10`) and `game-ui/game/components/tool-options/tool-options-panel.tsx` → `useToolOptionsVisible`, which is what makes the panel appear at all for a tool the game does not know (`source.js:82169/82182`; corpus at `Anarchy/.../index.tsx:34`, `BetterBulldozer/.../index.tsx:17`, `AreaBucket/UI/area-bucket/src/constants.ts:5`, `CS2-Platter/Platter/UI/src/index.tsx:24`).
  **Verdict: the corpus was right about both paths and the export names.** Recorded because a path the corpus agrees on is still four copies of one author's guess until the game's own registry is opened, and both were unverified when the tickets for those two references were written.
...
(This file first named that second topic `terrain-and-water`, which is not a reference in the approved structure; `environment-and-pollution` claims landscaping, surfaces and terrain in its boundary. Corrected under the placement-definitions pass.)

## Dead ends

- **`kSnapAllIgnoredMask` has no consumer in `src/Game/` — and that dead end was wrong.** Struck by the 2026-08-03 re-sweep; see the verdict in the snapping finding. The grep was accurate and the conclusion drawn from it was not: `ToolUISystem.cs:135-143` consumes the constant as the literal `0xFFF8FFFFu`, because the compiler inlines a `const`. A name-based grep of the decompile is evidence about names, not about consumers, and this is the one place in this file where that gap changed a claim.
...
- **The underground triad now ships as consequence, not ownership.** `tool.undergroundModeSupported` is `activeTool.allowUnderground` and `tool.undergroundMode` is `activeTool.requireUnderground` (`src/Game/Game.UI.InGame/ToolUISystem.cs:155-156`); the UI's toggle forwards to `SetUnderground(bool)` (`ToolUISystem.cs:513-519`). The tool-options underground entry renders only while `undergroundModeSupported` is true and the tool is not the default tool (`src-ui/source.js:80765-80784`); the top toolbar's underground toggle always renders and greys out without it (`source.js:129530`, `:129566`), forwarding to the same `setUndergroundMode` trigger (`:129537`). The elevation arrows are NOT the same control for a mod tool: both frontend elevation sections bail on `elevationRange` (`min >= max` → null, `source.js:80702-80710`, `:81796-81797`) and `GetElevationRange` hard-wires `default(Bounds1)` for any tool but the net tool (`ToolUISystem.cs:455-466`), so they never render in game for a non-net tool; the `elevationUpDisabled`/`elevationDownDisabled` bindings (`:401-412`, `:422-437`) are created and re-exported to mods but read by no vanilla component (`source.js:46100-46101`, `:13113`, `:13118`). The editor's main screen fires `ElevationUp`/`ElevationDown` for any active tool through an unconditional action map (`source.js:132826`, `:46120-46122`); `ElevationScroll` has no caller in the bundle (`source.js:46129`, declared and exported at `:13115`, read nowhere else). The underground view itself switches on `requireUnderground`, not on the collision mask: `UndergroundViewSystem.undergroundOn = activeTool.requireUnderground` (`src/Game/Game.Rendering/UndergroundViewSystem.cs:115-116`). So `CollisionMask.Underground` alone casts against objects the player cannot see, and a tool that never overrides `allowUnderground` (a virtual defaulting to false, `src/Game/Game.Tools/ToolBaseSystem.cs:184`) has a dead `SetUnderground` hook and, in game, no player-facing path into its underground branch — the map editor's screen registers a Change Elevation input over the elevation hooks with no gate of its own (`source.js:132826`, `:46120-46122`), which SceneExplorer wires to underground itself (`InspectObjectToolSystem.cs:140-153`).
  A mod does this: SceneExplorer wires `allowUnderground => true`, all three elevation hooks and `requireUnderground` to one flag (`SceneExplorer/SceneExplorer/System/InspectObjectToolSystem.cs:61`, `:135-153`, `:264-279`).
  Unconfirmed: whether that editor input is bound and enabled while a non-net tool is active — the action lives in the game's Shortcuts input map (the corpus mimics `("Shortcuts", "Change Elevation")`, the mimic finding above) but its name appears in no decompiled C#, so the install's input asset, or an editor session with such a tool, settles it.
  Rots: the `tool.undergroundModeSupported` and `tool.undergroundMode` binding names — `Game.UI.InGame.ToolUISystem`; the `elevationUpDisabled`/`elevationDownDisabled`/`elevationScroll` binding names and every `source.js` line cite in this finding — the reformatted bundle copy, re-checked against its 135,021-line count.
- **The editor toolbar is a separate surface, now derived in full.** `Game.UI.Editor.EditorToolUISystem.tools` is an `IEditorTool[]` behind a property whose setter also rebuilds `m_Disabled`, a parallel `bool[]` the update walks index-for-index (`src/Game/Game.UI.Editor/EditorToolUISystem.cs:35-46`, `:152-165`), so growing the array and assigning it back through the property is the whole registration. The four vanilla tools are constructed inline in `OnCreate` (`:80-86`) and no registration API exists: every reference to the type in `src/Game/` is the generated system registry, one `SystemOrder` registration into `UIUpdate` (`src/Game/Game.Common/SystemOrder.cs:996`), the class itself, or `EditorHierarchyUISystem`'s selection calls. Consumers are the `editorTool` binding group — `tools`, `activeTool`, `selectTool` (`EditorToolUISystem.cs:87-89`) — with `SelectTool` taking `FirstOrDefault` on `id` (`:147-150`), so a duplicate id makes the later tool unreachable. `EditorTool` adds settable `panel` and `tool`; `IsActive()` compares `activePanel == panel` and then, only when `tool` is non-null, `activeTool == tool` (`src/Game/Game.UI.Editor/EditorTool.cs:56-67`), so an entry with neither set reports active whenever no panel is open. Timing: `GameManager` runs `CreateWorld` at `:591`, `CreateSystems` at `:615` and `InitializeModManager` at `:618` (`src/Game/Game.SceneFlow/GameManager.cs`), so the vanilla four exist before any mod code runs, and the world lives until shutdown, so an `OnCreate` write survives every load while a per-load hook needs a duplicate guard. The tooltip locale key is `Editor.TOOL[<id>]` (`src/Game/Game.UI.Editor/BulldozeToolPanel.cs:14`). Mods do this: SceneExplorer (`SceneExplorer/SceneExplorer/SceneExplorerUISystem.cs:66-75`, inside `#if DEBUG_PP`, which its Release configuration also defines — `SceneExplorer/SceneExplorer/SceneExplorer.csproj:23` — with the locale entry at `SceneExplorer/SceneExplorer/Localization.cs:65`), Water Features from a UI system's `OnCreate` (`Water_Features/Water_Features/Tools/WaterToolUISystem.cs:257-267`), Road Builder (`RoadBuilder-CSII/RoadBuilder/Systems/RoadBuilderToolSystem.cs:88-99`) and Write Everywhere (`CS2-WriteEverywhere/BelzontWE/Systems/WEMainUISystem.cs:27-36`).
  This shipped in the catalog's four entries only. **Maintainer finding: no shipped reference owns editor-tool registration** — `custom-tools` owns `ToolSystem.tools` and the frontend skill owns the UI side; the mechanism above has no home, and this section is the derivation waiting for one.
  Rots: `EditorToolUISystem.tools`, `IEditorTool`'s member set, the four vanilla tool ids, and the `editorTool` binding names — `Game.UI.Editor`.
  Unconfirmed: whether an unsynchronised write from a background thread corrupts the binding read — derived from the absence of any lock, and a `MainThreadDispatcher.RunOnMainThread` wrapper settles it; and the visible toolbar symptom of the null-panel activity test — needs the editor open with such a mod loaded.
- **The sweep's per-mode raycast lead closed as a dead end.** SceneExplorer's `InitializeRaycast` is the reference's "call it first and then set only what you want" form — the base call, then per-mode collision and type masks (`SceneExplorer/SceneExplorer/System/InspectObjectToolSystem.cs:176-198`) — and nothing in the shipped raycast treatment moved.
- **Source-list check: nothing to add.** Ticket 36's premise that `docs/SOURCES.md` entry 11 states a corpus size was stale — the entry states none and points at the catalog — and the sweep reached no artifact the list does not name.
