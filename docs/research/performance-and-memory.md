...
The **running game** is a fourth, driven over the Mono soft debugger on 2026-08-04 and again on 2026-08-05; everything it settled is confined to the "Settled against the running game" section below, which states its own dates and cites the expressions evaluated rather than file lines.
...
Two limits on the callstack. Its frames are native — `(UnityPlayer)`, `(mono-2.0-bdwgc)` and address-only entries — with the only managed entries being `runtime-invoke` wrappers, so it locates a region of the engine rather than a line of mod code. And this allocation was made through the debugger, whose invoke path filled the managed half of the stack; whether a mod's own JIT'd frames appear in their place is unestablished, and leaking one from ordinary mod code would settle it.
...
- **The job-body free cannot be settled from the debugger, which is why it still carries a marker.** Breaking inside a job and evaluating `GameObject.Destroy` or `GCHandle.Free` from that frame looks like the experiment, and is not: the SDB tooling marshals method and property calls to the game's main thread even when the paused frame belongs to a worker, so it can never reproduce a worker thread reaching a Unity API. Settling it needs a mod that actually makes the call from a job body, built and run — once bursted and once not. Recorded so the next pass does not spend a live session rediscovering that the obvious route is closed, as a frame-time comparison against a frame-rate cap was before it.
