...
**Baseline.** Decompiled game 1.6.2f1, read under `C:\Users\Morgan\Documents\Projets\DecompiledCitiesSkylines2\src\`.
Installed game read at the same version: the shipped locale package `Cities2_Data/Content/Game/Locale.cok`, and the UI bundle `Cities2_Data/Content/Game/UI/index.js` cited through a copy reformatted with prettier at its defaults at `DecompiledCitiesSkylines2/src-ui/source.js`, **140,104 lines** — check your copy's count before trusting a line number from it.
...
`ModInfo.Load` returns early with `state = State.IsNotUniqueWarning` on `asset.isMod && !asset.isUnique` (`src/Game/Game.Modding/ModManager.cs:105-109`), before `LoadAssembly`, before `IMod` instantiation, before `OnLoad`.
...
At first initialization the order therefore reduces to **local beats non-local, then highest version wins, then asset id**; `isLoaded` only bites on a mid-session re-initialization (`ModManager.cs:245`, `GameManager.cs:1643`), where a copy already in the process wins over a higher-versioned one.
...
**One quirk of that filter is worth recording exactly as written.** The `where` clause reads `(d.isLoaded || d.canBeLoaded) && (isMod || isReference)` — the second half has no `d.` and so tests the _calling_ asset rather than each candidate (`ExecutableAsset.cs:188`).
...
And a mod whose references cannot all be resolved never loads: `canBeLoaded` is `m_References.Values.All(r => r != null)` (`:175`), and a false there is `State.MissedDependenciesError` with the unresolved reference names joined into `loadError` (`ModManager.cs:110-117`).
...
**One — the enabled-mod name list.** `ModManager.ListModsEnabled()` returns the `asset.fullName` of every **loaded** mod concatenated with the `name` of every `UIModuleAsset` in the global database (`ModManager.cs:222-228`).
...
**Two — enumerating the mod manager.** `ModManager` is `IEnumerable<ModManager.ModInfo>` over `m_ModsInfos.Values` (`:29`, `:193-201`), and `ModInfo` exposes `asset`, `isLoaded`, `isBursted`, `name`, `assemblyFullName`, `state` and `instances` (`:48-74`).
...
`ModManager.Initialize` calls `RegisterMods()` and then `InitializeMods()` (`:264-265`), and `RegisterMods` populates `m_ModsInfos` for every executable asset before any `IMod.OnLoad` runs (`:400-432`, the loop at `:409`).
So enumerating the manager from `OnLoad` sees every mod that will load, in any order.
`ListModsEnabled()` filters on `x.isLoaded` (`:224-225`), which is only true for mods already initialized — and initialization order is `Dictionary<Identifier, ModInfo>` iteration order (`:181`, iterated at `:441`).
...
`MainThreadDispatcher.RegisterUpdater(Action)` wraps the action in a `Func<bool>` returning true, so it runs exactly once (`src/Colossal.Core/Colossal.Core/MainThreadDispatcher.cs:97-104`), on the next `GameManager.Update()` (`:85-95`, called at `src/Game/Game.SceneFlow/GameManager.cs:723`) — which is after `InitializeModManager` has returned (`:674-680`) and therefore after every mod's `OnLoad`.
...
A prefab a code mod builds at runtime through `PrefabBase.Create<T>(name)` has `asset == null` (`src/Game/Game.Prefabs/PrefabBase.cs:415-420`), so its hash is `default` and its identity is nothing but the type name and the name.
...
**Verdict: at equal priority the winner is a binary-search artifact, and the shipped reference first said "whichever registered first", which is wrong.** Established during the mod-compatibility pass's review. `AddHostLocation` does `int num = value.BinarySearch(item, kPathPriorityComparer); if (num < 0) { num = ~num; } value.Insert(num, item);`, and `kPathPriorityComparer` (`:155`) compares priority alone. `ModManager.InitializeUIModules` (`ModManager.cs:471`, `:482`) passes `isLocal` as the third positional argument, which is `shouldWatch`, so every `ui-mods` path sits at priority 0 and the whole list compares equal. `BinarySearch` on an all-equal run returns its first probed midpoint, so registering A, B, C, D yields `[C, D, B, A]` — the third registrant is served, and with five it is `[C, E, D, B, A]`. Neither "first wins" nor "last wins" survives past the third registration.
**And the priority integer is a live lever elsewhere:** `GameManager.InitializeUI` passes `asset.priority` for every `UIHostAsset` (`GameManager.cs:1769`, `:1773`), and the shipped `.uiHost` files carry non-default values — the base `gameui` host at `-1000` and each content pack at `-10`. The comparer is ascending and the handler walks front-to-back, so a **lower** number is asked first and wins; any prose implying "raise the priority to win" is inverted. Nothing registers under `ui-mods` at a non-default priority.
Every mod's UI module directory is registered under the **same** host, `"ui-mods"` (`src/Game/Game.Modding/ModManager.cs:471`), so `coui://ui-mods/<file>` is a shared search path across every installed UI mod.
...
The single-argument `RemoveHostLocation(uri)` drops a whole host and every path under it (`UISystem.cs:306-317`); the game itself always uses the two-argument form when unregistering a UI module (`ModManager.cs:492`).
...
The corpus prefixes: `Traffic` uses `"Traffic_Compatibility_Detector"` (`Traffic/Code/Mod.cs:194`); the game's own are `"ModLoadingStatus"`, `"RestartRequired"` and one per failing mod's asset guid (`ModManager.cs:241`, `:536`, `:289`).
...
**Verdict: only the `name` collides, and the shipped file first said the file name did too.** Corrected during the mod-compatibility pass's review, against `settings-and-input.md:230` and the save path: `GroupSettingsByFile` keys a `Dictionary<string, List<SettingAsset>>` by `item.meta.path`, and `ProcessSingleSettingsFile` reads the existing file back and re-saves every setting written to it, each as its own `name` block. So several settings classes sharing one `[FileLocation]` coexist as separate named blocks and untouched blocks survive; two mods choosing the same `name` genuinely share a store. `settings-and-input` owns the mechanism; the collision is this topic's.
...
The registry object itself is `DecompiledCitiesSkylines2/src-ui/source.js:13389-13493`.
`override(path, export, value)` saves the pre-existing value into a backup map **only the first time** and then replaces (`:13400-13404`, the backup at `:13403`).
`extend(path, name, cb)` is implemented as `this.override(path, name, cb(current))` (`:13405-13424`, the delegation at `:13422`), so it reads whatever is there — which may already be another mod's wrapper — and installs a wrapper around it.
**Extends therefore chain and overrides clobber**, and the chain's order is registrar order.
`append` is built on `extend` against a modding-hook module, so appends compose too (`:13425-13472`).
`add(path, module)` throws `Module ${e} was already registered...` when the path exists (`:13395-13399`).
An SCSS `extend` called without a callback **merges** class strings rather than replacing them (`:13412-13418`), which is the composable form for styles.
`reset()` clears the append-target set and restores every backed-up vanilla export (`:13487-13491`), so a mod calling it would strip every other mod's UI changes.
...
`hasAppend(target)` exists so a component can ask whether anybody appended: the game's own bundle uses it for `"UniversalModMenu"` (`source.js:129814`, the implementation at `:13492`).

**Registrar order is import-completion order, which is nondeterministic.** The loader `import()`s every active UI module's `.mjs` in parallel and pushes each default export into an array as it resolves, then runs them (`source.js:140002-140024`, the push at `:140016`, the completion counter at `:140009`).
`AR` is the runner: `Q.reset(); PR.clear(); for (const t of e) t(Q);` (`:47298-47301`).
...
What ships is the middle: both member names, verified against the local Entities assembly, each with the one cost that decides which you use — the reinterpret route makes you mirror a layout you do not own, and the boxed route costs a boxed copy per entity on the main thread.
**Corrected, then reversed on the mechanism (review gate, 2026-08-26). Read the reversal at the end of this bullet before acting on it — the pin IS on the component path.** The correction as first written read: the `GCHandle.Alloc(…, Pinned)` this section attributed the throw to is not on this build's component path — `GetComponentBoxedUnchecked` returns `TypeManager.ConstructComponentFromBuffer` for `TypeCategory.ComponentData` with no pinning at all (`src/Unity.Entities/Unity.Entities/DebuggerDataAccess.cs:169-181`); the only pin is on the buffer branch (`:203`). The clause was cut from the shipped reference rather than rephrased, since the paragraph's contrast survives on the boxing cost alone. The buffer half stays open in `conflicts.md`.

**Reversal (same gate, found by writing the code rather than reading it).** That correction resolved the wrong overload. `DebuggerDataAccess.cs:180` passes `(TypeIndex, byte*)`, which can only bind to `TypeManager.ConstructComponentFromBuffer(TypeIndex, void*)` at `TypeManager.cs:1662` — a `GCHandle.Alloc(…, Pinned)` plus `MemCpy` — and cannot bind to the `(void*, int)` overload at `:2487` that fronts the `AssemblyTypeRegistry` switch, since `TypeIndex` converts only to `int` and `byte*` not to `int` at all. That second overload has **zero call sites in this build**. So the generated switch both passes cited is dead code, and a grep for the identifier is what led each of them to it.

**Corrected in place (live read, 2026-08-26): the pin refuses nothing, and the marker is gone.** This paragraph read: the shipped sentence carries the failure under `UNVERIFIED:`, because the remaining step — whether the runtime refuses to pin a `bool`- or `char`-carrying box — is an `InternalCall` (`GCHandle.cs:114`) behind the same wall as the buffer half.
Two reads against the maintainer's running 1.6.0f1 settled both halves. `EntityManager.Debug.GetComponentBoxed` on `Game.City.PlayerMoney`, which carries `public bool m_Unlimited` (`src/Game/Game.City/PlayerMoney.cs:13`), returned the same field values as `GetComponentData<PlayerMoney>` on that entity; the same call on a 51-element `Game.Net.LabelPosition` buffer, whose element carries `public bool m_IsUnderground` (`src/Game/Game.Net/LabelPosition.cs:18`), returned all 51 elements matching a direct buffer read. So neither `AddrOfPinnedObject()` nor `UnsafeUtility.PinGCArrayAndGetDataAddress` rejects a `bool`.
The `char` case cannot be exercised against vanilla — no `IComponentData` or `IBufferElementData` under `src/Game/` carries a `char` field — and rests on the same two calls the `bool` case cleared. The shipped sentence keeps the pin as mechanism and the boxed copy as its cost, with no marker. `conflicts.md`'s entry dissolved on this.
...
One more clause had to come off after that. The boxed route's throw shipped as "on a component carrying a `bool` or a `char` — which most of them do", and the prevalence half was invented rather than counted: parsing every `IComponentData` under `src/Game/` gives 862 components, 56 carrying one of those types directly and 43 transitively, so under a tenth. The clause was the only thing steering the route choice, and it steered readers to the layout-asserting route for nineteen components in twenty. The frequency was never established and is gone. **Superseded in place (review gate, 2026-08-26): this line closed "The throw is real and stays", which is neither established nor refuted** — the pin is on the component path after all (see the reversal above), and the live read above then showed it refuses a `bool`-carrying box on neither path. The shipped sentence carries the pin as mechanism and the boxed copy as its cost, with no marker; the throw is refuted rather than unverified.
...
- The other mod is never told, because `IMod` carries no hook that could tell it (`src/Game/Game.Modding/IMod.cs`, and `OnDispose` fires only at process shutdown — `src/Game/Game.SceneFlow/GameManager.cs:802`).
...
`GameManager` subscribes `OnEntryIsInActivePlaysetChanged` on the Paradox mods data sources (`src/Game/Game.SceneFlow/GameManager.cs:1517`, `:1523`) and dispatches per asset kind (`:1529-1651`). For an `ExecutableAsset` that is an IL assembly:

- **Enabled mid-session** (`!isLoaded && isInActivePlayset`): the asset is collected (`:1589-1592`) and, if any were, `ModManager.Initialize(m_UpdateSystem, reinitialize: true)` runs (`:1641-1644`). That re-runs `RegisterMods` and `InitializeMods`, so the new mod's `OnLoad` fires **there and then**, mid-session, with no restart. Every already-loaded mod is a no-op because `ModInfo.Load` returns immediately on `state != State.Unknown` (`src/Game/Game.Modding/ModManager.cs:96-99`).
- **Disabled mid-session** (`isLoaded && !isInActivePlayset`): `ModManager.RequireRestart()` (`GameManager.cs:1585-1588`). The mod is **not** unloaded, its systems keep running, and `OnDispose` is not called. `RequireRestart` sets `restartRequired`, logs `Restart required`, and pushes a `"RestartRequired"` notification whose click opens a confirmation dialog offering to quit the game (`ModManager.cs:527-548`).
- **UI modules** are handled live in both directions — added through `ModManager.AddUIModule` and removed through `RemoveUIModule` (`GameManager.cs:1574-1581`, `:1645-1648`, the implementations at `ModManager.cs:478-496`) — which is what changes `activeUIModsLocation` on the frontend and re-runs every registrar from a reset registry.
- **Prefab assets** are added to or removed from `PrefabSystem` in batches of a fixed size with a four-frame yield between batches (`GameManager.cs:1545-1559`, `:1594-1616`).

**`IMod.OnDispose` fires only at process shutdown**, from `ModManager.Dispose()` at `GameManager.cs:802`, and from `ModManager.InitializeMods`'s catch on a mod whose `Load` threw (`ModManager.cs:454-458`). There is no "my mod is being disabled" hook.

What a mod can do about it: read `GameManager.instance.modManager.restartRequired` (`:191`) and re-run its own detection — the corpus's `static bool?` caches never invalidate.
`ModManager.AreCodeModsLoaded()` and the static `AreModsEnabled()`/`GetModsEnabled()` are the other public reads (`:207-233`).
...
`ParadoxModsDataSource` declares two public events (`src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/ParadoxModsDataSource.cs:30`, `:32`), and `GameManager` subscribes to both (`GameManager.cs:1517-1518`, `:1523-1524`).
`onAfterActivePlaysetOrModStatusChanged` is a plain `Action` invoked at `ParadoxModsDataSource.cs:186` and `:318`, in each case **after** `await InvokeOnEntryIsInActivePlaysetChanged(...)` has returned — and that awaited call is the one `GameManager` handles by running `ModManager.Initialize(reinitialize: true)`.
...
Clicking it opens a `MessageDialog` titled `Common.DIALOG_TITLE_MODDING[ModLoadingWarning|ModLoadingError]` with body `Common.DIALOG_MESSAGE_MODDING[{state}]` and a `MODNAME` argument; when `loadError` is set the dialog also carries a copyable details pane, with backslashes and asterisks escaped (`:293-316`).
A non-local mod gets two extra actions, `[ModPage]` and `[Disable]`, whose callbacks call `PdxSdkPlatform.ShowModDetail` and `EnableModInActivePlayset(..., false)` (`:301-305`, `:317-336`).
So the player can disable the offending mod from the dialog.

**Decoding the base game's package `en-US.loc` confirms every one of these keys exists** — 22,163 entries, decoded with the recipe in `method-decoding-shipped-locale-data.md` from `Cities2_Data/Content/Game/Locale.cok`.
...
**The frontend contributes nothing.** The reformatted bundle's only occurrence of any of these is the generated `Loc` dictionary entry `DIALOG_MESSAGE_MODDING: new yc("MODNAME")` (`DecompiledCitiesSkylines2/src-ui/source.js:26870`); `ModLoadingWarning`, `EnabledModsChanged` and `ModsLoading` do not appear at all. The whole surfacing path is C#.
...
`<Dependency Id="<pdx mod id>" DisplayName="..." Version="..." />` in the mod project's `Properties/PublishConfiguration.xml` becomes a `PDX.SDK.Contracts.Service.Mods.Models.ModDependency` with `Type = DependencyType.Mod` at upload (`src/Colossal.PSI.PdxSdk/Colossal.PSI.PdxSdk/PdxSdkPlatform.cs:2047-2055`, composed into the staging metadata at `:1828`).
...
- **`diagnostics`** owns the log. Take from here the strings a compatibility problem produces, all on the `Modding` logger (`ModManager.cs:179`, `ExecutableAsset.cs:117`): `Assembly "{0}" is in-game assembly and it should NOT be shipped with mod "{1}"`, `Error loading assembly {0}`, `Error loading assembly reference {0}`, `Error initializing mod {0} ({1})`, `Restart required`, plus `Duplicate prefab ID: {0}` and `Dependency not available in {0}: {1}` on the base log. `Modding.log` also carries the `======= Active Playset =======` and `======= Enabled Mods =======` block, written by an unawaited asynchronous call so it can land anywhere among the loading lines (`ModManager.cs:260`, `:367-398`), which is the fastest way to see what the player actually had on.
...
- **The frontend as a source on mod conflicts.** The reformatted bundle was searched for `ModLoadingWarning`, `DIALOG_MESSAGE_MODDING`, `EnabledModsChanged`, `ModsLoading` (one hit, the generated `Loc` entry at `source.js:26870`) and for `Dependenc` (12 hits, none of them a mod dependency: Cohtml's own `addSynchronizationDependency`/`removeSynchronizationDependency` at `:234` and `:244`, and the options screen's `ModdingToolchainDependency` widget at `:26003-26004`, `:26320-26323`, `:74037-74057` and `:74363`). The mod-loading dialogs, the restart prompt and the whole dependency story are C#-side; the store browser that would show a dependency is Paradox-hosted and not in this bundle. The absence is a finding — a mod cannot extend or intercept the conflict UI, because there is none to extend.
...
- **`ModInfo.instances` as a route to another mod's `IMod` object.** It is `public IReadOnlyList<IMod>` (`ModManager.cs:48`), so a mod can reach another mod's live instance and reflect over it. No corpus repository does: every one of the eleven detectors goes through the _type_ (`asset.assembly.GetTypesDerivedFrom<IMod>()` in Find It's case, `ModManager.cs:120`) and reaches a static, never an instance. The reason is visible in the loader — mods are constructed with `FormatterServices.GetUninitializedObject` (`:122`), so no constructor ran and no instance field is guaranteed initialized beyond what `OnLoad` set.
- **`Colossal.PSI.PdxSdk` as a runtime dependency source.** `PdxSdkPlatform` carries `GetAllDependencies`, `GetDependencyCandidates` and a `ModDependency` model (`PDX.SDK/PDX.SDK.Contracts.Service.Mods/IModsInformationService.cs:15-19`), all of it store-side and reached only from the upload path (`PdxSdkPlatform.cs:1828`, `:2047-2063`). Nothing in `src/Game/` queries a mod's declared dependencies at load. The only runtime read of the active mod set is `ParadoxModsDataSource.ContainsActiveMod`, through `ModRequirement`.
- **The corpus's use of `moduleRegistry.override`.** Swept all 22 repositories' `.ts` and `.tsx` for each of the eight registry operations. `override`, `add` and `reset` return zero on any receiver, aliased or not, and that absence is a real finding: nobody has hit the `add`-throws-on-second-run hazard that `AR`'s `Q.reset()` loop implies.

  **Corrected 2026-08-06 (the mod-compatibility pass's review):** the same sweep's zero for `find` and `get` was an artifact of pinning the receiver to the literal `moduleRegistry.`, and this is [a search taken for a census](../solutions/empty-grep-read-as-proof-of-absence.md) exactly. `source.js:47254-47255` aliases them — `const CR = Q.find, NR = Q.get;` — and exports the pair to mods as `findModule`/`getModule` on `cs2/modding` (`:13383`, handed over at `:47266`), so the registry's read side is in fact the operation the corpus uses most, across roughly 147 files. Mods also read the backing map directly through aliased receivers a receiver-pinned pattern cannot see (`registry.registry.get(...)`, `this.registryData.registry.get(...)`, `for (const [key, module] of moduleRegistry.registry)`). Re-sweep by operation name across every binding, not by receiver.
...
## Spot-check 2026-08-26: Unity's documentation (ticket 38)

This file was in neither of ticket 38's lists and got a bounded skim rather than a sweep, looking for a sentence asserting engine *runtime behaviour* rather than naming an engine type in passing. The expected answer was none. It found one, and it does not hold.

- **`GetComponentBoxed`'s throw is attributed to a mechanism this build's component path does not take — appended to `conflicts.md`, not decided.** The shipped sentence at `mod-compatibility/mod-compatibility.md:179` says the call "throws outright on a component carrying a `bool` or a `char`", and this file's `:231` records the reason as a `GCHandle.Alloc(…, Pinned)` on the boxed struct. That pinning is not on the component path: `EntityManager.Debug.GetComponentBoxed` (`src/Unity.Entities/Unity.Entities/EntityManager.cs:667-675`) reaches `DebuggerDataAccess.GetComponentBoxedUnchecked`, which for `TypeCategory.ComponentData` returns `TypeManager.ConstructComponentFromBuffer` (`src/Unity.Entities/Unity.Entities/DebuggerDataAccess.cs:169-181`) — a generated `switch` of plain `*(T*)buffer` dereferences (`src/Game/Unity.Entities.CodeGeneratedRegistry/AssemblyTypeRegistry.cs:10237ff` at 1.6.0f1, `:10291ff` at 1.6.2f1) with no pinning anywhere. The only pin in the method is on the **buffer** branch, `UnsafeUtility.PinGCArrayAndGetDataAddress` (`DebuggerDataAccess.cs:205`), and that is a native `InternalCall` (`src/UnityEngine.CoreModule/Unity.Collections.LowLevel.Unsafe/UnsafeUtility.cs:68-75`) rather than `GCHandle.Alloc`. A search for `typeof(bool)` across `src/Unity.Entities/` returns nothing, and across the Entities package source returns no rejection either.
  **The component half is settled and wrong; the buffer half is not settleable statically**, since whether Unity's native array pin rejects a non-blittable element type sits behind that `InternalCall`. **Superseded (review gate, 2026-08-26): the component half is *not* settled, and the reason given here for calling it settled was itself wrong** — the pin is on that path, reached through the only overload the call site can bind to. Both halves became one entry in `conflicts.md`, and a live read the same day dissolved it: neither the component pin nor the buffer pin rejects a `bool`, so the shipped sentence carries the pin as mechanism with no marker (the correction at the `GetComponentBoxed` verdict above holds the reads). It was not corrected at the time because the clause cannot be deleted alone: the paragraph's conclusion — that the API's layout-free promise fails in the cases that matter — rests on it, so deleting the clause leaves a consequence with no cause. **Both of those closing claims were themselves overturned:** the shipped paragraph had already lost its "fails in the cases that matter" tail, so nothing rested on the clause; and there is no "remaining half" to settle singly, since the component half rejoined the buffer half once the pin turned out to be on both paths. The live read then cleared both.
- **One claim in the same file that the docs would have got wrong and the prose got right.** `:178` says a `GetDynamicComponentDataArrayReinterpret<T>` layout mismatch "misreads rather than throws". Unity's documentation describes a method that validates; in the package the guards are `CheckComponentSizeMatches` and `CheckCannotBeAliasedDueToSizeConstraints`, both `[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]` (`src/Unity.Entities/Unity.Entities/ArchetypeChunk.cs:708-728`), and the emitted body at `:730-743` calls neither. The prose is right *because* it was written build-aware, which is the outcome entry 13's precedence rule exists to produce.
