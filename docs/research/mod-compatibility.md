...
The shipped sentence now names the pin as the mechanism and carries the failure under `UNVERIFIED:`, because the remaining step — whether the runtime refuses to pin a `bool`- or `char`-carrying box — is an `InternalCall` (`GCHandle.cs:141`) behind the same wall as the buffer half. Both halves are now one open entry in `conflicts.md`, settleable by a single live read.
