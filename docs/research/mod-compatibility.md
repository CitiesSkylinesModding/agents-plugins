...
**Reversal (same gate, found by writing the code rather than reading it).** That correction resolved the wrong overload. `DebuggerDataAccess.cs:180` passes `(TypeIndex, byte*)`, which can only bind to `TypeManager.ConstructComponentFromBuffer(TypeIndex, void*)` at `TypeManager.cs:1662` — a `GCHandle.Alloc(…, Pinned)` plus `MemCpy` — and cannot bind to the `(void*, int)` overload at `:2488` that fronts the `AssemblyTypeRegistry` switch, since `TypeIndex` converts only to `int` and `byte*` not to `int` at all. That second overload has **zero call sites in this build**. So the generated switch both passes cited is dead code, and a grep for the identifier is what led each of them to it.

**Corrected in place (live read, 2026-08-26): the pin refuses nothing, and the marker is gone.** This paragraph read: the shipped sentence carries the failure under `UNVERIFIED:`, because the remaining step — whether the runtime refuses to pin a `bool`- or `char`-carrying box — is an `InternalCall` (`GCHandle.cs:141`) behind the same wall as the buffer half.
