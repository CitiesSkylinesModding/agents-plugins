...
**`ecs-in-this-game`** — the mangled `TypeHandle` field names are how a reader answers "who reads and who writes this component" without a call graph, and the eight `RO`/`RW` shapes with their populations are the vocabulary. The two limits belong there too: shared-component handles carry no access segment, and a command-buffer write carries no type name at all (`ObjectToolBaseSystem.cs:1270`). The `IJobChunk`/`IJobEntity` census (771 / 0) is that topic's and was re-verified here.
...
**`performance-and-memory`** — the Burst mangled names (`Game_002ERendering_002EDequeueAndSort_00004B5A_0024BurstDirectCall`, `AssemblyInfo.cs:14-18`) are a dead end for a reader chasing a Burst-compiled method, and the decode (`_002E` → `.`, `_0024` → `$`) is what gets them back to `Game.Rendering.DequeueAndSort`.
