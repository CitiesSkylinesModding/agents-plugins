...
**Baseline.** Decompiled game 1.6.2f1; mod corpus read 2026-08-03 at the commits the 20-repository checkout carried; wiki (`Localize your mod`) fetched live 2026-08-03, so no snapshot substitution was needed.
A fourth source is the user's own installed copy at `1.6.2f1 (767.21d1) [6300.26419]` — the decompile's own version — and it holds two things the decompile cannot: the strings themselves as compiled `.loc` assets, and the whole frontend as a plain 2.2 MB bundle at `Cities2_Data/Content/Game/UI/index.js`.
...
The bundle ships minified to one line, so the frontend claims here cite a copy of it reformatted with prettier at its defaults and read at `DecompiledCitiesSkylines2/src-ui/source.js`, **140,104 lines** — the count to check a fresh reformat against before trusting a line number below.
...
### The dictionary-source contract is two methods, and the manager is the only runtime consumer
...
`LocalizationManager` is the single runtime consumer. It is created once, before the ECS world exists, with `en-US` as the hard-coded fallback locale (`src/Game/Game.SceneFlow/GameManager.cs:2371-2376`):
...
and is reachable from a mod as `GameManager.instance.localizationManager` (`GameManager.cs:300`).
...
**`AddLocale(LocaleAsset)` / `AddLocale(string, SystemLanguage, string)`** registers a _locale_, not a source (`LocalizationManager.cs:272-290`). It adds an empty `LocaleInfo` and the localized display name, and raises `onSupportedLocalesChanged`. It is what a new locale's `AddSource` needs to have run **first** — it replays nothing, so calling it afterwards does not rescue a source already added: `AddSource` records the pair in `m_UserSources` before consulting the locale table (`:323-327`) and `AddSourceInternal` returns silently when the locale is absent (`:340`), so the recorded pair then blocks the retry, and only `LoadAvailableLocales` (`:141-144`) ever replays them. The guard is `List<(string, IDictionarySource)>.Contains` (`:323`), so it compares with `EqualityComparer<IDictionarySource>.Default` — reference identity only for a source type that does not override `Equals`. `LocaleAsset : AssetData, IDictionarySource` (`src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/LocaleAsset.cs:9`) overrides `object.Equals(object)` as `return Equals(obj as IAssetData);` (`AssetData.cs:158-161`), forwarding to `Equals(IAssetData other)` whose comparison is `return id.Equals(other.id);` (`:173`, after the reference and null short-circuits at `:165-172`) — so for that route a fresh instance is refused too, and the same dispatch governs `m_Sources.Contains` (`LocalizationManager.cs:340`) and `m_Sources.Remove` (`:367`).
...
**A `.loc` asset in the database registers itself.** The manager subscribes to `AssetDatabase.global.onAssetDatabaseChanged` for `LocaleAsset` in its constructor (`:56`), and `UpdateSource` removes and re-adds the asset as a source on any change, skipping transient assets (`:162-182`). A bulk change reloads every locale from scratch (`PerformBulkOperation(ReloadAvailableLocales)`, `:166`, `:123-130`, `:147-151`). **That reload restores locales only from the asset database** — `Clear()` empties `m_LocaleInfos` (`:153-159`) and `LoadAvailableLocales` re-registers from `GetAssets<LocaleAsset>()` alone (`:134-138`) — so a locale that exists only because a mod called `AddLocale(string, …)` with no `.loc` asset is wiped and never restored, while `PerformBulkOperation` fires `NotifySupportedLocalesChanged()` regardless (`:129`). The enumeration is over `AssetDatabase.global`, the aggregate across every registered database including `user`, so a `.loc` a mod wrote at runtime **does** come back — the discriminator is having any `.loc` in the database, not being a shipped one. Nothing records or replays an `AddLocale`, so the string-overload case needs the mod to call it again: a retry that only tests `SupportsLocale` (`:251-258`) leaves the locale gone for the session, and one that skips the test records another dead pair each time.
...
`m_UserSources` is written by `AddSource` (`LocalizationManager.cs:323-326`) and by nothing else. `RemoveSource` removes the source from the _locale's_ list and rebuilds the affected dictionary, but **never touches `m_UserSources`** (`:357-382`, the removal at `:367`).
**Which dictionary it rebuilds is a two-branch choice, and the fallback branch leaves the player's view untouched.** `if (localeId == activeDictionary.localeID)` clears and reloads the active one; `else if (localeId == m_FallbackDictionary.localeID)` clears and reloads the fallback one (`:370-380`). Removing a fallback-locale source while the player is on another language therefore takes the second branch, and the entries that source contributed remain in the active dictionary, where `AddMissingEntriesFromFallback` (`:352`) put them — that fill no-ops on the fallback itself (`:423`), so the rebuild cannot undo it. The removal is not merely un-redoable; on that path it is not effective.
...
The practical rule for a mod that wants to swap a source out and back in: construct a source the guard's `Equals` comparison tells apart from the recorded one — a **new** instance suffices only where the source type does not override `Equals`, which `LocaleAsset` does (above) — or accept that the removal is permanent until the next bulk asset change undoes it.
...
The shipped strings confirm the spec first-party rather than leaving it on the vendored bundle's authority, and show where the game itself leans on it — once in the base game's package, and with a literal number as the name in a content pack's, detailed in the compiled-locale-data finding below.
...
| `GetOptionLabelLocaleID(opt)` | `Options.OPTION[<id>.<name>.<opt>]` | `:308-311` vs `AutomaticSettings.cs:340-372` |
| `GetOptionDescLocaleID(opt)` | `Options.OPTION_DESCRIPTION[<id>.<name>.<opt>]` | `ModSetting.cs:313-316` vs `AutomaticSettings.cs:340-372` |
| `GetOptionWarningLocaleID(opt)` | `Options.WARNING[<id>.<name>.<opt>]` | `ModSetting.cs:318-321` vs `AutomaticSettings.cs:1159-1165` |
| `GetOptionTabLocaleID(tab)` | `Options.TAB[<id>.<tab>]` | `ModSetting.cs:323-326` |
| `GetOptionGroupLocaleID(group)` | `Options.GROUP[<id>.<group>]` | `ModSetting.cs:328-331` vs `OptionsUISystem.cs:1042-1051` |
| `GetEnumValueLocaleID<T>(value)` | `Options.<id>.<ENUMTYPENAME>[<Member>]` | `ModSetting.cs:333-336` vs `AutomaticSettings.cs:845-854`, `:905-911` |
| `GetOptionFormatLocaleID(opt)` | `Options.FORMAT[<id>.<name>.<opt>]` | `ModSetting.cs:338-341` vs `source.js:74145` |
| `GetBindingKeyLocaleID(action[,c])` | `Options.OPTION[<id>/<action>/<component>]` | `ModSetting.cs:343-361` |
...
`GetEnumValueLocaleID<T>` uppercases with the culture-sensitive `typeof(T).Name.ToUpper()` (`:335`) while the engine that reads the key uses `enumType.Name.ToUpperInvariant()` (`AutomaticSettings.cs:907`). The game forces `CultureInfo.CurrentCulture = CultureInfo.InvariantCulture` at boot (`src/Game/Game.SceneFlow/GameManager.cs:542`), so the two agree in practice; the mismatch is real in the source and inert in the process.

`GetOptionFormatLocaleID` has **no C# consumer at 1.6.2f1** — a grep of `src/Game/` for `Options.FORMAT` returns only its own definition — and **zero corpus uses**. Its consumer is the frontend: the slider widget builds `` `Options.FORMAT[${e.path}]` `` with the default `"{SIGN}{VALUE}"` and substitutes `VALUE` and `SIGN`, and only when the slider's unit is `custom` (`DecompiledCitiesSkylines2/src-ui/source.js:74131-74151`, the custom branch also carrying the widget's `fractionDigits`, `separateThousands` and `maxValueWithFraction` props). The widget's `path` is `<pageId>.<declaringType>.<property>` (`AutomaticSettings.cs:326-338`), which is exactly what the helper builds, so the pair is correct.
A slider gets `unit = "custom"` from `[SettingsUICustomFormat]` (`AutomaticSettings.cs:1304-1310` for `int`, `:1341-1349` for `float`). One corpus mod carries that attribute and ships no `Options.FORMAT` string for it, so it renders the default (`LineTool-CS2/Code/ModSettings.cs:34-36`).

**Three attributes can override the generated key, and all three drop the page prefix.**
`[SettingsUIDisplayName(overrideId, overrideValue)]` produces `Options.OPTION[<overrideId verbatim>]` and `[SettingsUIDescription]` produces `Options.OPTION_DESCRIPTION[<overrideId verbatim>]`, each through `LocalizedString.IdWithFallback(key, overrideValue)`; with only a value and no id they produce `LocalizedString.Value(...)` and skip the dictionary entirely; with neither they fall through to the path-derived key (`AutomaticSettings.cs:340-372`).
`[SettingsUIConfirmation(overrideConfirmMessageId, overrideConfirmMessageValue)]` behaves the same way for `Options.WARNING[...]` (`:1145-1166`).
...
The game's own worked examples of substitution: `src/Game/Game.Modding/ModManager.cs:338-348` builds a mod-loading notification with `LOADED` and `TOTAL` as `LocalizedNumber<int>(n, "integer")`, and `src/Game/Game.Settings/GraphicsSettings.cs:633-654` builds a resolution label from `WIDTH`, `HEIGHT` and a `LocalizedNumber<double>` for `REFRESH_RATE` whose unit switches between `"screenFrequency"` and `"integer"`.
...
The frontend's own `Unit` enum has **38** members: the 33 plus `PercentagePrecise`, `BodiesPerMonth`, `TemperaturePrecise`, `Height` and `DurationSeconds` (`DecompiledCitiesSkylines2/src-ui/source.js:29024-29063`, registered as `game-ui/common/localization/unit.ts` at `:29064-29071`). Those five have no C# constant, so reaching them from C# means writing the literal.
...
Rots: the unit list on both sides — re-read `src/Game/Game.UI/Unit.cs`, and the `Unit` enum in a fresh reformat of `Cities2_Data/Content/Game/UI/index.js` (`src-ui/source.js:29024-29063` in this one).

**The frontend's number formatter is a lookup table with a garbage fallback.**
The dispatch is a lookup on the unit with a fallback lambda that renders the number followed by the unit name in angle brackets, so an unrecognised unit string prints as `1234 <myUnit>` (`DecompiledCitiesSkylines2/src-ui/source.js:29178-29180`). That is the visible symptom of a typo'd unit.
The table itself is where the player's preferences bite (`:29181-29314`). A sample, with the locale key each branch renders through:

- `Integer` → plain, thousands-separated. `IntegerRounded` switches to `Common.VALUE_THOUSAND` above 1,000 and `Common.VALUE_MILLION` above 1,000,000 (`:29190-29196`).
- `Length` → metric: `Common.VALUE_METER` below 1,000, `Common.VALUE_KILOMETER` above; freedom: `Common.VALUE_YARD` below 1,609, `Common.VALUE_MILE` above (`:29209-29216`).
- `Area`, `Volume`, `Weight`, `WeightPerCell`, `WeightPerMonth`, `Height`, `NetElevation`, `MoneyPerDistance`, `MoneyPerDistancePerMonth` all branch on `unitSettings.unitSystem` the same way (`:29217-29289`).
- `Temperature` branches on `unitSettings.temperatureUnit` across `Common.VALUE_CELSIUS`, `VALUE_FAHRENHEIT`, `VALUE_KELVIN` (`:29291-29300`).
- `Power` divides by 10 and renders kilowatts below 10,000, by 10,000 for megawatts above — so the raw C# value is in units of 100 W (`:29265-29268`).
- `Money` uses `Common.VALUE_MONEY` with no unit-system branch at all (`:29275`).

The separators come from the dictionary too: `Common.THOUSANDS_SEPARATOR` and `Common.DECIMAL_SEPARATOR` are rendered per call and applied by regex (`:29318-29351`). The sign prefix is `-` for a negative value always, and `+` for a positive one or `±` for zero **only when `signed` is set** — otherwise both are empty (`:29377-29379`).

**Verdict: `PercentagePrecise` and `TemperaturePrecise` have formatter entries.**
`PercentagePrecise` renders `Common.VALUE_PERCENT` at two fraction digits against `PercentageSingleFraction`'s one (`:29205-29208`), and `TemperaturePrecise` branches on `unitSettings.temperatureUnit` exactly as `Temperature` does, also at two — against `Temperature`'s none, since it renders through the integer formatter's `toFixed(0)` rather than the fraction one (`:29301-29310`, `Temperature` at `:29291-29300`, the two formatters at `:29319` and `:29335`).
Both are live in the map editor's climate curve readouts — the weather-classification keyframe formatter and the temperature keyframe formatter (`:57130-57131`, `:57159-57161`).

**One unit of the 38 has no number formatter: `BodiesPerMonth`.**
The number table covers 37 (`:29181-29314`); `BodiesPerMonth` appears only in the fraction table (`:29638-29639`), so `LocalizedNumber` with it hits the angle-bracket fallback while `LocalizedFraction` with it renders correctly.

**`LocalizedFraction` and `LocalizedBounds` support far fewer units than `LocalizedNumber`, and fail loudly-but-ugly outside them.**
Fractions handle exactly eleven: `Volume`, `VolumePerMonth`, `Weight`, `WeightPerMonth`, `Power`, `Energy`, `BodiesPerMonth`, `XP`, `Integer`, `IntegerPerMonth` and `IntegerRounded`; anything else renders `` `${value} / ${total} <${unit}>` `` (`:29586-29650`, the fallback at `:29590`).
The vendored bundle had ten, missing `IntegerRounded` (`:29644-29649`).
Bounds handle exactly three: `Power`, `PercentageSingleFraction`, `Temperature`; anything else renders `` `${min}–${max} <${unit}>` `` (`:29538-29565`, the fallback at `:29543`). Bounds also short-circuits to `LocalizedNumber` when `min === max` (`:29540-29545`).
...
`LocalizedPercentage(value, max)` computes `100 * value / max` and renders it as a `Percentage`-unit number — but **clamps any positive result to a minimum of 1** (`source.js:30156-30163`), so 0.2% displays as 1%. The clamp sits behind a `value > 0 && max > 0` guard whose else-branch renders a plain `0`, so a zero or negative max never divides (`:30160`).
`LocalizedDate({year, month})` renders `Common.MEDIUM_DATE_FORMAT` with `MONTH` resolved through the indexed key `Common.MONTH_SHORT:<month>` (`source.js:29840-29855`, `:29890-29892`). The month is **zero-based**: the game's only C# producer of a `SimulationDateTime` passes `currentDateTime.DayOfYear - 1` as the month (`src/Game/Game.UI.Menu/MenuUISystem.cs:869`), and a game year is twelve days (`ClimateSystem` asserts `daysPerYear == 12`, `src/Game/Game.Simulation/ClimateSystem.cs:319`), so a day _is_ a month.
`LocalizedDuration({value, daysPerYear, maxMonths})` takes a value in days and picks `Common.VALUE_YEARS`/`VALUE_YEAR` at or above `maxMonths` (defaulting to `daysPerYear`), `Common.VALUE_MONTHS` above one, `Common.VALUE_MONTH` above 23.5/24 of a day, and otherwise falls through to `Common.TIME_FORMAT` with hours and minutes derived from the fraction (`source.js:29992-30024`). The first two thresholds compare the **rounded** value, the third the raw one (`:29995-30010`).
...
The game has `LocalizedTime`, `LocalizedDateTime` and `LocalizedTimestamp`, plus `useTimeFormat`, `useDateFormat`, `formatInteger` and `formatFloat` — all registered in the module registry under `game-ui/common/localization/localized-date.tsx` and `.../localized-number.tsx` (`source.js:29941-29990`, `:29381-29424`). `LocalizedTime` already branches on `unitSettings.timeFormat` and renders `Common.TIME_FORMAT` or `Common.TIME_FORMAT_12` with `Common.TIME_PERIOD_AM`/`PM` (`:29856-29877`, the AM/PM helper at `:29936-29940`). `LocalizedTimestamp` goes further and interprets a `yyyy/MM/dd/HH/hh/mm/aa` pattern out of `Common.TIMESTAMP_FORMAT` (`:29893-29935`).
What `cs2/l10n` exports at runtime is **eleven names and no more**: `Localized`, `LocalizedBounds`, `LocalizedDate`, `LocalizedDuration`, `LocalizedEntityName`, `LocalizedFraction`, `LocalizedNumber`, `LocalizedPercentage`, `LocalizedString`, `Unit`, `useLocalization` (`source.js:12413-12427`, attached to `window` as `cs2/l10n` at `:47256-47269` alongside `cs2/api`, `cs2/bindings`, `cs2/ui`, `cs2/utils`, `cs2/input`, `cs2/modding`, `cohtml/cohtml` and `chart.js`).
...
The runtime export list settles it precisely: `Unit` **is** exported and its members are live string values (`source.js:12425`, the enum body at `:29024-29063`), so `Unit.Money` works. `TimeFormat`, `TemperatureUnit` and `UnitSystem` are **not** on that list, even though the `.d.ts` declares all three as `export enum` (`@colossalorder/create-csii-ui-mod/template/types/l10n.d.ts:95-107`). Their values are `TwentyFourHours = 0`/`TwelveHours = 1`, `Celsius = 0`/`Fahrenheit = 1`/`Kelvin = 2`, `Metric = 0`/`Freedom = 1` (`source.js:26105-26116`).

**But they are not out of reach, which narrows the wiki's advice by one step.**
All three are registered in the module registry under `game-ui/menu/data-binding/options-bindings.ts`, beside `BindingConflict` and `defaultUnitSettings` (`source.js:26127`, the three getters at `:26362-26379`).
...
Three enums on one settings class, all under a `Unit` group in the interface settings page (`src/Game/Game.Settings/InterfaceSettings.cs:186-193`):

- `TimeFormat { TwentyFourHours, TwelveHours }` (`:33-37`)
- `TemperatureUnit { Celsius, Fahrenheit, Kelvin }` (`:39-44`)
- `UnitSystem { Metric, Freedom }` (`:46-50`)

Defaults are `TwentyFourHours`, `Celsius`, `Metric` (`:248-250`).
...
`InterfaceSettings.locale` is a hidden string defaulting to the literal `"os"` (`InterfaceSettings.cs:93-94`, `:217`), which `LocalizationManager.SetActiveLocale` resolves to the system language through `Application.systemLanguage` (`LocalizationManager.cs:206-234`, `kOsLanguage = "os"` at `:23`, `GetSystemLocaleId` at `:184-192`).
`currentLocale` is the visible dropdown, `[Exclude]`d from serialization, resolving `"os"` to the live active id on read (`InterfaceSettings.cs:73-89`); its item list is `GetLanguageValues()`, which pairs each supported locale id with `LocalizedString.Value(GetLocalizedName(id))` — a **literal** display name from the locale asset's header, not a key (`:262-278`, `LocaleAsset.localizedName` at `:61`, read at `:83-92`).
`InterfaceSettings.Apply()` calls `SetActiveLocale(locale)` (`src/Game/Game.Settings/InterfaceSettings.cs:256-260`), and the frontend's own `l10n`/`selectLocale` trigger does both — switch the manager and write the setting back (`src/Game/Game.UI.Localization/LocalizationBindings.cs:75-83`).
...
Rots: the three preference enums and their member order — re-read `src/Game/Game.Settings/InterfaceSettings.cs:33-50`.
...
**The installed version is `1.6.2f1 (767.21d1) [6300.26419]`, which is the decompile's baseline.**
...
The game itself answers it — `GameManager` logs `Version.current.fullVersion` at startup and writes the same string into a `version` file in its persistent data folder (`src/Game/Game.SceneFlow/GameManager.cs:2310`, `:2351`, the string composed at `src/Colossal.Core/Colossal/Version.cs:105-109`) — and both read that build.
So the shipped data and the decompiled source are the same build, and every count below is a 1.6.2f1 count rather than an approximation of one.

**Twelve locales, packed per content directory.**
The base set is packed into `Cities2_Data/Content/Game/Locale.cok`: `de-DE`, `en-US`, `es-ES`, `fr-FR`, `it-IT`, `ja-JP`, `ko-KR`, `pl-PL`, `pt-BR`, `ru-RU`, `zh-HANS`, `zh-HANT`.
Each content pack directory under `Cities2_Data/Content/` that ships one carries a `Locale.cok` of its own holding the same twelve locale files with that pack's strings, keys absent from the base `en-US` (`BridgesAndPorts`: 1,426 `en-US` entries, 1,424 of them not in the base); which packs are present varies by install, and the radio-pack directories carry none.
...
**Decoding all twelve base locales.** `en-US` holds **22,163 entries and 260 index-count keys**.
The other eleven hold 22,165 entries each and the same 260 index-count keys.
...
Because `en-US` is the hard-coded fallback locale (`src/Game/Game.SceneFlow/GameManager.cs:2371-2376`), its key set is the one that defines what the base game ships — each content pack's keys sit in that pack's own package — and every count in the findings below is an `en-US` count over the base game's package alone.

**Verdict: the shipped locale set is twelve, and it is not the set the corpus translates into.**
...
The install settles it: the twelve packed ids.
The corpus's most-translated set does not match it — it carries `nl-NL` in 29 or more translation folders, `pt-PT` in 34 and `ar-SA` in 24, and **none of those three is a base-game locale**; nor is `uk-UA`, which appears in five paths across the twenty repositories.
...
**Verdict: the four identifier shapes account for every key in the base game's package, with no exceptions.**
Running the four regexes from `LocalizationValidation.cs:22-25` over all 22,163 `en-US` keys classifies every one of them and leaves nothing over: 16,642 Hashed, 3,715 Indexed, 1,684 Single, 122 HashedIndexed.
So the one-dot grammar is not merely what the compiler enforces, it is what the shipped data obeys — no key in the base package has the extra dots the wiki's recommended mod shape has, and a mod matching vanilla's shape is matching a rule with 22,163 worked examples behind it.

**Indexed keys are a vanilla workhorse, which is worth knowing because no mod ships one.**
The base package's `en-US` index-count table declares 260 keys totalling **3,837 variants**, and that total is exactly the number of Indexed plus HashedIndexed entries in the same file — the table accounts for every indexed entry with none left over.
...
The largest are `Assets.DISTRICT_NAME` at 1,015 variants and `Assets.CITY_NAME` at 501, then five road-name keys at 210 each — which is the generated-name pool the game draws from, and the reason the `Assets` group carries 12,030 of the file's 22,163 entries against only 30 distinct ids.

**Verdict: the `{NAME:UnitName}` inline format spec is real, and the game's own strings use it — once in the base game's package, and more freely in a content pack's.**
That claim previously rested only on the vendored bundle's substitution code. Sweeping every value in the base package's twelve locales finds exactly one occurrence, and it is the same key in every one of them: `Common.ERROR_ACTION[Mute]` is `Mute ({TIME:DurationSeconds})` in English and the equivalent elsewhere. A content pack's package is not bound by that: `Skyscrapers`' asset descriptions write `Height: {350:Height}.`-style specs in every locale, a literal number where the argument name goes, which the substitution formats in the unit because a missing value whose name parses as a number takes that number (`source.js:29430-29436`).
Two things follow. The spec is confirmed from shipped data rather than from a third-party copy of the UI. And the units the game's strings reach for through it, `DurationSeconds` and `Height`, are two of the five that have **no** C# constant in `Game.UI.Unit` — so the frontend-only tail of the unit list is not a toolchain artefact, the game's own strings depend on it.
Everything else in the base package is the plain `{UPPER_SNAKE}` form: 421 of its 22,163 `en-US` values carry a placeholder at all, and 422 carry any `{...}` token.

**Every shipped base locale file carries trailing bytes, and the decompile says why.**
All twelve have bytes past the end of their structure, from 651 in `ru-RU.loc` to 4,109 in `zh-HANT.loc` (`en-US.loc` 1,657).
...
Nothing reads it — both counts in the format are explicit, so `LocaleAsset.Load` stops at the declared end — but a parser that treats end-of-file as end-of-data will trip on every one of them, and any tool writing a `.loc` through the compiler inherits the same defect.

Rots: the locale set and the per-locale entry counts — re-derive by decoding `Cities2_Data/Content/Game/Locale.cok` and each content pack's `Cities2_Data/Content/<pack>/Locale.cok` in a current install.
...
Both count columns ship rather than the ids column alone: a reader who sees `Assets` at 30 ids has no way to guess it carries 12,030 rows, and the gap between the two columns is what tells them a group is hashed or indexed rather than flat, which is the difference between looking a key up and constructing one.
...
The table below is derived from the base game's package `en-US.loc` at 1.6.2f1, decoded as the previous finding describes.
...
**75 groups, 2,181 ids, 22,163 entries.**
...
The generated `Loc` dictionary in the shipped bundle carries **75 groups and 2,181 ids** (`DecompiledCitiesSkylines2/src-ui/source.js:26637-28982`, registered as `game-ui/common/localization/loc.generated.ts` at `:29016-29023`) — the same 75 groups and the same 2,181 ids the base package's `en-US.loc` decode gives, reached from a different artifact by a different route.
It also carries the two things the `.loc` format cannot encode.
Each id's **identifier shape** is which of four classes it is constructed from, and each class is recognisable by the key template it builds: the bare id, `` `${id}[${hash}]` ``, `` `${id}:${index}` `` and `` `${id}[${hash}]:${index}` `` (`:26574-26627`, the templates at `:26579`, `:26594`, `:26606`, `:26620`). The counts are 1,684 Single, 334 Hashed, 153 Indexed and 10 HashedIndexed.
Each id's **argument names** are that constructor's arguments — `Assets.ADDRESS_NAME_FORMAT` is built with `"NUMBER"` and `"ROAD"` (`:26642`).
The Single count cross-checks against the decode exactly: 1,684 Single ids against 1,684 Single entries, which is what a shape carrying one entry per id has to give.
...
It lists 72 groups and 2,013 ids (`CS2-Platter/Platter/UI/tools/source.js:25770-27958`), short by three whole groups — `BikesInfoPanel` (2 ids), `Glossary` (8) and `WealthInfoPanel` (13) — and by 145 further ids spread across fifteen groups, the largest being `Options` (+29), `Editor` (+26), `SelectedInfoPanel` (+22), `Paradox` (+17), `Tutorials` (+10) and `ToolOptions` (+8).
By count the divergence runs one way only — no group has more ids in the copy than in the decode — which is what a copy taken from an older build looks like and not what a wrong copy looks like, and it is the same one-way staleness behind the two formatter divergences this pass found. By name it does not: 102 `Editor` ids in the copy are absent from the decode, against 270 decode ids absent from the copy (128 of them `Editor`).
...
They are 21 of the base package's 75, so **72% of the vanilla namespaces are invisible from C# and built entirely in the frontend** — which is why the decompile alone could never have produced this table.
Representative C# anchors: `src/Game/Game.UI/LocaleIds.cs:5-13` holds five format constants (`Assets.NAME[{0}]`, `Policy.TITLE[{0}]`, `StatisticsPanel.STAT_TITLE[{0}]`, `Maps.MAP_TITLE[{0}]`, `Maps.MAP_DESCRIPTION[{0}]`), and `src/Game/Game.UI/NameSystem.cs:433-490` holds the citizen, train and address name keys.
...
The frontend's localization provider inspects exactly that: its `translate` calls the engine's, compares the result to the id it asked for, and treats equality as a miss, returning the caller's fallback or `null` (`DecompiledCitiesSkylines2/src-ui/source.js:68780-68783`; the Show-IDs and Show-Fallback debug modes are two sibling one-line implementations at `:68784-68789`).

Two renderers consume that, and they differ.

- **A localized element sent from C#** renders `translate(id, value) ?? id`, so a miss shows the **id** (`:29725-29738`). This is the path every `LocalizedString`, `LocalizedNumber`, `LocalizedFraction` and `LocalizedBounds` off the wire takes, dispatched on `__Type` at `:29679-29692`.
- **A component built from the generated `Loc` dictionary** renders `translate(id, fallback) || (showIdOnFail && id ? id : "")`, so a miss with no fallback shows the **empty string** unless the call site opted in (`:26526-26539`).

`showIdOnFail` is set at twelve call sites in the whole bundle, and every one of them renders one of five keys: `Common.ACTION` (`:39338`, `:39669`, `:75676`), `Options.OPTION` (`:67117`, `:67272`, `:73543`, `:73580`), `Options.INPUT_MAP` (`:67263`, `:73537`, `:73574`), `Options.SECTION` (`:75233`) and `Options.FORMAT` (`:74143-74149`) — the input hints, the keybinding rows, the rebinding-conflict dialog, the options page section title and the slider's custom format string.
...
- a **Print input bindings and controls** button that logs a ready-to-paste block of `Options.OPTION[...]` and `Options.OPTION_DESCRIPTION[...]` rows, tab-separated, for every rebindable binding (`LocalizationDebugUI.cs:96-126`);
...
- **`settings-and-input`** generates the keys this topic supplies strings for, and the seam is the eleven `ModSetting` helpers (`src/Game/Game.Modding/ModSetting.cs:303-371`). That file's own bridge (`settings-and-input.md:416`) states the split from the other side and it holds: it owns the widget, the page and the action; this owns the string, the source and the file it lives in. Three of its established facts are load-bearing here and were re-verified — the `id` and `name` composition (`ModSetting.cs:36-44`), the widget path formula `prefix + "." + declaringType.Name + "." + propertyName` (`AutomaticSettings.cs:326-338`), and that a property inherited from a shared settings base gets a widget path carrying the base's name while the helper builds one carrying the derived name, so the row renders its raw key.
- **`mod-lifecycle-and-ordering`** decides when a source can be added and who wins a key collision. The locale manager and every shipped locale exist before `OnLoad` (`src/Game/Game.SceneFlow/GameManager.cs:2371-2387`), and because later sources overwrite earlier ones (`LocalizationDictionary.cs:87`), two mods claiming the same key are resolved by mod load order and by nothing else.
...
- **`frontend-and-injection`, in the UI skill**, owns the module registry, and this topic has three concrete errands for it *[ed. 2026-08-23: the withheld-formatters errand landed as `units-and-formatting`'s in the shipped reference — that file claims it as one of its two — while this topic's shipped `localization.md` claims the `Loc` dictionary alone]* — plus the file itself, since the shipped bundle this pass read is that reference's whole subject. The `Loc` generated dictionary is registered as `game-ui/common/localization/loc.generated.ts` (`DecompiledCitiesSkylines2/src-ui/source.js:29016-29023`), which is how a mod reaches a vanilla key as a typed component rather than as a string literal. The time formatters that `cs2/l10n` does not export live at `game-ui/common/localization/localized-date.tsx` and `.../localized-number.tsx` (`:29941-29990`, `:29381-29424`), which is the answer to the one formatting gap the public module leaves. And the three unit-preference enums the public module also withholds are registered at `game-ui/menu/data-binding/options-bindings.ts` (`:26127`, `:26362-26379`).
  The whole localization surface is nineteen registry entries under `game-ui/common/localization/` (`:26467`-`:30173`, plus the input-path one at `:34462` and the provider at `:68791`).
...
- **~~The vendored UI bundle carries no version.~~ Closed by reading the game's own.** `CS2-Platter/Platter/UI/tools/source.js` is a beautified copy of the game's compiled UI with no provenance note in that repository and no version string found in it, and every frontend claim here once rested on it. The game ships the same file plain on disk at `Cities2_Data/Content/Game/UI/index.js`, first-party and at the installed build, so the question of the copy's age stopped mattering: each claim was re-derived from the shipped bundle in the 2026-08-03 pass, and the copy is now cited only where the two disagree.
  It is left recorded for the rule it leaves. The divergences are the verdict's above, and they mostly run the way an older copy's do. So a vendored artifact that disagrees with the install is stale until something shows otherwise, and there is no reason to take a frontend fact from a mod repository while the game is installed.
...
- **`ToUpper()` versus `ToUpperInvariant()` in the enum key builders is inert.** `ModSetting.GetEnumValueLocaleID<T>` uses the culture-sensitive form (`ModSetting.cs:335`) and `AutomaticSettings.GetEnumValues` the invariant one (`AutomaticSettings.cs:907`); they can only disagree for a type name containing `i` under a Turkish or Azeri culture, and the game pins `CultureInfo.CurrentCulture` to invariant at boot (`src/Game/Game.SceneFlow/GameManager.cs:542`). Checked and closed.
- **No corpus mod ships an indexed key.** Sweeping all 20 repositories for `:0`-suffixed identifiers and for `RandomLocalizationIndex` returns nothing, so the random-variant mechanism has no worked mod example. It is not short of vanilla ones: the base game's package `en-US.loc` declares 260 indexed keys totalling 3,837 variants, a content pack's package can declare index-count keys of its own (`BridgesAndPorts` does), and both are first-party.
- **Network Tools' loading path is not readable.** Its mod class derives from `LucaModBase<T>`, which is not in the checkout (`CS2-NetworkTools/NetworkTools.Mod/NetworkToolsMod.cs:32`), so how its `L10n/lang/*.json` files reach the manager cannot be established — only that the files exist, that Crowdin is wired to them (`CS2-NetworkTools/crowdin.yaml:1-3`), and that an `I18N` build configuration regenerates the English one. The same limitation applies to Move It's `QCommonLib` and Write Everywhere's `BasicIMod`, as `settings-and-input.md:39` and `mod-lifecycle-and-ordering.md:474` already record.
