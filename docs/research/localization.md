...
A fourth source carries the strings themselves: the game's own compiled `.loc` assets, read out of an installed copy at `1.6.0f1 (419.d6c6) [6216.19404]` — the same build the decompile was taken from — and cited as an install-relative path throughout.
It is first-party and version-known, so it outranks everything else on anything it can answer.
One source used here carries no version of its own and is flagged wherever it is cited: `CS2-Platter/Platter/UI/tools/source.js`, a hand-vendored copy of the game's own compiled UI bundle sitting inside a corpus repository, with no version stamp and no provenance note anywhere in that repo.
The shipped `.loc` data has since bounded it — the bundle's namespace list is a strict subset of what the install carries, so it is accurate and a build or more stale — but the frontend behaviour only that bundle records still rests on it alone.
...
The frontend substitutes with the much broader `/{([^{}]+)}/g` and supports an inline format spec the C# regex cannot even see: `{NAME:UnitName}` and `{NAME:UnitName signed}`, where the part after the colon is looked up in the `Unit` enum by **member name** and used to format the value as a number (`CS2-Platter/Platter/UI/tools/source.js:28251-28267`, the vendored game bundle). A placeholder whose value is missing is left in the output verbatim (`:28264`).
...
| `GetOptionFormatLocaleID(opt)`      | `Options.FORMAT[<id>.<name>.<opt>]`             | `:338-341` vs `source.js:67733`                          |
...
The `cs2/l10n` TypeScript declaration the modding toolchain ships declares the same set as an enum with **38** members: the 33 plus `PercentagePrecise`, `BodiesPerMonth`, `TemperaturePrecise`, `Height` and `DurationSeconds` (`Anarchy/Anarchy/UI/types/l10n.d.ts:17-56`). Those five have no C# constant, so reaching them from C# means writing the literal.
That declaration file is vendored per mod and drifts: six distinct versions across 18 copies in the corpus, the newest at 213 lines in 8 repositories and the oldest at 202 (`Time2Work/NightShift/Time2WorkUI/src/types/l10n.d.ts`), which is missing `PercentagePrecise`, `TemperaturePrecise`, `ScreenFrequency`, `Height`, `Custom` and `DurationSeconds` outright.

Rots: the unit list on both sides — re-read `src/Game/Game.UI/Unit.cs` and a current `types/l10n.d.ts`.

**The frontend's number formatter is a lookup table with a garbage fallback.**
The dispatch is `Fc[unit]` with a fallback lambda that renders the number followed by the unit name in angle brackets, so an unrecognised unit string prints as `1234 <myUnit>` (`CS2-Platter/Platter/UI/tools/source.js:28096-28098`, the vendored bundle). That is the visible symptom of a typo'd unit.
The table itself is where the player's preferences bite (`source.js:28099-28145`). A sample, with the locale key each branch renders through:

- `Integer` → plain, thousands-separated. `IntegerRounded` switches to `Common.VALUE_THOUSAND` above 1,000 and `Common.VALUE_MILLION` above 1,000,000.
- `Length` → metric: `Common.VALUE_METER` below 1,000, `Common.VALUE_KILOMETER` above; freedom: `Common.VALUE_YARD` below 1,609, `Common.VALUE_MILE` above.
- `Area`, `Volume`, `Weight`, `WeightPerCell`, `WeightPerMonth`, `Height`, `NetElevation`, `MoneyPerDistance`, `MoneyPerDistancePerMonth` all branch on `unitSettings.unitSystem` the same way.
- `Temperature` branches on `unitSettings.temperatureUnit` across `Common.VALUE_CELSIUS`, `VALUE_FAHRENHEIT`, `VALUE_KELVIN` (`:28132-28142`).
- `Power` divides by 10 and renders kilowatts below 10,000, by 10,000 for megawatts above — so the raw C# value is in units of 100 W.
- `Money` uses `Common.VALUE_MONEY` with no unit-system branch at all.

The separators come from the dictionary too: `Common.THOUSANDS_SEPARATOR` and `Common.DECIMAL_SEPARATOR` are rendered per call and applied by regex (`:28150-28186`). The sign prefix is `-` for a negative value always, and `+` for a positive one or `±` for zero **only when `signed` is set** — otherwise both are empty (`:28203-28205`).

**`LocalizedFraction` and `LocalizedBounds` support far fewer units than `LocalizedNumber`, and fail loudly-but-ugly outside them.**
Fractions handle exactly ten: `Volume`, `VolumePerMonth`, `Weight`, `WeightPerMonth`, `Power`, `Energy`, `BodiesPerMonth`, `XP`, `Integer`, `IntegerPerMonth`; anything else renders `` `${value} / ${total} <${unit}>` `` (`source.js:28411-28426`).
Bounds handle exactly three: `Power`, `PercentageSingleFraction`, `Temperature`; anything else renders `` `${min}–${max} <${unit}>` `` (`source.js:28371-28389`). Bounds also short-circuits to `LocalizedNumber` when `min === max` (`:28371-28375`).
...
The game has `LocalizedTime`, `LocalizedDateTime` and `LocalizedTimestamp`, plus `useTimeFormat`, `useDateFormat`, `formatInteger` and `formatFloat` — all registered in the module registry under `game-ui/common/localization/localized-date.tsx` and `.../localized-number.tsx` (`source.js:28676-28722`, `:28207-28250`). `LocalizedTime` already branches on `unitSettings.timeFormat` and renders `Common.TIME_FORMAT` or `Common.TIME_FORMAT_12` with `Common.TIME_PERIOD_AM`/`PM` (`:28614-28628`).
What `cs2/l10n` exports at runtime is **eleven names and no more**: `Localized`, `LocalizedBounds`, `LocalizedDate`, `LocalizedDuration`, `LocalizedEntityName`, `LocalizedFraction`, `LocalizedNumber`, `LocalizedPercentage`, `LocalizedString`, `Unit`, `useLocalization` (`source.js:12068-12080`).
...
The runtime export list settles it precisely: `Unit` **is** exported and its members are live string values (`source.js:12078`, the enum body at `:27968-28006`), so `Unit.Money` works. `TimeFormat`, `TemperatureUnit` and `UnitSystem` are **not** exported, even though the `.d.ts` declares all three as `export enum` (`Anarchy/Anarchy/UI/types/l10n.d.ts:95-107`). Those are the three whose values a mod has to write as literals — `TwentyFourHours = 0`/`TwelveHours = 1`, `Celsius = 0`/`Fahrenheit = 1`/`Kelvin = 2`, `Metric = 0`/`Freedom = 1`.
...
**Verdict: the vendored bundle's list is accurate and stale, and it is a strict subset of what the game ships.**
The generated `Loc` dictionary in `CS2-Platter/Platter/UI/tools/source.js:25770-27958` lists 72 groups and 2,013 ids.
Every one of its 72 groups exists in the shipped data, and no group's id count exceeds the shipped one — the divergence runs one way only, which is what a copy taken from an older build looks like and not what a wrong copy looks like.
It is short by three whole groups — `BikesInfoPanel` (2 ids), `Glossary` (8) and `WealthInfoPanel` (13) — and by 117 further ids spread across fifteen groups, the largest being `Options` (+29), `SelectedInfoPanel` (+22), `Paradox` (+17), `Tutorials` (+10), `Chirper` (+7) and `Common` (+7).
So the bundle can be trusted for the parts the shipped data does not carry — the per-id argument names and the `Single`/`Hashed`/`Indexed`/`HashedIndexed` typing, which the `.loc` does not encode and which have to be re-derived from the key text — while the group set and the counts come from the install.
...
A missing key renders as the key. `UILocalizationManager.Translate` sets the data to the looked-up value or, failing that, to the key itself (`src/Game/Game.UI.Localization/UILocalizationManager.cs:19-29`); the frontend's string renderer does the same, falling back through `fallback` and then `showIdOnFail` (`source.js:28493-28501`).
...
- **`binding-layer`, in the UI skill**, carries every `ILocElement` across the wire. `LocalizedString`, `LocalizedNumber<T>`, `LocalizedFraction<T>` and `LocalizedBounds<T>` are `IJsonWritable` structs whose `TypeBegin` names are the strings the frontend switches on — `Game.UI.Localization.LocalizedString` and its three siblings (`src/Game/Game.UI.Localization/LocalizedString.cs:76-86`, `LocalizedNumber.cs:29`, `LocalizedFraction.cs:29`, `LocalizedBounds.cs:29`, matched at `Anarchy/Anarchy/UI/types/l10n.d.ts:57-69`). The `l10n` binding group itself — `locales`, `debugMode`, `activeDictionaryChanged`, `indexCounts` and the `selectLocale` trigger — is that reference's material (`src/Game/Game.UI.Localization/LocalizationBindings.cs:41-51`).
- **`frontend-and-injection`, in the UI skill**, owns the module registry, and this topic has two concrete errands for it. The `Loc` generated dictionary is registered as `game-ui/common/localization/loc.generated.ts` (`CS2-Platter/Platter/UI/tools/source.js:27962-27969`), which is how a mod reaches a vanilla key as a typed component rather than as a string literal. And the time formatters that `cs2/l10n` does not export live at `game-ui/common/localization/localized-date.tsx` and `.../localized-number.tsx` (`source.js:28676-28722`, `:28207-28250`), which is the answer to the one formatting gap the public module leaves.
...
- **The vendored UI bundle carries no version.** `CS2-Platter/Platter/UI/tools/source.js` is a beautified copy of the game's compiled UI with no provenance note in that repository and no version string found in it. Its `Unit` enum matches the newest `l10n.d.ts` in the corpus (it has `Height` and `DurationSeconds`), so it is recent, but every claim sourced from it may be a build or two off 1.6.0f1. Two units the newest `.d.ts` declares — `PercentagePrecise` and `TemperaturePrecise` — have no entry in that bundle's formatter table and would hit the `<unit>` fallback there; whether that is a bundle-age artefact or a real gap could not be settled.
  The shipped `.loc` data bounds the staleness without removing it: every one of the bundle's 72 groups exists in the install, no group's id count exceeds the install's, and it is short by three groups and 117 ids — divergence running one way only, which is what an older copy looks like rather than a wrong one. So it is trustworthy where it is the only source (the per-id typing and argument names, the formatter tables, the module registry paths) and superseded everywhere the `.loc` can answer.
