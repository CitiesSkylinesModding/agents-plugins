...
`TripNeededSystem` also fires it when a work commute finds no destination and no household car is free to try — the household owning none, or the citizen already keeping one (`src/Game/Game.Simulation/TripNeededSystem.cs:1250-1267`) — **an unreachable workplace un-employs the citizen**, and the same block writes `m_LastCommuteTime` from the path duration on success.
...
The result is applied as **two ±1 random walks, one per byte, each drawn separately against its own target** (`:543-547`):
...
- Teleports: if the citizen is already in the target building, it enables `Arrived`, adds `TravelPurpose` directly and clears the trips, writing no `CurrentBuilding` (`TripNeededSystem.cs:1036-1051`); a separate path-done short-circuit at `:1538-1548` is the branch that also sets `CurrentBuilding`.
- Enqueues a pathfind: it builds a `PathfindParameters` and a `SetupQueueItem` and pushes it to the pathfind queue (`:1176` for a citizen already holding a `Target`, `:1518-1519` otherwise), adding a null `Target` as the pending marker only at the second site, on the citizen-level `IsPathfindingPurpose` purposes (`:1520-1523`).
...
`CitizenBehaviorSystem` (`SystemOrder.cs:375`, interval 16 at `src/Game/Game.Simulation/CitizenBehaviorSystem.cs:1016`) is what a citizen does when it has no `TravelPurpose`. Its main job (`:616-780`) checks dead, imprisoned and moving away first, then resolves the citizen's "home" — rented property, homeless temp home, tourist hotel, or an outside connection for a commuter (`:694-727`) — and continues, in order: meeting, work or study hours, sleep, shopping, leisure.
...
The generalisable technique is **re-apply on every load**. `LateInitialize` is the first writer, called from `PrefabInitializeSystem` in `PrefabUpdate` for entities carrying `Created` — but it is not the only one. `GameModeSystem` (registered in the front band of `Deserialize`, `SystemOrder.cs:796`) calls `RestoreDefaultData` then `ApplyMode` on every `GameManager.Load`, and all but three of the parameter components this topic reads have a mode class under `Game.Prefabs.Modes` that rebuilds them field by field from authored prefab data (`EconomyParametersMode.cs:156`, `:207`, and the sibling mode classes). `TimeSettingsData`, `CitizenParametersData` and `TripPriorityParametersData` have none and are genuinely write-once.
