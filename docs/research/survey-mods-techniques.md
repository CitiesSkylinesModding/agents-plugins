...
**Correction, 2026-08-11:** the `<LangVersion>9</LangVersion>` above is what the corpus does, not what the toolchain requires. `Mod.props` pins C# 9, but a `<LangVersion>` set *after* the two imports raises it — C# 14 builds, Bursts and runs, verified end to end against 1.6.0f1. What the raise costs is shims for the runtime types `net48` lacks, and block namespaces in any file the Entities generators emit a partial for. `plugins/cs2-modding/skills/cs2-mod-project/references/build-pipeline.md` carries both; do not re-derive "mods use C# 9" from this section.
