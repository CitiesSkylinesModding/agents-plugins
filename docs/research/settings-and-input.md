...
**Baseline.** Decompiled game 1.6.2f1; mod corpus read 2026-08-03 at the commits the 20-repository checkout carried; wiki fetched live 2026-08-03 (the bot challenge did not fire, so no snapshot substitution was needed).
The frontend claims added in the 2026-08-03 re-sweep read the user's own installed copy at `1.6.2f1 (767.21d1) [6300.26419]` — the decompile's own version — through two artifacts the original pass had no list naming: the shipped UI bundle, cited as a copy of `Cities2_Data/Content/Game/UI/index.js` reformatted with prettier at its defaults and read at `DecompiledCitiesSkylines2/src-ui/source.js`, **140,104 lines**; and the official UI mod scaffold's declarations at `@colossalorder/create-csii-ui-mod/template/types/`, cited package-relative — the package resolving through the npm-global junction to `<install>/Cities2_Data/Content/Game/.ModdingToolchain/npx-create-csii-ui-mod/`, the game's own files, versioned by the install and not by npm (conflicts.md's ruled scaffold-citation entry).
...
- the prefix on every localization key the page generates (`GetSettingsLocaleID`, `GetOptionLabelLocaleID` and the nine siblings at `:303-371`);
...
The twentieth repository, CS2-WriteEverywhere, keeps its settings class inside the closed-source `BasicIMod` framework `mod-lifecycle-and-ordering.md:474` records as absent from the checkout; only its data class `BelzontWE/WEModData.cs:17` is readable here.
...
**Nothing in the game ever calls it on a mod's setting.** Several of the game's own settings classes call their own `SetDefaults()` from their own constructors (`src/Game/Game.Settings/AudioSettings.cs:78`, `GameplaySettings.cs:44`, `GeneralSettings.cs:188`, `InputSettings.cs:148`, `EditorSettings.cs:79`, `GraphicsSettings.cs:533/551`), and the only external caller is `SharedSettings`, which iterates the game's own list and no other (`SharedSettings.cs:126-127/136-138`).
...
`src/Game/Game.Settings/` holds 38 `SettingsUI*` types at 1.6.2f1, plus `IgnoreEqualsAttribute` and `ModdingToolchainUIButtonAttribute` which are not part of this surface.
...
**Formatting of the chosen widget:** `SettingsUICustomFormatAttribute` (`:6`, fields `fractionDigits`, `separateThousands = true`, `maxValueWithFraction = 100f`, `signed`) — setting it replaces the slider's `unit` with the literal `"custom"` and hands the numeric formatting to the frontend (`AutomaticSettings.cs:1304-1310` for the int slider, `:1341-1349` for the float slider).

**Verdict (2026-08-03 re-sweep): the four fields are not symmetric across the two sliders, and the original pass read the attribute rather than the two builders.** The int builder copies `separateThousands` and `signed` only (`AutomaticSettings.cs:1304-1310`); the float builder copies all four, clamping `fractionDigits` at 0 (`:1341-1349`). The two first-party sources outside `src/` agree with the builders and with each other: the scaffold declares `IntSliderField extends IntSliderFieldBase<number>, WarningSign { separateThousands?: boolean; signed?: boolean }` against `FloatSliderField`'s `fractionDigits` (inherited from `FloatSliderFieldBase`), `separateThousands`, `maxValueWithFraction` and `signed` (`@colossalorder/create-csii-ui-mod/template/types/bindings.d.ts:664-697`), and the bundle's slider row destructures exactly the float set (`DecompiledCitiesSkylines2/src-ui/source.js:74134-74151`). So `fractionDigits` and `maxValueWithFraction` on an `int` property are inert, and no source reports it.

**The frontend half of the custom format, first-party.** The slider row branches on `unit === Unit.Custom` and, only then, resolves the locale id `Options.FORMAT[<path>]` with a `"{SIGN}{VALUE}"` default and `VALUE`/`SIGN` substitution (`source.js:74130-74151`), which is the same key `ModSetting.GetOptionFormatLocaleID` builds as `"Options.FORMAT[" + id + "." + name + "." + optionName + "]"` (`src/Game/Game.Modding/ModSetting.cs:338-341`). `localization.md:190-196` owns that key and already states it; recorded here so the attribute's other end is not left dangling.
...
`SettingsUISection`, `SettingsUIAdvanced`, `SettingsUISearchHidden`, `SettingsUIDisableByCondition` and `SettingsUIHideByCondition` all check the property first and then fall back to `property.declaringType.GetCustomAttributes(...)` (`AutomaticSettings.cs:410/419/474/497/773`).
`SettingsUIHidden`, `SettingsUIDeveloper` and `SettingsUIPlatform` do **not**: they read the property alone (`:734-741/743-746/748-755`), so putting `[SettingsUIHidden]` on the settings class has no effect at all even though its `AttributeUsage` permits it.
...
Each of those items wraps an `AutomaticSettings.ManualProperty` — a hand-built `IProxyProperty` carrying its own getter, setter and attribute list (`ModdingSettings.cs:226/241/263/281/302`, type at `AutomaticSettings.cs:595-650`).
That is the sanctioned escape hatch for a row whose shape no property can express.

The automatic path is `FillSettingsPage(SettingPageData, Setting)` (`AutomaticSettings.cs:984-1046`) and it runs exactly once per registration:

1. Read `[SettingsUITabOrder]` and `[SettingsUIGroupOrder]` off the settings **class**, resolving the getter-method overload first and falling back to the literal array (`:986-1021`). Each name is entered into an ordered dictionary; the index is what later sorts the tabs and the groups.
2. Enumerate `setting.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public)` (`:1022`). Inherited properties are included, which is why `ModSetting.id`, `name` and `keyBindingRegistered` need `[SettingsUIHidden]` on them.
3. Skip the property when `IsSupportedOnPlatform` is false, `IsHidden` is true, or `IsDeveloperOnly` is true (`:1026`).
4. `GetWidgetType(property)` (`:1030`, body `:1048-1143`); `WidgetType.None` skips the property.
5. `GetSections(property)` returns one `SectionInfo` per `[SettingsUISection]`, falling back to the declaring type's attributes and then to a synthetic `General` section with empty group names (`:757-793`). **A property carrying several `[SettingsUISection]` attributes appears once per tab**, as a separate `SettingItemData` each time (`:1035-1044`).
6. Each `SettingItemData` resolves its path, display name, description and the seven delegates in its constructor (`:306-324`).

`GetWidgetType` is a flat dispatch on the property's CLR type and its accessors (`:1048-1143`):

| Property type | Condition | `WidgetType` | Widget built |
| --- | --- | --- | --- |
| `bool` | `[SettingsUIButton]` + `[SettingsUIConfirmation]` | `BoolButtonWithConfirmation` | `ButtonWithConfirmation` (`:1168-1190`) |
| `bool` | `[SettingsUIButton]` | `BoolButton` | `Button` in a `ButtonRow` (`:1218-1243`) |
| `bool` | readable **and** writable | `BoolToggle` | `ToggleField` (`:1192-1216`) |
| `bool` | write-only | `BoolButton` | `Button` in a `ButtonRow` |
| `int` | `[SettingsUIDropdown]` | `IntDropdown` | `DropdownField<int>` (`:1245-1271`) |
| `int` | `[SettingsUISlider]` | `IntSlider` | `IntSliderField` (`:1273-1312`) |
| `float` | `[SettingsUISlider]` | `FloatSlider` | `FloatSliderField` (`:1314-1351`) |
| `string` | read+write, `[SettingsUITextInput]` | `StringTextInput` | `StringInputField` (`:1353-1373`) |
| `string` | read+write, `[SettingsUIDropdown]` | `StringDropdown` | `DropdownField<string>` (`:1375-1401`) |
| `string` | read+write, `[SettingsUIDirectoryPicker]` | `DirectoryPicker` | `DirectoryPickerField` (`:1526-1554`) |
...
| `string` | read-only, no attribute | `StringField` | `LocalizedValueField` (`AutomaticSettings.cs:1403-1420`) |
...
**Buttons group by side effect.** `GetButtonsGroup(name, out ButtonRow, Button)` keeps a static dictionary for the duration of one `FillSettingsPage` call, cleared at both ends (`AutomaticSettings.cs:717-732`, `:957/980`).
...
Enum members resolve through `GetEnumValues(enumType, "Options." + prefix)` to `"Options.<id>.<ENUMTYPENAME>[<Member>]"` (`AutomaticSettings.cs:856-916`, key built at `:907-911`), matching `ModSetting.GetEnumValueLocaleID<T>` (`ModSetting.cs:333-336`); a `[SettingsUIHidden]` on an enum **field** drops that member from the dropdown (`AutomaticSettings.cs:905`).
The strings behind these keys are `localization`'s topic, not this one.

**`TryGetAction<T>` is the resolver behind every `Type`+`methodName` attribute** (`:795-833`). It searches methods first, then read-only properties, matching on name, exact return type `T` and zero parameters; instance members are only searched when `type.IsInstanceOfType(setting)`.
It swallows a miss (returns false, the delegate stays null) and only throws when reflection itself fails, wrapping the cause as `$"TryGetAction error with {name} in {type}"` (`:827-830`).
`GetSetterAction` is the stricter sibling: it demands a `void` method with one parameter, accepting the property's type, `int`, or the enum's underlying type for an enum property (`:422-461`).
...
A **section** is what the player sees as a tab down the left of the page; there is one per distinct tab name, built from a `SettingTabData` (`Section`, `:115-325`; `BuildTab` at `AutomaticSettings.cs:215-238`).
Its id is `pageData.id + "." + tabId` when the prefix is on (`:217`), which is exactly what `ModSetting.GetOptionTabLocaleID(tabName)` builds (`ModSetting.cs:323-326`).
A **group** is not an object at all: it is an integer index carried on each option, and the renderer emits a `Breadcrumbs` separator when consecutive options disagree about it _and_ the index resolves to a known group name (`Section.GetItems`, `OptionsUISystem.cs:148-180`, the separator at `:162-175`, the name guard at `:165`). An option whose group name was never registered carries `int.MaxValue` (`AutomaticSettings.cs:231-232`) and therefore gets no separator at all.

**Ordering is by first mention, not alphabetical.** `SettingPageData.AddTab` and `AddGroup` are `TryAdd(name, count)` into an ordered dictionary (`AutomaticSettings.cs:156-164`), populated first from `[SettingsUITabOrder]` and `[SettingsUIGroupOrder]` and then from each property as it is scanned (`:986-1021`, `:1042-1043`).
...
**A group's heading only renders when the class asks for it.** `Section.GetItems` attaches a `Label` to the separator only when the group name is in `groupToShowName`, and emits a bare separator otherwise (`:165-173`), and that set comes from `[SettingsUIShowGroupName]` — the parameterless form sets `showAllGroupNames` and the `params string[]` form names individual groups (`AutomaticSettings.cs:918-930`, `:963-976`, `groupToShowName` at `:87-97`).
...
1. **The property is not public, or not an instance property.** `GetProperties(BindingFlags.Instance | BindingFlags.Public)` (`AutomaticSettings.cs:1022`).
2. **It is filtered before dispatch** — `[SettingsUIPlatform]` excludes the running platform, `[SettingsUIHidden]` is present, or `[SettingsUIDeveloper]` is present and `GameManager.instance.configuration.developerMode` is false (`:1026`, `:734-755`).
3. **`GetWidgetType` returned `None`** — the type/accessor/attribute combination is not in the table above (`:1048-1143`). A numeric property with no slider or dropdown, a read-write `string` with no attribute, and any collection type all land here.
4. **The builder returned null even though the type matched.** `AddBoolButtonProperty` rejects a readable property (`:1220`); `AddIntDropdownProperty`, `AddStringDropdownProperty` and `AddCustomDropdownProperty` reject a missing `[SettingsUIDropdown]` (`:1247/1377/1558`); `AddIntSliderProperty` rejects a missing `[SettingsUISlider]` (`:1276`); `AddCustomDropdownProperty` also rejects a type that is not both `IJsonWritable` and `IJsonReadable` with a parameterless constructor (`:1562-1573`); a second button in a group returns null by design (`:1238-1242`); and the base `SettingItemData.GetWidget()` maps `WidgetType.MultilineText` to null, which only the `MultilineTextSettingItemData` subclass rescues (`:302`, `MultilineTextSettingItemData.cs:17`).
   `SettingTabData.BuildTab` skips every item whose `widget` is null without a word (`AutomaticSettings.cs:221-236`).
...
An attribute whose `Type`+`methodName` pair does not resolve never fails loudly either: `TryGetAction` returns false and the widget simply has no setter, no disable condition or no item list (`AutomaticSettings.cs:795-833`). A dropdown whose getter returns the wrong element type — anything other than `DropdownItem<T>[]` for the matching `T` — gets a null `itemsAccessor` and renders empty (`:835-843`, and the constructor's own type check at `:660-672` is only reached on the other overload).
...
`ExtraAssetsImporter` and `ExtraDetailingTools` register and never unregister; `mod-lifecycle-and-ordering.md:436` records the same count from the disposal side.
...
The whole-database `SaveSettings()` and `SaveAllSettings()` exist as well (`AssetDatabase.cs:692-726`).
...
`Disallow` and `Ignore` are not interchangeable, and the difference is not a UI one: `ProxyBinding.PathEquals` picks `onlyPathComparer` over `pathAndModifiersComparer` whenever either side is `disallowModifiers` (`ProxyBinding.cs:314`, `:824`), and that is the comparison `ConflictsWith` runs (`:690`) for both `ProxyBinding.hasConflicts` (`:447`) and `InputManager.HasConflicts` (`InputManager.cs:746`) — so two `Disallow` bindings on one key conflict whatever modifiers either carries. `ignoreModifiers` (`:318`) has no consumer anywhere in `src/`, and every other test asks only whether the option is `Allow` (`InputManager.cs:852`/`:1028`/`:1914`/`:2090`, `InputRebindingUISystem.cs:194`/`:296`/`:664`/`:734`), so `Ignore` is behaviourally "not `Allow` and not `Disallow`". Corrected 2026-08-10 against the earlier reading of `:314-318`.
...
The **displayed** conflict is usage-aware: `ProxyBinding.hasConflicts` and `ProxyBinding.conflicts` both call `ConflictsWith(x, y, checkUsage: true)`, which requires `Usages.TestAny(x.usages, y.usages)` on top of `PathEquals` and a matching device (`ProxyBinding.cs:447/492`, `:680-698`) — so modifiers are part of that match too, except where either side is `Disallow` (`:824`).
...
- A usage narrows the conflict the player is **shown** — the warning triangle on the options row, and the per-map notification. That is the whole of its effect anywhere this pipeline can look at 1.6.2f1.
...
- **`mod-lifecycle-and-ordering`** owns the `OnLoad`/`OnDispose` frame this whole topic sits in, and two of its established facts are load-bearing here: the ECS world already exists when `OnLoad` runs (`mod-lifecycle-and-ordering.md:71`), which is why `GetOrCreateSystemManaged<OptionsUISystem>()` succeeds inside `RegisterInOptionsUI`; and `OnDispose` is called even on a mod whose `OnLoad` threw (`:349`), which is why every corpus unregistration is null-guarded.
...
- **Which vanilla actions are built-in cannot be read from `src/`.** `custom-tools.md:417` already records this: the `builtIn` flag is deserialized from the input asset (`src/Game/Game.Input/CompositeInstance.cs:128/261`), which is not decompiled source. It bites this topic in one specific place — `[SettingsUIBindingMimic]` requires `action.isBuiltIn` (`ModSetting.cs:126`) and silently does nothing otherwise, so whether a given map/action pair can be mimicked at all is not provable here. The three corpus mimics all target `"Tool"` and `"Shortcuts"` actions and are the only evidence that those are built-in.
  ...
  **The static route was chased first and does not reach, which is why it is recorded rather than dropped.** The asset is `Resources.Load<InputActionAsset>("Input/InputActions")` (`src/Game/Game.Input/InputManager.cs:1301`), so it lives in `Cities2_Data/resources.assets`, not in any `.cok` package. That file is a Unity serialized-object file **without type trees**: it carries type names (`grep -ac ServiceConsumption` → 213, `BuildingPrefab` → 257) and no field names (`m_BuiltIn`, `m_Upkeep`, `"bindings"` → 0 each), and the InputSystem's JSON form is absent too. So reading the flag statically needs a Unity serialized-file parser driven by the field order of the decompiled class, which is a derivation rather than a read. Recorded so a later pass reaches for the running game first rather than repeating this.
...
- **`InputRebindingUISystem` was not pursued.** The rebinding flow the options screen runs when the player presses a key is entirely inside `src/Game/Game.UI.Menu/InputRebindingUISystem.cs`, driven by the three triggers `InputBindingField.Bindings` publishes, and no mod in the corpus touches it. It matters to a mod through the `RebindOptions`, `canBeEmpty` and `modifierOptions` values the action attribute supplies — that system branches on `allowModifiers` throughout (`InputRebindingUISystem.cs:194`/`:296`/`:664`/`:734`).
...
- **The chord-display API is first-party, and the shipped reference now names it.** `ProxyBinding.ToHumanReadablePath()` yields each modifier's display form and then the key's (`src/Game/Game.Input/ProxyBinding.cs:701-713`), over `ControlPath.ToHumanReadablePath`, a one-line wrapper on Unity's `InputControlPath.ToHumanReadableString` with the device omitted (`src/Game/Game.Input/ControlPath.cs:147-150`). `ControlPath.Get(path)` parses a control path into name, device and display name, uppercasing a single-letter name (`ControlPath.cs:21-43`), and is the shape `ProxyBinding.Write` and the game's input hints send to the frontend (`ProxyBinding.cs:782-784`, `src/Game/Game.UI/InputHintBindings.cs:337-401`). The game's own C# use of the string form is a single site, the developer menu's localization tab (`src/Game/Game.Debug/LocalizationDebugUI.cs:109`). A mod does this: SceneExplorer joins the sequence on `+` and re-renders it on every settings-applied event (`SceneExplorer/SceneExplorer/Settings.cs:125-128`) — which follows the player's own rebinds but misses the swaps the game's conflict resolution makes, since `InputRebindingUISystem.CompleteAndSwapConflicts`/`CompleteAndUnsetConflicts` call `InputManager.SetBindings` directly with no settings apply (`src/Game/Game.UI.Menu/InputRebindingUISystem.cs:256`, `:272`).
  Every rebind path instead funnels through `SetBindingImpl`, whose closing `action.Update()` (`src/Game/Game.Input/InputManager.cs:998`) raises the public `ProxyAction.onChanged` (`src/Game/Game.Input/ProxyAction.cs:363`, `:642-643`); `ProxyBinding` is a struct (`ProxyBinding.cs:12`), so any held copy is a snapshot. The shipped rule therefore names display-time rebuild off `ProxyAction.bindings` (`ProxyAction.cs:204-217`) or `onChanged`, not the settings-applied event.
  Rots: `ProxyBinding.ToHumanReadablePath`, `ProxyAction.onChanged` and `ControlPath.Get`'s three fields — `Game.Input.ProxyBinding`, `Game.Input.ProxyAction` and `Game.Input.ControlPath`.
- **Dual usage declaration was already covered and nothing moved.** SceneExplorer names `Usages.kDefaultUsage` and `Usages.kToolUsage` together on three actions and the tool usage alone on a fourth (`SceneExplorer/SceneExplorer/Settings.cs:13-16`), which against the five-member default set (`src/Game/Game.Input/BuiltInUsages.cs:8-20`) is a narrowing — exactly the replace-not-add rule the reference states, with the ruled `conflicts.md` entry on usage scoping governing what it buys.
