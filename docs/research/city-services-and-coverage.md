...
**`transportation-and-vehicles`** owns every vehicle this topic dispatches — `Ambulance`, `Hearse`, `FireEngine`, `PoliceCar`, `GarbageTruck`, `PostVan`, `MaintenanceVehicle`, `Helicopter` — and the vehicle `*AISystem`s at `SystemOrder.cs:315-335`. The boundary is the request: this topic creates it and chooses the source, that topic drives the vehicle there and reports back with `HandleRequest`. `TransportDepot` is theirs despite carrying `ServiceDispatch` (and `ServiceDistrict` when it is a taxi depot, `src/Game/Game.Prefabs/TransportDepot.cs:48-52`); `ParkingFacility` is theirs and carries neither.
