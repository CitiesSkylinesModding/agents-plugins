...
**Baseline.** Every claim was established against game version 1.6.2f1 (Unity 2022.3.71f1), assembly `VersionInternal("1.6.2f1 (767.21d1) [6300.26419]")` at `src/Game/Properties/AssemblyInfo.cs:19`.
...
Live values were read from the user's running city through the sibling Unity plugin on 2026-08-11 under `EasyMode`, and re-read at 1.6.2f1 on 2026-09-18 under `NormalMode` where a reading below carries that stamp; the simulation was **paused** (`SimulationSystem.selectedSpeed == 0`, frame 8,435,350) for the whole session — the dead ends record the consequences.
...
UI-bundle citations are to the reformatted copy at `DecompiledCitiesSkylines2/src-ui/source.js`, produced with prettier at its defaults, **140,104 lines** — the count matches this file's citations.
...
The result lands in `DynamicBuffer<Game.Net.ServiceCoverage>` on the **road edge**, indexed by `(int)CoverageService` (`:268`, `src/Game/Game.Net/ServiceCoverage.cs:7-10`). A building reads its own coverage by sampling that buffer at its position along its road edge: `NetUtils.GetServiceCoverage(coverages, service, curvePos)` is `lerp(m_Coverage.x, m_Coverage.y, curvePos)` (`src/Game/Game.Net/NetUtils.cs:425-433`), against `Building.m_RoadEdge` and `Building.m_CurvePosition`.
...
**How the search travels is per service, and it is the sharpest split in the topic** (`ServiceCoverageSystem.SetupPathfindMethods`, `ServiceCoverageSystem.cs:665-700`):
...
`ServiceCoverageSystem` is registered plainly into `SystemUpdatePhase.GameSimulation` (`src/Game/Game.Common/SystemOrder.cs:307`) with **no `GetUpdateInterval` override** — it runs every simulation frame and gates itself:
...
Each of the eight services therefore owns **32 consecutive frames of a 256-frame cycle**, and the whole rotation completes once per 256 frames. On the one frame where the service is about to change (`frameService != frameService2`, `ServiceCoverageSystem.cs:525-532`), the system does two things at once (`ServiceCoverageSystem.cs:533-614`):

1. **Enqueues the next service's searches.** Every building whose `CoverageServiceType` shared component matches the *next* service is queued with `m_QueueFrame = frameIndex + 192` and `m_ResultFrame = frameIndex + 256` (`ServiceCoverageSystem.cs:537-552`). `EnqueuePendingCoverages` submits **at least** `ceil(count / 192)` items per call and continues past that only for items whose `m_QueueFrame` has already arrived (`ServiceCoverageSystem.cs:625-648`) — so a city with many covering buildings spreads its searches across the slot rather than spiking on one frame. `m_ResultFrame` is a deadline rather than a delivery time — `PathfindResultSystem` consumes a coverage action the moment it completes (`src/Game/Game.Pathfind/PathfindResultSystem.cs:303-306`, the pending gate at `:331`) — so **the values a slot applies come from searches enqueued during the previous rotation, landing whenever the pathfinder finished them.**
...
Every consumer of `Game.Net.ServiceCoverage` at 1.6.2f1, from a grep of `src/Game/` for `CoverageService.` outside the declaring files and for the raw indexed reads:

| Service | Read by | What it does |
| --- | --- | --- |
| `Healthcare` (0) | `CitizenHappinessSystem.GetHealthcareBonuses` (`src/Game/Game.Simulation/CitizenHappinessSystem.cs:1190-1201`); `LandValueSystem` edge job index `[0]` (`src/Game/Game.Simulation/LandValueSystem.cs:186-187`) | health and wellbeing bonus; land-value bonus |
...
| `Police` (2) | `CrimeAccumulationSystem` (`src/Game/Game.Simulation/CrimeAccumulationSystem.cs:107-111`); `CitizenPathfindSetup` homeless-shelter target cost (`src/Game/Game.Simulation/CitizenPathfindSetup.cs:424-426`, `:604`); `LandValueSystem` index `[2]` (`LandValueSystem.cs:190-191`) | crime accumulation, shelter choice, land value |
| `Park` (3) | `CitizenHappinessSystem.GetEntertainmentBonuses` (`CitizenHappinessSystem.cs:1215-1228`) | wellbeing bonus, `min(1, sqrt(coverage/1.5))` shaped |
| `PostService` (4) | **nothing** | — |
| `Education` (5) | `CitizenHappinessSystem.GetEducationBonuses` (`CitizenHappinessSystem.cs:1203-1213`); `LandValueSystem` index `[5]` (`LandValueSystem.cs:188-189`) | wellbeing bonus scaled by `sqrt(n)`, where the caller's `n` is the household's size when the scored citizen is itself a child and 0 otherwise (the count at `CitizenHappinessSystem.cs:331-336` tests the scored citizen's age, never the member's — `citizens-and-households` traps it); land value |
| `EmergencyShelter` (6) | **nothing** | — |
| `Welfare` (7) | `CitizenHappinessSystem.GetWellfareBonuses` / `GetWelfareValue` (`CitizenHappinessSystem.cs:1398-1409`); `CrimeCheckSystem` recurrence suppression (`src/Game/Game.Simulation/CrimeCheckSystem.cs:167-174`) | wellbeing bonus weighted by how unhappy the citizen already is; a repeat offender's crime is cancelled with probability `coverage * m_WelfareCrimeRecurrenceFactor` against a population-scaled roll (`CrimeCheckSystem.cs:159`, `:170`) |
...
- **Coverage feeds where households choose to live**, through `PropertyUtils.GetPropertyScore` / `GetApartmentQuality` / `GetGenericApartmentQuality`, which take the road edge's whole `ServiceCoverage` buffer plus the healthcare, park, education, telecom, garbage and police service prefabs (`src/Game/Game.Buildings/PropertyUtils.cs:482`, `:602`, `:676`, read at `:628`), called from `HouseholdFindPropertySystem` (`HouseholdFindPropertySystem.cs:193`, `:533`, `:602`).
- **The healthcare, education and entertainment bonuses are gated on progression, and the welfare pair is not.** `GetHealthcareBonuses`, `GetEducationBonuses` and `GetEntertainmentBonuses` each return `int2(0,0)` when `isLocked` (`CitizenHappinessSystem.cs:1192-1195`, `:1205-1208`, `:1217-1220`) — the caller's `locked.HasEnabledComponent(serviceEntity)` check on the service prefab entity, passed in as a bool — while `GetWellfareBonuses`/`GetWelfareValue` (`:1398-1406`) open straight on the coverage read. A city that has not unlocked one of the three gated services gets no wellbeing from it even if a mod places the building.
...
**The curve's shape, read live at 1.6.2f1** from the `BuildingEfficiencyParameterData` singleton (entity 63:1) by evaluating `m_ServiceBudgetEfficiencyFactor`:
...
- **A factor at exactly 1 is not in the buffer.** A query for a building's `ServiceBudget` factor returns nothing at 100% budget, because `SetEfficiencyFactor` deleted it. `GetEfficiencyFactor` returns `1f` for a missing entry (`BuildingUtils.cs:249-259`), which is the correct read.
...
**The education requirement is a triangular kernel and it is entirely C#** (`src/Game/Game.Economy/EconomyUtils.cs:1386-1416`):
...
The fee slider is a 200-step lerp: the panel sends `lerp(item.min, item.max, t / 200)` and displays `200 * (fee - min) / (max - min)`, with `min` written as the literal 0 and `max` as `FeeParameters.m_Max` (`source.js:109602-109626`, the C# side at `src/Game/Game.UI.InGame/ServiceBudgetUISystem.cs:217-220`).
...
The nine joins sum to 166 against a total of 163 because all three `PostFacilityData` prefabs also carry `MailBoxData` — `PostFacility.GetPrefabComponents` adds it whenever `m_MailBoxCapacity > 0` (`src/Game/Game.Prefabs/PostFacility.cs:34-37`) — and `Initialize` tests `PostFacilityData || MailBoxData` in one arm.
...
Four sampled prefabs, live on `NormalMode`, to show the field shape (the values are prefab data and do not ship; `EasyMode` scales all three fields through `ServiceCoverageGlobalMode`'s range, capacity and magnitude multipliers, `src/Game/Game.Prefabs.Modes/ServiceCoverageGlobalMode.cs:31-33`, so the 1.6.0f1 read on that mode returned larger figures): `PoliceStation01` `{ Police, m_Range=5000, m_Capacity=20000, m_Magnitude=4 }`, `ElementarySchool01` `{ Education, 3500, 15000, 10 }`, `EmergencyShelter01` `{ EmergencyShelter, 1500, 5000, 5 }`, `PostMailbox01` `{ PostService, 1000, 2000, 1 }`. What the sample shows and the reference can state is the **spread**: the three fields move together across a service's building tiers but not in a fixed ratio, and the services differ widely in range — a derived ratio over the sampled values is a prefab magnitude and does not ship (the ruling at the top of this file) — so no reader should reason about one field from another.
...
`TelecomCoverageSystem` (`src/Game/Game.Simulation/TelecomCoverageSystem.cs`) runs at `GameSimulation` on a 4096-frame interval (`SystemOrder.cs:430`, `TelecomCoverageSystem.cs:647-650`) and rebuilds a **128 × 128** map from scratch each time (`public const int TEXTURE_SIZE = 128`, `:631`). Per facility (`:188-226`):
...
The city-wide figure is `TelecomStatus { m_Capacity, m_Load, m_Quality }` (`src/Game/Game.Simulation/TelecomStatus.cs:5-9`), with `m_Quality` a **density-weighted** mean of `min(1, 2*strength / (1 + 1/capacity))` over all 16,384 cells (`TelecomCoverageSystem.cs:162-186`) — so empty land does not dilute it.
...
| `MailAccumulationSystem` | 64 (`kUpdatesPerDay = 256`) | `src/Game/Game.Simulation/MailAccumulationSystem.cs:326`, `:360` |
...
`CrimeAccumulationSystem` asserts its own cadence at construction: `Assert.IsTrue((long)(kUpdateInterval * 16) >= 512L)` (`CrimeAccumulationSystem.cs:283`) — a first-party statement that lowering the interval is unsupported.

The registration block for this topic is one contiguous run: `ServiceRequestSystem` at `ModificationEnd` (`SystemOrder.cs:270`), `CityServiceEfficiencySystem` at `:292`, `CityServiceBudgetSystem` `UpdateAfter` at `:302`, `ServiceCoverageSystem` at `:307`, the vehicle AI systems at `:316-336`, the facility AI systems at `:522-542` (two of which — `SewageOutletAISystem` and `WaterPumpingStationAISystem` — belong to `utilities-and-flow-networks`), and the dispatch systems at `:555-569`, all in `GameSimulation`.
...
| `Game.Prefabs.GarbageParameterData` | `m_HomelessGarbageProduce`, `m_CollectionGarbageLimit`, `m_RequestGarbageLimit`, `m_WarningGarbageLimit`, `m_MaxGarbageAccumulation`, `m_BuildingLevelBalance`, `m_EducationBalance`, `m_HappinessEffectBaseline`, `m_HappinessEffectStep`, `m_AdaptiveCollectionMargin` (`GarbageParameterData.cs:31`), plus the service and two notification prefabs |
...
**Buffers on the city entity — `EntityManager.GetBuffer<T>(CitySystem.City)`:** `Game.City.ServiceFee` (nine elements at 1.6.2f1), `Game.City.CityModifier`.

**A buffer on a plain singleton entity, not the city:** `Game.Simulation.ServiceBudgetData`, reached through `CityServiceBudgetSystem.GetServiceBudget(servicePrefab)` / `SetServiceBudget(servicePrefab, percentage)` (`CityServiceBudgetSystem.cs:1059-1114`; the query is options-free at `:810`, and `OnGameLoaded` creates the entity when a save lacks one, `:830-836`). **An empty buffer is a legitimate state**: `SetServiceBudget` appends the only entries there ever are, a missing entry reads as 100 (`:1080`), `GetServiceBudget` returns 0 for a prefab outside the system's collected budget map (`:1067-1070`), and `SetServiceBudget` silently no-ops on one (`:1086-1089`). This pass read entity 98443 carrying a zero-length buffer while the getter returned figures — which is exactly the untouched-sliders state, the getter supplying the 100 defaults.
...
Initializer against live singleton, 1.6.2f1:
...
Entry 3's line-count check on the reformatted bundle passed (140,104 at 1.6.2f1).
...
**`city-state-and-progression`** owns `Locked`, `UnlockRequirement`, `CityModifier`, `CityEffectProvider` and the XP economy — which is where an administration building's entire contribution lives, since `AdminBuilding` itself is an inert tag. It also owns the gate on this topic's happiness bonuses: three of `CitizenHappinessSystem`'s coverage bonuses return zero while the service prefab carries an enabled `Locked` (`CitizenHappinessSystem.cs:1192`, `:1205`, `:1217`).
