...
- A gameface mcp release publishes itself to npm from `release-please.yml` through trusted publishing; `mise publish` is the manual fallback. NuGet stays MANUAL (`mise publish:unity`, run by the user).
