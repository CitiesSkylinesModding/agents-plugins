...
- `environment-and-pollution` — owns the pollution cell maps and water bodies this topic reads and writes (the aquifer's own `GroundWater` map stays here): ground pollution feeds the aquifer, pumps draw surface water down and outlets raise it; pollution inside the pipes is this topic's, pollution in the world is theirs.
