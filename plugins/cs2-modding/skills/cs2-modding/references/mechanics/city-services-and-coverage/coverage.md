...
| `PostService` | nothing | — |
| `Education` | `CitizenHappinessSystem.GetEducationBonuses`; `LandValueSystem`, raw index `[5]` | wellbeing scaled by `sqrt(n)` against `m_NeutralEducation`, where `n` is the household's size when the scored citizen is itself a child and 0 otherwise ([`citizens-and-households`](../citizens-and-households/citizens-and-households.md) traps it); land value |
