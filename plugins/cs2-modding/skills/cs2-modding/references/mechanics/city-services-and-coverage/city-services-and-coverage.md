...
- `transportation-and-vehicles` — owns every vehicle this topic dispatches and their `*AISystem`s; the boundary is the request: this topic creates it and picks the source, that topic drives there and reports back with `HandleRequest`; `TransportDepot` is theirs even though it carries `ServiceDispatch` (and, as a taxi depot, `ServiceDistrict`).
...
- `roads-and-traffic` — owns the substrate coverage is stored on and searched over: `Game.Net.Edge`, `Curve`, `Density`, `BorderDistrict` and the pathfind graph; the `RoadPrefab` zone-block gate deciding which edges carry a coverage buffer at all is a fact both need.
