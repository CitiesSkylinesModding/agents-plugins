...
Disambiguate the two `GetRaycastResult` overloads with explicit type arrays when you declare the patches, since one of them takes a `ref ControlPoint`.
...
The dispatcher splits the `Temp` set by flags — entities flagged `Delete` get `Deleted`, the rest get `Updated` and `Overridden`, except those carrying `Cancel`.
