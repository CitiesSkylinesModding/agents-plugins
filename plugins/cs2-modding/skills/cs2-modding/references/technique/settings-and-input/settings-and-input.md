...
`ModSetting` is abstract and derives from `Setting`, which the game's own settings classes share.
...
Source: `src/Game/Game.Modding/ModSetting.cs` (the `id` formula and the registry write), `src/Game/Game.UI.Menu/OptionsUISystem.cs` (the page replaced under that id).
...
Source: `src/Game/Game.UI.Menu/AutomaticSettings.cs` (the `declaringType` fallback and the widget's path), `src/Game/Game.Modding/ModSetting.cs` (the key helpers built from `name`).
...
Source: `src/Game/Game.Settings/Setting.cs`, `src/Game/Game.Settings/SharedSettings.cs` (the list the only external caller iterates).
...
Source: `src/Game/Game.Settings/Setting.cs`, `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/AssetDatabase.cs` (the type-name match).
...
**`SettingsUIMultilineText` ignores the property's accessor**, rendering the localized string for the property's path instead — where the same read-only `string` property without it renders its runtime value, which is the form a status or version row needs.
...
Source: `src/Game/Game.UI.Menu/AutomaticSettings.cs` (the two slider builders), `Cities2_Data/Content/Game/.ModdingToolchain/npx-create-csii-ui-mod/template/types/bindings.d.ts` (the two frontend field sets).
...
Source: `src/Game/Game.Settings/SettingsUISectionAttribute.cs`.
...
## The reflection pass, and the three rules that decide whether a property becomes a widget

Registration runs one reflection pass over the settings class's public instance properties, and builds the page from what that pass finds.
Read [building the options page](building-the-options-page.md) for the pass itself and for the page's three-level shape — and reach for it when a button or a group lands somewhere you did not put it, when a `Type` + method-name attribute silently resolved to nothing, or when you need the keys a widget's text is looked up under.
The idiom that shape asks for is one line long: declare every tab and group as a `const string` on the settings class, list them in `[SettingsUITabOrder]` and `[SettingsUIGroupOrder]` in the order you want, repeat the group list in `[SettingsUIShowGroupName]`, and reference the constants from every `[SettingsUISection]`.

Three of the pass's consequences decide whether a property becomes a widget at all, so they are here.
...
**`ProxyBinding`'s dispatch ignores the accessors**, but the binding machinery behind it filters for properties that are both readable and writable.
A getter-only `ProxyBinding` property therefore renders a keybinding row over a default-valued, unregistered binding.
Source: `src/Game/Game.UI.Menu/AutomaticSettings.cs` (the dispatch), `src/Game/Game.Modding/ModSetting.cs` (the readable-and-writable filter).
...
Source: `src/Game/Game.UI.Menu/OptionsUISystem.cs`.
...
Source: `src/Game/Game.UI.Menu/OptionsUISystem.cs` (what unregistration removes), `src/Game/Game.Input/InputManager.cs` and `src/Game/Game.Input/ProxyActionMap.cs` (the add-only action and map surfaces), `src/Game/Game.Modding/ModSetting.cs` (the registry that is only written and read).
...
Source: `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/COCParser.cs` (the name-then-block parse), `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/DefaultAssetFactory.cs` (the named setting asset each entry becomes), `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/AssetDatabase.cs` (the name `LoadSettings` looks up).
...
Source: `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/AssetDatabase.cs` (the defaults stored on each fragment), `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/SettingAsset.cs` (the diff on save), `src/Colossal.Core/Colossal.Json/DiffUtility.cs` (the unconditional return when there is no default).
...
Source: `src/Colossal.Core/Colossal.Json/DiffUtility.cs`.
...
Source: `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase/SettingAsset.cs`, `src/Colossal.IO.AssetDatabase/Colossal.IO.AssetDatabase.Internal/SaveSettingsHelper.cs`.
...
Source: `src/Game/Game.Input/ProxyBinding.cs` (the included and excluded fields, and what a sourceless binding reports), `src/Game/Game.Settings/KeybindingSettings.cs` (where the game's own rebinds go).
...
Source: `src/Game/Game.Settings/SettingsUIKeybindingAttribute.cs`, `src/Game/Game.Settings/SettingsUIKeyboardBindingAttribute.cs`.
...
Source: `src/Game/Game.Settings/SettingsUIKeyboardBindingAttribute.cs`, `src/Game/Game.Modding/ModSetting.cs` (the action-name default).
...
Source: `src/Game/Game.Input/BindingKeyboard.cs`, `src/Game/Game.Input/BindingMouse.cs`, `src/Game/Game.Input/BindingGamepad.cs`.
...
Source: `src/Game/Game.Settings/SettingsUIInputActionAttribute.cs`, `src/Game/Game.Settings/SettingsUIKeyboardActionAttribute.cs`, `src/Game/Game.Settings/SettingsUIGamepadActionAttribute.cs`, `src/Game/Game.Input/RebindOptions.cs`, `src/Game/Game.Input/ModifierOptions.cs`, `src/Game/Game.Input/Mode.cs`, `src/Game/Game.Input/ActionType.cs`.
...
Source: `src/Game/Game.Input/InputManager.cs` (the modifier parts and the supported set), `src/Game/Game.Input/ProxyBinding.cs` (the path comparison, and the `Ignore` accessor nothing reads).
...
Source: `src/Game/Game.Settings/SettingsUIInputActionAttribute.cs`, `src/Game/Game.Input/Usages.cs`, `src/Game/Game.Input/BuiltInUsages.cs`.
...
Source: `src/Game/Game.Input/ProxyBinding.cs` (the usage-aware comparison), `src/Game/Game.Input/InputManager.cs` (the per-map notification), `src/Game/Game.UI.Menu/InputRebindingUISystem.cs` (the rebinding cascade's usage gate).
...
Source: `src/Game/Game.Input/InputManager.cs`, `src/Game/Game.Input/InputConflictResolution.cs`.
...
Source: `src/Game/Game.Input/InputConflictResolution.cs` (the three tiers and the pairings), `src/Game/Game.Input/ProxyAction.cs` (the built-in and system-action tests), `src/Game/Game.Modding/ModSetting.cs` (the not-built-in stamp).
...
Source: `src/Game/Game.Input/InputConflictResolution.cs` (only enabled actions are paired), `src/Game/Game.Input/ProxyAction.cs` (what decides that state).
...
Source: `src/Game/Game.Input/InputManager.cs` (the per-map notification and the registry lookup), `src/Game/Game.UI.Menu/InputBindingField.cs` (the row's warning test), `src/Game/Game.Input/ProxyBinding.cs` (`ConflictType`).
...
Source: `src/Game/Game.Input/ProxyAction.cs`, `src/Game/Game.Input/InputBarrier.cs`.
