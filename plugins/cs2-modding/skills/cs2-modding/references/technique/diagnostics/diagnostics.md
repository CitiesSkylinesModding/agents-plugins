...
| `IsNotUniqueWarning`         | Another asset with the same assembly **name** won the duplicate resolution, which orders by already-loaded, then local, then version descending. A local build beats a subscribed copy of the same name — which is why a local deployment overrides the store copy, and why a stale local copy shadows an updated one. |
