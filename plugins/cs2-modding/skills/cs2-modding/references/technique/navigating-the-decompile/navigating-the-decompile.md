...
2. **Local names carry no meaning.** `num`, `num2`, `flag`, `text2`, and worse, locals named after their own type with a numeric suffix — `int2 int5 = m_UpdateRanges[(int)phase];`. Never infer intent from a local identifier, and note the second-order cost: this is exactly why a call site with an inferred generic type argument is unsearchable by type name.
