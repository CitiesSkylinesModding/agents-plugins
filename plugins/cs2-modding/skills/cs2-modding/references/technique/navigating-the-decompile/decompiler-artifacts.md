...
11. **No closure or iterator residue.** No `<>c__DisplayClass`, `_003C`, `_003E` or `<>c` anywhere in `src/Game`, and only a handful of files retain a visible `MoveNext()` — down to a genuine `from r in asset.references where r.Value == null select r.Key` surviving as LINQ. `navigating-the-decompile` states what that buys a reader.
