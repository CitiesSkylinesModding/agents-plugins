...
(VOLATILE: every accessor and registration name on this page — the `SearchSystem` and `UpdateCollectSystem` types in the objects, net, zones, areas, routes and effects namespaces, the simulation namespace's terrain, water, cell-map, demand and counting systems, the prefabs namespace's resource and zone systems, the pathfind namespace's queue system, and the rendering namespace's culling and batch-manager systems.)
