...
- **The mods corpus is input, never output.** It is where techniques and gotchas were learned, and knowledge prose states the technique on its own authority. The single place a mod is named is the setup skill's provisioning catalog, `skills/cs2-modding-setup/references/mod-catalog.md`, which is also the only name list the content lint reads. That catalog ships to an agent provisioning a corpus and says nothing about the lint: the entry shape the lint parses is a contract `scripts/check-skill-content.ts` states and the catalog only has to keep, so an entry losing its `###` heading or its `Source:` line fails the check rather than escaping it. The lint matches both spellings — the display name and the `owner/repo` slug — whole-word and case-sensitively, at two strengths, because a third of the display names are ordinary words and a mechanics reference heading `## Traffic` trips the match while citing nothing. A slug is a citation and nothing else, and so is a display name by default, so both fail the check. A display name on the lint's ordinary-word list is instead reported as a question naming the file and line, and does not fail: a word the game's own subject matter owns cannot credit anybody, because no reader takes it for a name. When the check fires on a word your subject owns, report the collision and stop — the fix is the maintainer adding that word to the list, and never prose bent around it.
  **An entry certifies what its own source shows.** Reading one repository establishes nothing about the others, so a comparative — _the only_, _the deepest_, _the fullest_ — is a claim about the whole corpus and ships only with the sweep that established it. The first entry written without this rule carried three, and the flattest was false: three other catalogued repositories register an editor tool by the very idiom it called unique. A superlative is also the hardest kind of sentence to doubt on re-reading, which is why the rule is a gate on writing one rather than a caution to weigh.
...
- **Volatility marker.** A claim that rots — component field names, system names, save-format versions, UI module paths, raycast mask combinations — carries `VOLATILE:` inline, naming what moves and where it lives: `(VOLATILE: the field names on this component — the component's own declaration.)`. That uppercase token is the only spelling, and durable architectural facts carry none, so `VOLATILE:` greps into the maintenance checklist for the next game version.
  ...
  **A marker nobody can close is noise on the checklist, so propose dropping it rather than rewording it.** The token earns its place by grepping into the next version's work, which means a `VOLATILE:` claim a version sweep cannot clear — a mod's own source, anything outside the game — was never this token's to carry, and an `UNVERIFIED:` claim whose settling experiment is impractical buys a reader nothing but doubt. Offer the drop as the first option and let the maintainer keep it; rewording one twice before proposing that is two rounds spent on a sentence that was going to go.
  **Where the experiment is cheap, run it instead of writing the marker.** A marker costs every future reader a little doubt and the next maintainer a sweep entry, so a question one minute against the running game can answer is not worth either. The answer also tends to beat the doubt: a leak marked as unknown became, once run, the rule that a `TempJob` container names its own allocation site and a `Persistent` one never does — a technique a reader acts on, out of a sentence that would have taught nothing.
...
**Where the answer is something, count the complement instead.** A count that keeps coming back wrong usually counts a set with a contested edge, so each pass draws the boundary somewhere new. One container tally ran five, three, six and wrong again, because which helper and `Unsafe*` types belong was never settled; the stable form was the other side of the same fact — exactly two of that library's types carry an asynchronous dispose. Look for the smaller set the claim implies before deleting a claim the prose still needs.
