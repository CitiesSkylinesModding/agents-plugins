...
// The server has no configuration surface: the game is found by listening for the PlayerConnection
// beacon it multicasts.
builder.Services.AddSingleton<BeaconListener>();

// One persistent debugger session shared by every tool. The container owns it and disposes it on
// shutdown, which resumes the game and frees the exclusive debugger slot.
builder.Services.AddSingleton<UnitySession>();
...
// Realize the listener before the transport serves anything: a lazily created singleton would join
// the multicast group only on the first tool call, missing every beacon sent until then.
