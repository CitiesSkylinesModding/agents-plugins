...
internal sealed class Ecs {
  ...
  public Value CreateQuery(TypeMirror[] componentTypes) {
    ...
    var cts = componentTypes
      .Select(t =>
        this.inv.InvokeStatic(
          ctType,
    ...
    var arr = (ArrayMirror)this.inv.InvokeStatic(
      ...
      ctType.GetTypeObject(), this.inv.Prim(componentTypes.Length)
    );
    ...
    return this.inv.Invoke(
      this.EntityManager,
      this.inv.FindMethod(
        this.EntityManagerType, "CreateEntityQuery", 1, paramTypes: ["ComponentType[]"]
      ),
      arr
    );
  }
  ...
  public StructMirror FindEntity(Value query, int index, int? version) {
    ...
    for (var offset = 0; offset < arr.Length; offset += chunk) {
      ...
      foreach (var v in slice) {
        ...
        if (
          (int)((PrimitiveValue)e["Index"]).Value == index &&
  ...
  public Value GetComponent(StructMirror entity, TypeMirror componentType) {
    var method = this.inv
      .FindMethod(this.EntityManagerType, "GetComponentData", 1, 1, ["Entity"])
  ...
  public void SetComponent(StructMirror entity, TypeMirror componentType, StructMirror value) {
    var method = this.inv
      .FindMethod(this.EntityManagerType, "SetComponentData", 2, 1, ["Entity"])
      .MakeGenericMethod([componentType]);
  ...
  public Value GetBuffer(StructMirror entity, TypeMirror elementType) {
    var m = this.inv
      .FindMethod(this.EntityManagerType, "GetBuffer", 2, 1, ["Entity"])
  ...
  public Value ParseFieldValue(TypeMirror fieldType, string raw) {
    if (fieldType.FullName == "Unity.Entities.Entity") {
      ...
      return this.MakeEntity(int.Parse(parts[0]), parts.Length > 1 ? int.Parse(parts[1]) : 1);
    }

    if (fieldType.IsEnum) {
      return this.inv.Vm.CreateEnumMirror(fieldType, this.inv.Prim(int.Parse(raw)));
    }

    object parsed = fieldType.FullName switch {
      "System.Int32" => int.Parse(raw),
      "System.UInt32" => uint.Parse(raw),
      "System.Int64" => long.Parse(raw),
      "System.UInt64" => ulong.Parse(raw),
      "System.Int16" => short.Parse(raw),
      "System.UInt16" => ushort.Parse(raw),
      "System.Byte" => byte.Parse(raw),
      "System.SByte" => sbyte.Parse(raw),
