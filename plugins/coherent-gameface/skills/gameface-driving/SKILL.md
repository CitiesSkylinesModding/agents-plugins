...
CS2-verified watcher behavior: the mod-file watcher is event-driven (a native file-system watcher) behind a ~0.5s debounce that every new write re-arms; a trigger swaps to a holding page at once and swaps back half a second after the last write, so a tight burst lands as one away/back pair (counted as +2), while a slower build holds the view away for its whole write span — a hold that can outlast the quiescence window and satisfy `game_wait` while the holding page is still up.
So on CS2, prefer the `location.reload()` idiom for determinism; when relying on the watcher, still budget generously (`game_wait({reload: true, timeoutMs: 45000, ...})`) — the wait is dominated by the rebuild itself, not the trigger — and on a reload that never comes, write once more before diagnosing — a build that deleted the watched folder leaves the watcher re-arming for a few seconds, dropping writes meanwhile — then treat a persistent absence as a mod that is not watched or a dev loop that was never enabled, not as watcher jitter.
