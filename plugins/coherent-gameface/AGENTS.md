...
- `mcp/tests/`: the server's unit tests (`mise test:gameface`), hermetic and typed against Bun through their own tsconfig. A facet reaches them by taking an injected CDP facade, which is what makes it testable without a live application (the console pipeline and the debugger session), by being pure to begin with (the selector diagnosis), or, for a page-context function, by running against a DOM the test installs on `globalThis` and removes after (the fill, type, click and hover functions, on happy-dom).
...
Re-probing it for a new engine version updates that module and, in each of the two skills, the whitelist sentence, the `:nth-child()` argument sentence, the wrongly-answered sentence (`::slotted()`, `::part()`, `::selection`) and the `:host`-scope sentence beside it. A test binds the whitelist and `:nth-child()` copies, reading the skills' sentences off disk; the other two carry no binding, and neither does the `tooling-workflow.md` Shadow DOM entry, so a re-probe re-reads all three by hand. The `version-gating.md` timeline is deliberately not among them: it dates a feature's arrival, and how well the JS query APIs answer it is a different fact family. The [probe catalogue](../../.agents/skills/gameface-update/probes.md) lists the carriers with the entries that settle each.
