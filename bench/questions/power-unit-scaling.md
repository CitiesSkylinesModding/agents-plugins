...
- `:29265` — `        [jc.Power]: (e, t, n) =>`
- `:29266` — `          Math.abs(t) < 1e4`
- `:29267` — `            ? Xc(e, t / 10, Sc.Common.VALUE_KILOWATT, n, 1)`
- `:29268` — `            : Xc(e, t / 1e4, Sc.Common.VALUE_MEGAWATT, n, 2),`
...
Raw 5000 takes the kilowatt branch and becomes 500. In `Xc` (`source.js:29333`) the default rounding cutoff is `r = 100`, and 500 exceeds it, so the value prints through `l = c.toFixed(0)` (`:29341`) as `500`. Raw 10,000 takes the megawatt branch and becomes 1, which is under the cutoff and so goes through `toFixed(2)` to `"1.00"`, then the trailing-zero regex at `:29331` trims it to `1`.
...
Two further formatter tables carry their own power arms — bounds at `source.js:29549` and fractions at `:29630` — and both threshold on the max/total argument rather than on the value, so a small value inside a large bound renders in MW.
